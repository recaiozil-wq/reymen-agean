...
def cozum_kaydet(imza: str, hata_tipi: str, hata_ozet: str,
                 cozum_kodu: str, kaynak_script: str, basarili: bool):
    ts = datetime.now().isoformat()
    with _db() as con:
        mevcut = con.execute(
            "SELECT basarili, basari_sayisi FROM cozumler WHERE imza=?",
            (imza,)).fetchone()
        if mevcut is None:
            con.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?)
            """, (imza, hata_tipi, hata_ozet[:500], cozum_kodu,
                  kaynak_script, int(basarili),
                  1 if basarili else 0, ts, ts))
        elif basarili:
...
def ttl_temizle():
    ...
    with _db() as con:
        ...
        """, (sinir, TTL_MUAF_BASARI)).rowcount
    if silinen:
...
def istatistik() -> dict:
    ...
    with _db() as con:
        ...
        basarili = con.execute(
            "SELECT COUNT(*) FROM cozumler WHERE basarili=1").fetchone()[0]
    return {"toplam": toplam, "basarili": basarili,
...
def backoff_bekle(deneme: int) -> float:
    ...
    """
    bekleme = min(RETRY_TABAN_BEKLEME * (RETRY_CARPAN ** (deneme - 1)), RETRY_MAX_BEKLEME)
    return bekleme
...
class OgrenmeDongusu:
    ...
    def ogren(self, hata: Exception, kod: str = "", kaynak: str = "") -> str | None:
        ...
        for deneme in range(1, self.max_deneme + 1):
            ...
            if deneme > 1:
                logger.info("[öğrenme] ⏳ Backoff: %.1fs (deneme %d/%d)", bekleme, deneme, self.max_deneme)
                time.sleep(bekleme)
            ...
            try:
                ...
                if cozum and not cozum.startswith("[COZ]"):
                    # Çözümü kaydet
                    cozum_kaydet(imza, hata_tipi, hata_ozet, cozum, kaynak, basarili=True)
                    self._basarili_ogrenme += 1
    ...
    def istatistik(self) -> dict:
        ...
        return {
            ...
            "basari_orani": (self._basarili_ogrenme / self._toplam_ogrenme * 100) if self._toplam_ogrenme > 0 else 0,
        }
...
def motor_ogren(motor, hata: Exception, kod: str = "", kaynak: str = "") -> str | None:
    ...
    if motor and hasattr(motor, "_log"):
        motor._log(f"Öğrenme: {type(hata).__name__} → {'çözüldü' if cozum else 'çözülemedi'}")
