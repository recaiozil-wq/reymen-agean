...
UYARI: Bu arac exec() kullanir. Yalnizca guvenilir girdilerle kullanilmalidir.
Kod calistirilmadan once _guvenlik_kontrol() ile zararli moduller filtrelenir.
...
def _log_guvenlik_uyarisi(kod: str, namespace: dict) -> None:
    """Exec cagrisi oncesi guvenlik uyarisi logla (PRODUCTION: harici SIEM'e gonder)."""
    uzunluk = len(kod)
    warnings.warn(
        f"[EXECUTE_CODE] Guvenlik denetimi: kod={uzunluk}ch, "
        f"namespace={len(namespace)} sembol, "
        f"builtins={'__builtins__' in namespace}"
    )
...
def _kod_calistir(kod: str, timeout: int = 30, calisma_dizini: str = "") -> dict:
    ...
    try:
        ...
        # GUvenlik denetim logu (PRODUCTION: harici logging servisine gonder)
        _log_guvenlik_uyarisi(kod, temiz_globals)

        # Kodu çalıştır (timeout manuel olarak exec içinde kontrol edilmez)
        exec(duzgun_kod, temiz_globals)  # nosec
