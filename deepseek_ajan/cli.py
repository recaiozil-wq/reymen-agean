...
from deepseek_ajan.hafiza import hafizada_ara, kaydet as h_kaydet, basari_kaydet, hata_kaydet, istatistik, temizle
from deepseek_ajan.session import listele as s_listele, kaydet as s_kaydet, yukle as s_yukle
...
def repl():
    ...
    messages = [{"role": "system", "content": "Sen yardimsever bir AI asistanisin. Turkce cevap ver."}]
    ...
    while True:
        ...
        if ui.startswith("/"):
            ...
            if cmd == "exit":
                ...
                for s in s_listele():
                    print(f"  {R.C}{s['id']}.{R.R} {s['name']} ({s['mesaj']} mesaj, {s['tarih']})")
            elif cmd == "model":
                if arg:
                    model = arg
                ...
                s = istatistik()
                print(f"  Kayit: {s['kayit']}, Basari: {s['basari']}, Hata: {s['hata']}")
            elif cmd == "clear":
                ...
                if arg:
                    if _onayla("py", arg[:80]):
                        r = cmd_py(arg)
                        if r.output: print(f"  {R.G}{r.output}{R.R}")
                        if r.error: print(f"  {R.R2}{r.error}{R.R}")
                else:
