...
VERSION_FILE = REPO_ROOT / "ReYMeN_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "ReYMeN@marian.local": "Schrotti77",
    ...
    "274096618+ReYMeN-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "ReYMeN@nousresearch.com": "NousResearch",
    ...
    "ReYMeN@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ReYMeN_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ReYMeN_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: ReYMeN-eval community link)
    ...
    "ReYMeNagent26@gmail.com": "ReYMeNagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+ReYMeNagent26@users.noreply.github.com": "ReYMeNagent26",
    ...
    "vigo@ReYMeN": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.ReYMeN@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local ReYMeN runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ReYMeN_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live ReYMeN
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"ReYMeN-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "ReYMeN@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/ReYMeN-agent",
    ...
    lines.append(f"# ReYMeN Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for ReYMeN Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="ReYMeN Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  ReYMeN Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"ReYMeN Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"ReYMeN Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'ReYMeN Agent v{new_version} ({calver_date})' "
