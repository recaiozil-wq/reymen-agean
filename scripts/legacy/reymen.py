...
if not API_KEY:
    print(
        "Kurulum tamam. Baslatmak icin API anahtarini tanimla, terminali yeniden ac, 'python reymen.py' yaz."
    )
    sys.exit(0)
...
def sor(p):
    r = client.chat.completions.create(
        model=MODEL, max_tokens=3000, messages=[{"role": "user", "content": p}]
    )
...
def calistir(d):
    try:
        s = subprocess.run(
            [sys.executable, d], capture_output=True, text=True, timeout=TIMEOUT
        )
...
def guvenli_paket_kur(komut):
    ...
    if not m:
        return False
    paket = m.group(1)
    if paket not in IZINLI_PAKET:
        return False
    s = subprocess.run(
        [sys.executable, "-m", "pip", "install", paket], capture_output=True, text=True
    )
