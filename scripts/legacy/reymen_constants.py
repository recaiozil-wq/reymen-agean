...
def _get_platform_default_reymen_home() -> Path:
...
def get_reymen_home() -> Path:
    """Return the ReYMeN home directory (default: platform-native path).

    Reads REYMEN_HOME env var, falls back to the platform-native default.
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_reymen_home()
    ...
    return _get_platform_default_reymen_home()


def get_default_reymen_root() -> Path:
    """Return the root ReYMeN directory for profile-level operations.

    In standard deployments this is the platform-native ReYMeN home
    (``~/.reymen`` on POSIX, ``%LOCALAPPDATA%\\hermes`` on native Windows).
    ...
    """
    native_home = _get_platform_default_reymen_home()
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding ReYMeN profile HOME.

    ``REYMEN_HOME`` scopes ReYMeN state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={REYMEN_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{REYMEN_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{REYMEN_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply ReYMeN' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def is_container() -> bool:
    ...
    See: NousResearch/reymen-agent#47111
    """
