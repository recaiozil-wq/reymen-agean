...
def analyze_task(task_dir):
    ...
    if os.path.isfile(sol_py):
        strategies.append({"type": "solution_py", "file": sol_py, "priority": 2})

    # Priority 3: manual Python (fallback)
    strategies.append({"type": "manual", "priority": 3})
    ...
    if cmd_name in recovery_attempts:
        for alt in recovery_attempts[cmd_name]:
            ...
            with open(tmp_path, "w") as f:
                f.write(modified)
            rc, out, err, etype = run_in_docker_verbose(
                task_dir, f"bash /app/{os.path.basename(tmp_path)}"
            )
            os.remove(tmp_path)
            if rc == 0:
...
def recover_pip_error(task_dir, solution_path):
    ...
    modified = modified.replace(
        "uv add", "uv add --no-sync 2>/dev/null; uv sync 2>/dev/null || "
    )
...
def recover_import_error(task_dir, solution_path):
    ...
    if to_install:
        ...
        with open(tmp_path, "w") as f:
            f.write(modified)
        rc, out, err, etype = run_in_docker_verbose(
            task_dir, f"bash /app/{os.path.basename(tmp_path)}", timeout=120
        )
        os.remove(tmp_path)
        if rc == 0:
        ...
        if strategy["type"] == "solution_sh":
            ...
            if rc == 0:
                ...
                if analysis["has_tests"]:
                    trc, tout, terr = run_in_docker(
                        task_dir,
                        "cd /app && pip install pytest -q --break-system-packages 2>/dev/null && pytest tests/ -v 2>&1",
                        180,
                    )
                ...
                # Timeout durumunda daha uzun süre dene
                    ...
                    rc2, out2, err2, etype2 = run_in_docker_verbose(
                        task_dir, "bash /app/solution.sh", timeout=300
                    )
            ...
            rc, out, err, etype = run_in_docker_verbose(
                task_dir, "python3 /app/solution.py"
            )
            attempt["rc"] = rc
            attempt["error_type"] = etype
            if rc == 0:
...
def run_all_with_recovery():
    ...
    print(f"\n{'='*60}")
    print(
        f"📊 SONUÇ: ✅ {passed} başarılı + 🔄 {recovered} recovery = {passed+recovered}/{len(all_tasks)}"
    )
