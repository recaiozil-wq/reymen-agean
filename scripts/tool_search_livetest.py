#!/usr/bin/env python3
"""Live test harness for ReYMeN Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any ReYMeN imports.
ORIGINAL_HOME = os.environ.get("ReYMeN_HOME")
ORIGINAL_AUTH = Path.home() / ".ReYMeN" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.ReYMeN/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.ReYMeN/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="ReYMeN_ts_live_"))
    ReYMeN_home = home_dir / ".ReYMeN"
    ReYMeN_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, ReYMeN_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".ReYMeN" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, ReYMeN_home / ".env")
        ...
        load_ReYMeN_dotenv(ReYMeN_home=str(Path.home() / ".ReYMeN"))
    ...
    (ReYMeN_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return ReYMeN_home
...
def reset_module_state():
    """Drop cached modules so the new ReYMeN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "ReYMeN_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ReYMeN_HOME"] = str(home)
...
def main():
    ...
    # Restore original ReYMeN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ReYMeN_HOME"] = ORIGINAL_HOME
    else:
