...
def trend_hesapla(onceki_skor, suanki_skor):
    ...
    for kategori in ["easy", "medium", "hard"]:
        onceki = (
            onceki_skor.get("stats", {}).get(f"{kategori}_solved", 0)
            if onceki_skor
            else 0
        )
        ...
        onceki_liste = (
            set(onceki_skor.get("solved", {}).get(kategori, []))
            if onceki_skor
            else set()
        )
...
def get_task_metadata(task_adi):
    ...
    task_dir = os.path.join(
        PROJE_KOKU, "terminal-bench-benchmark", "original-tasks", task_adi
    )
    ...
    if recovery:
        ...
        if rec_list:
            rec_basarili = sum(
                1
                for r in rec_list
                if isinstance(r, dict) and r.get("status") == "solved"
            )
...
def main():
    ...
    if onceki_rapor:
        ...
        onceki_tarih = (
            os.path.basename(onceki_rapor)
            .replace("tbench-report-", "")
            .replace(".md", "")
        )
        trend_data = skor_db
    else:
    ...
    print(f"\n📊 ÖZET:")
    print(
        f"   Easy:   {stats['easy_solved']}/{stats['easy_total']} (%{stats['easy_percent']})"
    )
    print(f"   Medium: {stats['medium_solved']}/{stats['medium_total']} (%{med_pct})")
    print(f"   Hard:   {stats['hard_solved']}/{stats['hard_total']} (%{hard_pct})")
    metadata = skor_db.get("metadata", {})
    print(
        f"   Toplam: {stats['total_solved']}/{metadata.get('total_tasks', stats['easy_total'])} (%{stats['total_percent']})"
    )
