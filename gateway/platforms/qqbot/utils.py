...
def _get_ReYMeN_version() -> str:
    """Return the ReYMeN-agent package version, or 'dev' if unavailable."""
    try:
        from importlib.metadata import version
        return version("ReYMeN-agent")
    except Exception:
        return "dev"
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; ReYMeN/<ReYMeN_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; ReYMeN/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    ReYMeN_version = _get_ReYMeN_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; ReYMeN/{ReYMeN_version})"
