...
ReYMeN middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing ReYMeN'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before ReYMeN calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving ReYMeN retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving ReYMeN guardrails,
...
ReYMeN stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, ReYMeN runs
them as a nested chain in registration order. Middleware failures are fail-open:
ReYMeN logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, ReYMeN applies middleware in this order:
...
For each tool call, ReYMeN applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal ReYMeN pre-execution path against the effective arguments:
...
For isolated local testing, use one `REYMEN_HOME_PATH` for plugin enablement and the
agent run:

```bash
export REYMEN_HOME_PATH=/tmp/hermes-middleware-test
mkdir -p "$REYMEN_HOME_PATH"
hermes plugins enable <plugin-name>
...
Return the same response shape ReYMeN expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, ReYMeN treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, ReYMeN preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. ReYMeN does not convert downstream
