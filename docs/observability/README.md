# ReYMeN Observer Hooks

ReYMeN observer hooks are the read-only telemetry contract for plugins that
...
They do not replace ReYMeN' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. ReYMeN catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so ReYMeN only builds
