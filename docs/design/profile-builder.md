...
needs, and a profile is just a REYMEN_HOME_PATH directory — so the dashboard is the
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — REYMEN_HOME_PATH override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with REYMEN_HOME_PATH override |
...
### Seam #1 — hub-skill install cannot use the REYMEN_HOME_PATH override

`tools/skills_hub.py` binds `SKILLS_DIR = REYMEN_HOME_PATH / "skills"` at **module
...
A fresh subprocess re-imports `skills_hub` with the profile's REYMEN_HOME_PATH bound
...
2. write model (synchronous, REYMEN_HOME_PATH override)
3. write selected MCP servers (synchronous, REYMEN_HOME_PATH override)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, REYMEN_HOME_PATH override
...
page afterward. Mirror `_write_profile_model`'s REYMEN_HOME_PATH-override helper for
...
- Backend E2E with isolated REYMEN_HOME_PATH: POST a full create body
