...
- **ReYMeN Agent double-nesting:** Nous Research ReYMeN Agent zip'ten çıkarılırken `hermes/hermes/` şeklinde çift klasör oluşur. Dosyaları bir katman yukarı taşı: `mv hermes/* . && rmdir hermes`
...
- **pip install -e . hatası:** ReYMeN Agent zaten çalışıyorsa `hermes.exe` kilitli olabilir. Normal `pip install` yeterli.
