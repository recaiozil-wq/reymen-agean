...
title: "ReYMeN Migration Restore"
tags: [automation, devops, system, tor]
description: >-
  ReYMeN Agent'i yeni bir bilgisayara tek GitHub linkiyle full restore etme.
...
ReYMeN Agent'in **tüm yapılandırmasını, skill'lerini, hafızasını ve konuşma geçmişini** tek bir GitHub reposu (`ReYMeN-full-backup`) + tek PowerShell scriptiyle yeni bilgisayara taşımak. Tek link: `github.com/latent-hermes/ReYMeN-full-backup`. Güncellemelerde aynı script `-Update` parametresiyle çalışır.

## Repo Yapısı

```
ReYMeN-full-backup/
├── skills/                          # Tüm ReYMeN skill'leri (30+ kategori)
├── ReYMeN Memor/
...
```powershell
# 1. ReYMeN Agent'i kur (https://hermes-agent.nousresearch.com/docs)
# 2. Repoyu klonla
git clone https://github.com/latent-hermes/ReYMeN-full-backup.git
cd ReYMeN-full-backup
...
2. **ReYMeN durdurma**: Varsa çalışan ReYMeN process'i kill
...
1. **state.db kilitli olabilir** — ReYMeN çalışırken state.db kilitlenir. Zip almadan önce `Stop-Process -Name hermes` ile durdur.
...
6. **Restore sırasında ReYMeN kapalı olmalı** — Script otomatik kapatır, ama el ile kapatmak daha güvenli.
7. **🔥 KRİTİK: Repodaki diğer dosyalara dokunma** — Repo'da ReYMeN dışı projeler de var (AmbientEar, LiveTranscriber, gmod_trainer, test.txt, state zips). Bunları asla silme/taşıma/değiştirme. Sadece `skills/`, `ReYMeN Memor/`, `hermes-config-template.yaml`, `hermes-full-restore.ps1` ve `README.md` güncellenir. Diğer dosyalar için kullanıcıdan açık onay al.
