#!/usr/bin/env python3
"""Google Workspace API CLI for ReYMeN Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing ReYMeN-facing JSON contract and falls back to the Python client
...
from _ReYMeN_home import get_ReYMeN_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ReYMeN_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for ReYMeN Agent")
