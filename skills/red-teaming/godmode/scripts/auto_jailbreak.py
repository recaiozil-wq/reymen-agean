...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ReYMeN_HOME", os.path.expanduser("~/.ReYMeN")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ReYMeN_HOME", Path.home() / ".ReYMeN")) / "skills" / "red-teaming" / "godmode"
...
ReYMeN_HOME = Path(os.getenv("ReYMeN_HOME", Path.home() / ".ReYMeN"))
CONFIG_PATH = ReYMeN_HOME / "config.yaml"
PREFILL_PATH = ReYMeN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/ReYMeN models — already uncensored, just needs clean prompt
    "ReYMeN": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "ReYMeN" in model_lower or "nous" in model_lower:
        return "ReYMeN"
...
def _get_current_model() -> tuple:
    """Read current model and provider from ReYMeN config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.ReYMeN/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart ReYMeN for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart ReYMeN for changes to take effect.")
