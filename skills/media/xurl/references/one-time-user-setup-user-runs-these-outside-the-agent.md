...
> **Docker HOME pitfall:** In the official ReYMeN Docker layout, `/opt/data` is `REYMEN_HOME_PATH`, but ReYMeN tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for ReYMeN-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, ReYMeN tool calls will not see the credentials.
