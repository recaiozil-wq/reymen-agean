...
def list_sessions():
    ...
    for i, s in enumerate(ss, 1):
        n = s.stem; sz = sum(1 for _ in open(s))
...
def cmd_py(code):
    try:
        ...
        with redirect_stdout(fo), redirect_stderr(fe):
            exec(code)
        if fo.getvalue(): print('  ' + R.G + fo.getvalue().strip() + R.R)
        if fe.getvalue(): print('  ' + R.R2 + fe.getvalue().strip() + R.R)
...
def cmd_sys(cmd):
    try:
        ...
        if r.stderr: print('  ' + R.R2 + r.stderr.strip() + R.R)
        if r.returncode != 0: print('  ' + R.R2 + 'Exit: ' + str(r.returncode) + R.R)
...
def help():
    ...
    for k, v in [
        ('/help', 'Bu mesaj'), ('/exit', 'Cikis (Ctrl+C)'),
        ('/new', 'Yeni session'), ('/save <ad>', 'Session kaydet'),
        ('/load <ad>', 'Session yukle'), ('/list', 'Session listele'),
        ('/model <ad>', 'Model degistir'), ('/clear', 'Ekran temizle'),
        ('/py <kod>', 'Python calistir'), ('/sys <komut>', 'Shell calistir'),
    ]:
        print(f'  {R.G}{k}{R.R}  {v}')
    print()
...
def repl():
    ...
    msgs = [{'role': 'system', 'content': 'Sen yardimsever bir AI asistanisin. Turkce cevap ver.'}]
    ...
    while True:
        ...
        if ui.startswith('/'):
            ...
            if cmd == 'exit': print(f'{R.Y}Gorusuruz!{R.R}'); break
            elif cmd == 'help': help()
            elif cmd == 'new': msgs = msgs[:1]; print('  ' + R.G + 'Yeni session.' + R.R)
            elif cmd == 'save': print('  ' + R.G + 'Kaydedildi: ' + str(kaydet(arg, msgs)) + R.R)
            elif cmd == 'load':
                loaded = yukle(arg)
                if loaded: msgs = loaded
            elif cmd == 'list': list_sessions()
            elif cmd == 'model':
                if arg: model = arg; print('  ' + R.G + 'Model: ' + model + R.R)
                else: print('  Model: ' + R.C + model + R.R)
            elif cmd == 'clear': os.system('cls' if os.name == 'nt' else 'clear')
            elif cmd == 'py':
                if arg: cmd_py(arg)
                else: print('  ' + R.Y + 'Kullanim: /py <kod>' + R.R)
            elif cmd == 'sys':
                if arg: cmd_sys(arg)
                else: print('  ' + R.Y + 'Kullanim: /sys <komut>' + R.R)
            else: print('  ' + R.R2 + 'Bilinmeyen: /' + cmd + R.R + ' (/help)')
            continue
