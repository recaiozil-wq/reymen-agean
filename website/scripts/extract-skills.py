...
2. The unified ReYMeN Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                ReYMeN_meta = metadata.get("ReYMeN", {})
                if isinstance(ReYMeN_meta, dict):
                    tags = ReYMeN_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
...
def _guess_category(tags: list) -> str:
    ...
    for tag in tags:
        if not isinstance(tag, str):
            continue
        # Check direct match first, then lowercase
        cat = TAG_TO_CATEGORY.get(tag) or TAG_TO_CATEGORY.get(tag.lower())
        if cat:
            return cat
        # Accept literal curated category keys
        if tag.lower() in CATEGORY_LABELS:
            return tag.lower()
    return "uncategorized"


def _source_url(source: str, identifier: str, extra: dict) -> str:
    """Synthesize a clickable origin URL for a community skill entry.

    Priority:
      1. ``extra["detail_url"]`` — explicit URL provided by the source adapter.
      2. ``extra["source_url"]`` — browse.sh adapter carries origin here.
      3. Synthesized from ``source`` + ``identifier`` shape.

    Built-in / optional skills return ``""`` — they have a ``docsPath`` instead.
    Unknown sources with no identifier also return ``""``.
    """
    if isinstance(extra, dict):
        detail = extra.get("detail_url", "")
        if detail:
            return detail
        src_url = extra.get("source_url", "")
        if src_url:
            return src_url
    ...
    if src in {"built-in", "optional", "official"}:
        return ""
    ...
    if src == "github":
        ...
        if len(parts) >= 3:
            ...
            return f"https://github.com/{owner}/{repo}/tree/main/{subpath}"
        if len(parts) == 2:
            owner, repo = parts[0], parts[1]
    ...
    if src == "clawhub":
        ...
        if slug.startswith("clawhub/"):
            slug = slug[len("clawhub/"):]
        return f"https://clawhub.ai/skills/{slug}"

    if src == "lobehub":
        parts = identifier.split("/")
        slug = parts[-1] if len(parts) > 1 else identifier
        return f"https://lobehub.com/agent/{slug}"

    if src in {"skills.sh", "skills-sh"}:
        # skills.sh detail_url is already checked above
        return ""

    if src == "browse-sh":
        # source_url is already checked above
        return ""
