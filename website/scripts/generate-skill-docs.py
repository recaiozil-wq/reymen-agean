...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/ReYMeN-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    ReYMeN_meta = (fm.get("metadata") or {}).get("ReYMeN") or {}
    tags = ReYMeN_meta.get("tags") or []
    related = ReYMeN_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `ReYMeN skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    return (
        ...
        "\n"
        ":::info\n"
        "The following is the complete skill definition that ReYMeN loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.\n"
        ":::\n"
        "\n"
        f"{body_clean}\n"
    )
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with ReYMeN Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "ReYMeN ships with a large built-in skill library copied into `~/.ReYMeN/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "ReYMeN also syncs bundled skills on `ReYMeN update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.ReYMeN/skills/` tree, it is still shipped with ReYMeN; restore it with `ReYMeN skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with ReYMeN-agent — install via ReYMeN skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with ReYMeN-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "ReYMeN skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "ReYMeN skills install official/blockchain/solana",
        "ReYMeN skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "ReYMeN skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
