    ...
    def test_mesaj_gonder_dm_disabled(self):
        with patch("gateway.platforms.whatsapp._REQUESTS_OK", True):
            with patch("gateway.platforms.whatsapp._token_al", return_value="tok"):
                ...
                with patch(
                    "gateway.platforms.whatsapp._config_str", side_effect=mock_config
                ):
                    from gateway.platforms.whatsapp import mesaj_gonder
        ...
        with patch("gateway.platforms.whatsapp._REQUESTS_OK", True):
            with patch("gateway.platforms.whatsapp._token_al", return_value="tok"):
                ...
                with patch(
                    ...
                    with patch(
                        "requests.post", side_effect=Exception("baglanti hatasi")
                    ):
                        from gateway.platforms.whatsapp import mesaj_gonder
            ...
            with patch("gateway.platforms.whatsapp._token_al", return_value="tok"):
                ...
                with patch(
                    ...
                    with patch("requests.post", return_value=mock_resp):
                        ...
                        result = send_message_json(
                            "90555", {"type": "text", "text": {"body": "test"}}
                        )
...
class TestWhatsAppAdapter:
    ...
    def test_adapter_send(self):
        with patch(
            "gateway.platforms.whatsapp.mesaj_gonder",
            return_value="[WhatsApp]: Mesaj gonderildi (ID: wamid.1)",
        ):
            from gateway.platforms.whatsapp import WhatsAppAdapter
