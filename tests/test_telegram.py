        ...
        with patch("gateway.platforms.telegram._token_al", return_value="tok"):
            with patch("gateway.platforms.telegram._REQUESTS_OK", True):
                with patch("requests.post", return_value=mock_resp) as mock_post:
                    ...
                    result = set_webhook(
                        "https://example.com/hook", drop_pending_updates=True
                    )
                    assert result["durum"] == "basarili"
                    assert (
                        mock_post.call_args[1]["json"]["drop_pending_updates"] is True
                    )
...
class TestTelegramSendStream:
    def test_send_stream_short_message(self):
        ...
        with patch("gateway.platforms.telegram._token_al", return_value="tok"):
            with patch("gateway.platforms.telegram._REQUESTS_OK", True):
                with patch(
                    "gateway.platforms.telegram.send_message",
                    return_value={
                        "durum": "basarili",
                        "mesaj_id": 1,
                        "chunk_sayisi": 1,
    ...
    def test_send_stream_long_message(self):
        with patch("gateway.platforms.telegram._token_al", return_value="tok"):
            with patch("gateway.platforms.telegram._REQUESTS_OK", True):
                with patch(
                    "gateway.platforms.telegram.send_message",
                    return_value={"durum": "basarili", "mesaj_id": 1},
                ):
                    with patch(
                        "gateway.platforms.telegram.edit_message",
                        return_value={"durum": "basarili"},
                    ):
