...
def _mock_urlopen_httperror(
    url: str = "http://example.com", code: int = 400, msg: str = "Bad Request"
) -> None:
    ...
    def test_callback_get_user_info_hata(self):
        ...
        with patch.dict(os_module.environ, self.ENV, clear=True):
            ...
            with patch.object(
                ...
                with patch.object(
                    p, "get_user_info", side_effect=ValueError("API hatasi")
                ):
...
class TestGitHubOAuthProviderHTTP:
    ...
    def test_callback_get_user_info_hata(self):
        ...
        with patch.dict(os_module.environ, self.ENV, clear=True):
            ...
            with patch("urllib.request.urlopen", return_value=mock_resp):
                with patch.object(
                    p, "get_user_info", side_effect=ValueError("API down")
                ):
                    token = p.callback_handler("code")
    ...
    def test_callback_http_error(self):
        ...
        with patch.dict(os_module.environ, self.ENV, clear=True):
            p = GitHubOAuthProvider()
            with patch(
                "urllib.request.urlopen", side_effect=_mock_urlopen_httperror(code=401)
            ):
                with pytest.raises(OAuthError) as exc:
    ...
    def test_get_user_info_http_error(self):
        ...
        with patch.dict(os_module.environ, self.ENV, clear=True):
            p = GitHubOAuthProvider()
            with patch(
                "urllib.request.urlopen", side_effect=_mock_urlopen_httperror(code=403)
            ):
                with pytest.raises(OAuthError) as exc:
...
class TestDiscordOAuthProviderHTTP:
    ...
    def test_callback_basic_auth_header(self):
        ...
        with patch.dict(os_module.environ, self.ENV, clear=True):
            p = DiscordOAuthProvider()
            expected_basic = base64.b64encode(b"dc-id:dc-secret").decode()
            mock_resp = _mock_urlopen_response(
                {
                    "access_token": "t",
                    "expires_in": 3600,
    ...
    def test_callback_get_user_info_hata(self):
        ...
        with patch.dict(os_module.environ, self.ENV, clear=True):
            ...
            with patch("urllib.request.urlopen", return_value=mock_resp):
                with patch.object(p, "get_user_info", side_effect=ValueError("fail")):
                    token = p.callback_handler("code")
                    assert token.access_token == "t"
    ...
    def test_callback_http_error(self):
        ...
        with patch.dict(os_module.environ, self.ENV, clear=True):
            p = DiscordOAuthProvider()
            with patch(
                "urllib.request.urlopen", side_effect=_mock_urlopen_httperror(code=400)
            ):
                with pytest.raises(OAuthError) as exc:
                    p.callback_handler("bad-code")
    ...
    def test_get_user_info_http_error(self):
        ...
        with patch.dict(os_module.environ, self.ENV, clear=True):
            p = DiscordOAuthProvider()
            with patch(
                "urllib.request.urlopen", side_effect=_mock_urlopen_httperror(code=429)
            ):
                with pytest.raises(OAuthError) as exc:
                    p.get_user_info("bad-token")
                assert exc.value.provider == "discord"
...
class TestOAuthManagerRefresh:
    ...
    def test_refresh_no_refresh_token(self, tmp_depo):
        ...
        with patch.dict(
            ...
            t = OAuthToken(
                access_token="old", provider="google", expires_in=3600, refresh_token=""
            )
            tmp_depo.kaydet(t)
            with pytest.raises(OAuthError) as exc:
                om.refresh("google")
    ...
    def test_refresh_github_no_refresh_needed(self, tmp_depo):
        ...
        with patch.dict(
            ...
            om = OAuthManager(deposu=tmp_depo)
            t = OAuthToken(
                access_token="old", provider="github", expires_in=3600, refresh_token=""
            )
            tmp_depo.kaydet(t)
            with pytest.raises(OAuthError) as exc:
                om.refresh("github")
    ...
    def test_refresh_basarili(self, tmp_depo):
        ...
        from reymen.core.oauth_manager import (
            OAuthManager,
            OAuthToken,
            GoogleOAuthProvider,
        )
...
class TestOAuthManagerGecerliToken:
    ...
    def test_expired_with_refresh_fails(self, tmp_depo):
        ...
        with patch.dict(
            ...
            tmp_depo.kaydet(t)
            with patch.object(om, "refresh", side_effect=OAuthError("refresh failed")):
                result = om.gecerli_token("google")
