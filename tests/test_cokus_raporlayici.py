...
class TestCokusRaporuUret:
    ...
    def test_temel_rapor_olusumu(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Test gorev", deneme_sayisi=5,
    ...
    def test_dosyaya_yazma(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            cokus_raporlayici.cokus_raporu_uret(
                gorev="Yazma testi", deneme_sayisi=1,
                hata_gecmisi=["hata"], denenen_ajanlar=["a"],
            )
    ...
    def test_bos_hata_gecmisi(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Bos hata", deneme_sayisi=3,
                hata_gecmisi=[], denenen_ajanlar=[],
            )
    ...
    def test_bos_ajan_listesi(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Bos ajan", deneme_sayisi=2,
                hata_gecmisi=["hata"], denenen_ajanlar=[],
            )
    ...
    def test_tiklanma_nedeni_otomatik(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Otomatik neden", deneme_sayisi=4,
    ...
    def test_tiklanma_nedeni_verilen(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Verilen neden", deneme_sayisi=4,
                hata_gecmisi=["hata1"], denenen_ajanlar=["x"],
    ...
    def test_ajanlar_sirali_ve_benzersiz(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Siralama", deneme_sayisi=3,
                hata_gecmisi=["h"], denenen_ajanlar=["z_ajan", "a_ajan", "z_ajan"],
            )
    ...
    def test_unicode_karakterler(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Şşş İiççğğüü Ööö", deneme_sayisi=1,
                hata_gecmisi=["ğüşıçö"], denenen_ajanlar=["test"],
            )
        assert "Şşş" in rapor
    ...
    def test_cok_sayida_hata(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            ...
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Cok hata", deneme_sayisi=150,
                hata_gecmisi=hatalar, denenen_ajanlar=["test"],
            )
    ...
    def test_rapor_sablon_yapisi(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            rapor = cokus_raporlayici.cokus_raporu_uret(
                gorev="Sablon test", deneme_sayisi=2,
                hata_gecmisi=["h"], denenen_ajanlar=["a"],
            )
    ...
    def test_dosya_adi_zamani_icerir(self, tmp_path):
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            cokus_raporlayici.cokus_raporu_uret(
                gorev="Dosya adi", deneme_sayisi=1,
                hata_gecmisi=["h"], denenen_ajanlar=["a"],
            )
        dosyalar = list((tmp_path / ".ReYMeN" / "cokus_raporlari").glob("cokus_*.txt"))
    ...
    def test_dizin_otomatik_olusturulur(self, tmp_path):
        ...
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", yeni_dizin):
            cokus_raporlayici.cokus_raporu_uret(
                gorev="Dizin yok", deneme_sayisi=1,
                hata_gecmisi=["h"], denenen_ajanlar=["a"],
            )
    ...
    def test_coklu_cagri(self, tmp_path):
        ...
        with patch.object(cokus_raporlayici, "RAPOR_DIZINI", tmp_path / ".ReYMeN" / "cokus_raporlari"):
            for i in range(3):
                cokus_raporlayici.cokus_raporu_uret(
                    gorev=f"Cagri {i}", deneme_sayisi=i,
                    hata_gecmisi=[f"hata_{i}"], denenen_ajanlar=["a"],
                )
