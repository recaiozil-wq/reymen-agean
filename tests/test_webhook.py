...
class TestWebhookModuleFunctions:
    ...
    def test_mesaj_gonder_timeout(self):
        with patch("requests.post", side_effect=__import__("requests").Timeout("timeout")):
            from gateway.platforms.webhook import mesaj_gonder
            result = mesaj_gonder("http://example.com/hook", "test")
...
class TestWebhookAdapter:
    ...
    def test_adapter_init_with_config(self):
        ...
        with patch("os.environ.get", lambda k, d=None: "" if k.startswith("WEBHOOK_") else (d or "")):
            from gateway.platforms.webhook import WebhookAdapter
    ...
    def test_adapter_baslat_disabled(self):
        with patch("os.environ.get", lambda k, d=None: {
            "WEBHOOK_ENABLED": "false", "WEBHOOK_PORT": "8644", "WEBHOOK_SECRET": ""
        }.get(k, d or "")):
            from gateway.platforms.webhook import WebhookAdapter
            adapter = WebhookAdapter()
    ...
    def test_adapter_baslat_success(self):
        ...
        with patch("os.environ.get", lambda k, d=None: "8766" if k == "WEBHOOK_PORT" else (d or "")):
    ...
    def test_adapter_gonder(self):
        with patch("gateway.platforms.webhook.mesaj_gonder", return_value="[Webhook]: Gonderildi (HTTP 200)."):
            from gateway.platforms.webhook import WebhookAdapter
            adapter = WebhookAdapter()
...
class TestWebhookHelpers:
    ...
    def test_security_constants(self):
        from gateway.platforms.webhook import _INSECURE_NO_AUTH, _INSECURE_MODES, _DYNAMIC_ROUTES_FILENAME
