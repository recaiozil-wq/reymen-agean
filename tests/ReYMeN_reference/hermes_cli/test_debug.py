"""Tests for ``ReYMeN debug`` CLI command and debug utilities."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from ReYMeN_cli.debug import _upload_paste_rs
        ...
        with patch("ReYMeN_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "ReYMeN_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from ReYMeN_cli.debug import _upload_dpaste_com
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, ReYMeN_home):
    ...
    def test_empty_primary_reports_file_empty(self, ReYMeN_home):
    ...
    def test_race_truncate_after_resolve_reports_empty(self, ReYMeN_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = ReYMeN_home / "logs" / "agent.log"
        from ReYMeN_cli import debug
    ...
    def test_truncates_large_file(self, ReYMeN_home):
        """Files larger than max_bytes get tail-truncated."""
        from ReYMeN_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (ReYMeN_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, ReYMeN_home):
        ...
        (ReYMeN_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, ReYMeN_home):
    ...
    def test_falls_back_to_rotated_file(self, ReYMeN_home):
    ...
    def test_prefers_primary_over_rotated(self, ReYMeN_home):
    ...
    def test_falls_back_when_primary_empty(self, ReYMeN_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of ReYMeN_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def ReYMeN_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated ReYMeN_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".ReYMeN"
        home.mkdir()
        monkeypatch.setenv("ReYMeN_HOME", str(home))
        ...
        monkeypatch.delenv("ReYMeN_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, ReYMeN_home_with_secret):
        from ReYMeN_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, ReYMeN_home_with_secret):
        from ReYMeN_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, ReYMeN_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when ReYMeN_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("ReYMeN_REDACT_SECRETS", "false")

        from ReYMeN_cli.debug import _capture_log_snapshot

        assert os.environ.get("ReYMeN_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, ReYMeN_home_with_secret
    ):
        from ReYMeN_cli.debug import _capture_log_snapshot

        log_path = ReYMeN_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, ReYMeN_home_with_secret):
        from ReYMeN_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from ReYMeN_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, ReYMeN_home):
        from ReYMeN_cli.debug import collect_debug_report
        ...
        assert "--- ReYMeN dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, ReYMeN_home):
        from ReYMeN_cli.debug import collect_debug_report
    ...
    def test_report_includes_errors_log(self, ReYMeN_home):
        from ReYMeN_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, ReYMeN_home):
        from ReYMeN_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, ReYMeN_home):
        from ReYMeN_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, ReYMeN_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from ReYMeN_cli.debug import run_debug_share
    ...
    def test_share_survives_sweep_failure(self, ReYMeN_home, capsys):
    ...
    def test_local_flag_prints_full_logs(self, ReYMeN_home, capsys):
    ...
    def test_share_uploads_four_pastes(self, ReYMeN_home, capsys):
        ...
        assert "--- ReYMeN dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- ReYMeN dump ---" in gateway_paste
    ...
    def test_share_continues_on_log_upload_failure(self, ReYMeN_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, ReYMeN_home, capsys):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        from ReYMeN_cli.debug import run_debug
        ...
        out = capsys.readouterr().out
        assert "ReYMeN debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, ReYMeN_home):
        from ReYMeN_cli.debug import run_debug
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from ReYMeN_cli.debug import delete_paste
...
class TestScheduleAutoDelete:
    ...
    ``~/.ReYMeN/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``ReYMeN debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, ReYMeN_home):
        ...
        import ast
        import inspect
        from ReYMeN_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, ReYMeN_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from ReYMeN_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, ReYMeN_home):
    ...
    def test_merges_with_existing_pending(self, ReYMeN_home):
        """Subsequent calls merge into existing pending.json."""
        from ReYMeN_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, ReYMeN_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, ReYMeN_home):
    ...
    def test_sweep_deletes_expired_entries(self, ReYMeN_home):
        from ReYMeN_cli.debug import (
        ...
        with patch("ReYMeN_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, ReYMeN_home):
    ...
    def test_sweep_survives_network_failure(self, ReYMeN_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, ReYMeN_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, ReYMeN_home):
        from ReYMeN_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("ReYMeN_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, ReYMeN_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: ReYMeN debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from ReYMeN_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("ReYMeN_cli.debug.delete_paste", return_value=True):
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, ReYMeN_home, capsys):
        from ReYMeN_cli.debug import run_debug_share
    ...
    def test_share_shows_privacy_notice(self, ReYMeN_home, capsys):
        from ReYMeN_cli.debug import run_debug_share
    ...
    def test_local_no_privacy_notice(self, ReYMeN_home, capsys):
        from ReYMeN_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``ReYMeN debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, ReYMeN_home):
        from ReYMeN_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("ReYMeN_cli.dump.run_dump"), patch(
            "ReYMeN_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("ReYMeN_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, ReYMeN_home):
        from ReYMeN_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (ReYMeN_home / "logs" / "desktop.log").unlink()
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, ReYMeN_home):
        from ReYMeN_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, ReYMeN_home):
        from ReYMeN_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, ReYMeN_home):
        from ReYMeN_cli.debug import build_debug_share
