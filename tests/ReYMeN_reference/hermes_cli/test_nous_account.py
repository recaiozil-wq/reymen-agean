...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "ReYMeN-cli",
            "product_id": "nous-ReYMeN-agent",
            "nous_client": "ReYMeN-agent",
    ...
    assert info.product_id == "nous-ReYMeN-agent"
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Log in with `ReYMeN model`" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `ReYMeN model`" in message
