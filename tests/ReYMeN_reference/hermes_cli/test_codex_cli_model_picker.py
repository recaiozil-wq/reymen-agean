...
Covers:
 - Normal path (tokens already in ReYMeN auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — ReYMeN
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `ReYMeN auth openai-codex`. The old
...
@pytest.fixture()
def ReYMeN_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in ReYMeN auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(ReYMeN_auth_only_env):
    """openai-codex appears when tokens are already in ReYMeN auth store."""
    from ReYMeN_cli.model_switch import list_authenticated_providers
...
def test_codex_picker_uses_live_codex_catalog(
    ReYMeN_auth_only_env, tmp_path, monkeypatch
):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or ReYMeN
    auth store."""
    ReYMeN_home = tmp_path / ".ReYMeN"
    ReYMeN_home.mkdir()
    ...
    (ReYMeN_home / "auth.json").write_text(json.dumps({"version": 2, "providers": {}}))
