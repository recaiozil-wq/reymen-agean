"""Tests for the top-level `./ReYMeN` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./ReYMeN` should use `ReYMeN_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "ReYMeN"
    called = []

    fake_main_module = types.ModuleType("ReYMeN_cli.main")

    def fake_main():
        called.append("ReYMeN_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "ReYMeN_cli.main", fake_main_module)
