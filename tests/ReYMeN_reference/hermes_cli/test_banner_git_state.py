...
def test_format_banner_version_label_without_git_state():
    ...
    assert value == f"ReYMeN Agent v{banner.VERSION} ({banner.RELEASE_DATE})"
...
def test_get_git_banner_state_falls_back_to_build_sha_when_no_repo():
    ...
    parent nor ``$ReYMeN_HOME/ReYMeN-agent/`` is a git repo (the canonical
    ...
    with patch.object(banner, "_resolve_repo_dir", return_value=None), patch(
...
def test_get_git_banner_state_falls_back_when_live_git_returns_nothing(tmp_path):
    ...
    with patch("ReYMeN_cli.banner.subprocess.run", return_value=failed), patch(
