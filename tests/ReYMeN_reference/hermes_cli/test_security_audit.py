"""Unit tests for ReYMeN_cli.security_audit — parsers + OSV plumbing.
...
from ReYMeN_cli import security_audit as sa
...
class TestRunAudit:
    def test_no_components_returns_empty(self, tmp_path: Path):
        findings = sa.run_audit(
            skip_venv=True, skip_plugins=True, skip_mcp=True, ReYMeN_home=tmp_path
        )
        assert findings == []
...
class TestExitCodes:
    ...
    def test_clean_audit_exits_zero(self, tmp_path: Path, monkeypatch, capsys):
        monkeypatch.setattr(sa, "get_ReYMeN_home", lambda: str(tmp_path))
