"""Tests for the ``ReYMeN send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``ReYMeN_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from ReYMeN_cli import send_cmd
...
def test_load_ReYMeN_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``ReYMeN send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``ReYMeN config set``) lives
    ...
    ReYMeN_home.mkdir()
    (ReYMeN_home / ".env").write_text("SOME_TOKEN=abc123\n")
...
def test_load_ReYMeN_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    ReYMeN_home = tmp_path / ".ReYMeN"
    ReYMeN_home.mkdir()
    (ReYMeN_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_ReYMeN_env_handles_missing_files(tmp_path, monkeypatch):
