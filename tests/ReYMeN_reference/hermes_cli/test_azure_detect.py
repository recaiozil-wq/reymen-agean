"""Tests for ReYMeN_cli.azure_detect — transport & model auto-detection."""
...
from ReYMeN_cli import azure_detect
...
def test_detect_anthropic_path_wins_without_http():
    ...
    with patch.object(azure_detect, "_http_get_json") as fake_get, patch.object(
...
def test_detect_falls_back_to_anthropic_probe():
    ...
    with patch.object(
        azure_detect, "_http_get_json", side_effect=_fake_get
...
def test_detect_all_probes_fail_returns_none():
    ...
    with patch.object(
        azure_detect, "_http_get_json", return_value=(500, None)
...
def test_http_get_json_on_urlerror_returns_zero_none():
    ...
    with patch(
        "ReYMeN_cli.azure_detect.urllib_request.urlopen",
...
def test_http_get_json_on_http_error_returns_code_none():
    ...
    with patch("ReYMeN_cli.azure_detect.urllib_request.urlopen", side_effect=err):
...
def test_lookup_context_length_returns_known():
    ...
    with patch("agent.model_metadata.get_model_context_length", fake), patch(
        "agent.model_metadata.DEFAULT_FALLBACK_CONTEXT", 128000
    ):
...
def test_lookup_context_length_returns_none_on_fallback():
    ...
    with patch(
        "agent.model_metadata.get_model_context_length", return_value=128000
