"""Tests for Bug #12905 fix — stale OAuth token detection in ReYMeN model flow.

Bug 3: `ReYMeN model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.ReYMeN/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            "ReYMeN_cli.secret_prompt.masked_secret_prompt", lambda _: ""
        )

        from ReYMeN_cli.main import _model_flow_anthropic
        ...
        assert (
            "subscription" in output or "API key" in output
