"""Comprehensive tests for ReYMeN_cli.profiles module.
...
from ReYMeN_cli.profiles import (
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["ReYMeN", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the ReYMeN install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_ReYMeN_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".ReYMeN" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".ReYMeN" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.ReYMeN must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".ReYMeN"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes ReYMeN-agent, .worktrees,
        ...
        (default_home / "ReYMeN-agent" / ".git").mkdir(parents=True)
        (default_home / "ReYMeN-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "ReYMeN-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "ReYMeN-agent").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `ReYMeN update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".ReYMeN" / ".env").write_text("KEY=root\n")
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".ReYMeN" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_ReYMeN_home_returns_default(self, profile_env):
        # ReYMeN_HOME points to tmp_path/.ReYMeN which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ReYMeN_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ReYMeN_HOME", str(custom))
        # With Docker-aware roots, a custom ReYMeN_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("ReYMeN")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        ...
        assert "exec /opt/ReYMeN/bin/ReYMeN -p mybot" in content
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "ReYMeN -p mybot" in content
        assert "%*" in content
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "ReYMeN -p redqueen" in content
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".ReYMeN" / "profiles" / "oldname"
        ...
        assert new_dir == tmp_path / ".ReYMeN" / "profiles" / "newname"
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "ReYMeN.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["ReYMeN_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["ReYMeN_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        assert "ReYMeN.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["ReYMeN_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["ReYMeN_heimdall"]["workspace"] == "ReYMeN"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["ReYMeN.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["ReYMeN_heimdall"]["aiPeer"] == "heimdall"
...
class TestInternalHelpers:
    ...
    def test_profiles_root_under_home(self, profile_env):
        ...
        assert root == tmp_path / ".ReYMeN" / "profiles"

    def test_default_ReYMeN_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_ReYMeN_home()
        assert home == tmp_path / ".ReYMeN"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ReYMeN_HOME outside ~/.ReYMeN), profiles go under ReYMeN_HOME."""
    ...
    def test_default_ReYMeN_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_ReYMeN_home() returns ReYMeN_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ReYMeN_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ReYMeN_HOME under ~/.ReYMeN), profiles root is still ~/.ReYMeN/profiles."""
        native = tmp_path / ".ReYMeN"
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ReYMeN_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ReYMeN_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".ReYMeN" / "profiles" / "mybot"
        assert result == expected
