...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # ReYMeN model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in ReYMeN_OVERLAYS
        overlay = ReYMeN_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from ReYMeN_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        assert headers.get("User-Agent", "").startswith("ReYMeNAgent/")

    def test_gmi_profile_declares_ReYMeN_user_agent(self):
        """The GMI plugin sets a ReYMeNAgent/<ver> User-Agent on its profile."""
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("ReYMeN_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "ReYMeN_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["ReYMeN", "chat", "--provider", "gmi"])
