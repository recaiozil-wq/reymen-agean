...
* Be macOS-only — no-op silently on Linux/Windows so ``ReYMeN update`` can
...
class TestInstallCuaDriverUpgrade:
    def test_upgrade_on_non_macos_is_silent_noop(self):
        ...
        with patch.object(tools_config, "_print_warning") as warn, patch(
    ...
    def test_upgrade_on_macos_with_binary_runs_installer(self):
        ...
        with patch("platform.system", return_value="Darwin"), patch.object(
            ...
            if n in {"cua-driver", "curl"}
            else None,
        ), patch.object(
            tools_config, "_check_cua_driver_asset_for_arch", return_value=True
        ), patch.object(
            tools_config, "_run_cua_driver_installer", return_value=True
        ) as runner, patch("subprocess.run"):
    ...
    def test_non_upgrade_on_macos_with_binary_skips_install(self):
        ...
        ), patch.object(tools_config, "_run_cua_driver_installer") as runner, patch(
            "subprocess.run"
        ):
...
class TestCheckCuaDriverAssetForArch:
    ...
    def test_x86_64_without_asset_returns_false(self):
        ...
        with patch("platform.machine", return_value="x86_64"), patch(
            "urllib.request.urlopen", return_value=mock_resp
        ), patch.object(tools_config, "_print_warning") as warn, patch.object(
            tools_config, "_print_info"
        ):
            assert tools_config._check_cua_driver_asset_for_arch() is False
            warn.assert_called_once()
            assert (
                "no Intel" in warn.call_args[0][0].lower()
                or "x86_64" in warn.call_args[0][0]
            )
    ...
    def test_x86_64_api_failure_returns_true(self):
        ...
        with patch("platform.machine", return_value="x86_64"), patch(
            "urllib.request.urlopen", side_effect=Exception("timeout")
        ):
            assert tools_config._check_cua_driver_asset_for_arch() is True
    ...
    def test_fresh_install_x86_64_no_asset_skips_installer(self):
        ...
        with patch("platform.system", return_value="Darwin"), patch.object(
            ...
            "urllib.request.urlopen", return_value=mock_resp
        ), patch.object(tools_config, "_print_warning"), patch.object(
            tools_config, "_print_info"
