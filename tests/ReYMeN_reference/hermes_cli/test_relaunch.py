"""Tests for ReYMeN_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from ReYMeN_cli import relaunch as relaunch_mod


class TestResolveReYMeNBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/ReYMeN"
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_ReYMeN_bin() is None
...
class TestBuildRelaunchArgv:
    ...
    def test_falls_back_to_python_module(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_ReYMeN_bin", lambda: None)
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/ReYMeN", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/ReYMeN", ["/usr/bin/ReYMeN", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\ReYMeN.exe", "chat"]]
...
class TestResolveReYMeNBinWindowsPyGuard:
    """On Windows, resolve_ReYMeN_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        bin_path = relaunch_mod.resolve_ReYMeN_bin()
        # Must NOT be the .py — must be the ReYMeN.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\ReYMeN.exe"
