...
from ReYMeN_cli import config as ReYMeN_config
...
def test_stash_local_changes_if_needed_returns_none_when_tree_clean(
    ...
    stash_ref = ReYMeN_main._stash_local_changes_if_needed(["git"], tmp_path)
...
def test_print_stash_cleanup_guidance_with_selector(capsys):
    ReYMeN_main._print_stash_cleanup_guidance("abc123", "stash@{2}")
...
def test_restore_stashed_changes_always_resets_on_conflict(
    ...
    assert "ReYMeN_cli/main.py" in out
...
def test_stash_local_changes_if_needed_raises_when_stash_ref_missing(
    ...
    with pytest.raises(CalledProcessError):
        ReYMeN_main._stash_local_changes_if_needed(["git"], Path(tmp_path))
...
def test_discard_lockfile_churn_skips_lock_when_package_json_dirty(tmp_path):
    ...
    ReYMeN_main._discard_lockfile_churn(["git"], tmp_path)
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(
    ...
    ReYMeN_main.cmd_update(SimpleNamespace())
...
def test_install_heartbeat_prints_when_dependency_install_is_silent(
    ...
    def fake_run(cmd, **kwargs):
        ReYMeN_main._time.sleep(1.2)
        return SimpleNamespace(returncode=0)

    monkeypatch.setattr(ReYMeN_main.subprocess, "run", fake_run)

    ReYMeN_main._run_install_with_heartbeat(
        ["uv", "pip", "install", "-e", "."],
...
def test_interactive_update_ignores_discard_setting(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ReYMeN_main.sys.stdout, "isatty", lambda: True)

    ReYMeN_main.cmd_update(SimpleNamespace())  # no gateway, no --yes
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``ReYMeN update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(ReYMeN_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".ReYMeN-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by ReYMeN update (ReYMeN_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "ReYMeN-update-autostash")

    assert marker.exists(), (
        ".ReYMeN-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".ReYMeN-bootstrap-complete" not in status
