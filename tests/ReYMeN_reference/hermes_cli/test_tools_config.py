"""Tests for ReYMeN_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `ReYMeN tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "ReYMeN_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "ReYMeN_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `ReYMeN tools`,
    ...
    # User explicitly opted into spotify but not x_search via `ReYMeN tools`.
    config = {"platform_toolsets": {"cli": ["ReYMeN-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[ReYMeN-cli, spotify]`` (composite + configurable) must keep the full
    ``ReYMeN-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``ReYMeN-cli`` on the floor,
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["ReYMeN-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``ReYMeN-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["ReYMeN-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _ReYMeN_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `ReYMeN tools`
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/ReYMeN-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (ReYMeN-cli, ReYMeN-telegram, etc.) must NOT
    ...
    ``ReYMeN tools``, but ReYMeN-cli stays in the config and re-enables
    ...
    # ReYMeN-cli must NOT survive — it's a platform default, not an MCP server
    assert "ReYMeN-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_ReYMeN_telegram():
    """Same bug for Telegram — ReYMeN-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser",
    ...
    assert "ReYMeN-telegram" not in saved
    assert "web" in saved
...
def test_visible_providers_force_fresh_shows_nous_subscription_after_upgrade(
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.tools_config.get_nous_subscription_features",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from ReYMeN_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "ReYMeN_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("ReYMeN_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "ReYMeN_cli.nous_subscription.get_nous_portal_account_info",
    ...
    configured = []
    monkeypatch.setattr(
        "ReYMeN_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from ReYMeN_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """ReYMeN-gateway includes list should cover all messaging platforms."""
        from ReYMeN_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["ReYMeN-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from ReYMeN_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from ReYMeN_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/ReYMeN-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `ReYMeN tools` must reach the
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`ReYMeN tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'ReYMeN-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `ReYMeN tools`,
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `ReYMeN tools`, the toolset appears."""
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `ReYMeN tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from ReYMeN_cli.tools_config import _get_effective_configurable_toolsets
...
def test_reconfigure_provider_syncs_use_gateway(
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from ReYMeN_cli.tools_config import apply_provider_selection
...
#    reported as added/removed by `ReYMeN tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `ReYMeN tools` printed ``- kanban``
