"""Tests for ReYMeN_cli/goals.py — persistent cross-turn goals."""
...
def ReYMeN_home(tmp_path, monkeypatch):
    """Isolated ReYMeN_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves ReYMeN_HOME.
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from ReYMeN_cli.goals import judge_goal
...
class TestGoalManager:
    def test_no_goal_initial(self, ReYMeN_home):
    ...
    def test_set_then_status(self, ReYMeN_home):
        from ReYMeN_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, ReYMeN_home):
        from ReYMeN_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, ReYMeN_home):
        from ReYMeN_cli.goals import GoalManager
    ...
    def test_clear(self, ReYMeN_home):
        from ReYMeN_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, ReYMeN_home):
    ...
    def test_evaluate_after_turn_done(self, ReYMeN_home):
    ...
    def test_evaluate_after_turn_continue_under_budget(self, ReYMeN_home):
        from ReYMeN_cli import goals
        from ReYMeN_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, ReYMeN_home):
    ...
    def test_evaluate_after_turn_inactive(self, ReYMeN_home):
    ...
    def test_continuation_prompt_shape(self, ReYMeN_home):
        ...
        mgr.set("port goal command to ReYMeN")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to ReYMeN" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from ReYMeN_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, ReYMeN_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from ReYMeN_cli import goals
        from ReYMeN_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, ReYMeN_home):
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, ReYMeN_home):
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from ReYMeN_cli import goals
        from ReYMeN_cli.goals import GoalManager, load_goal
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, ReYMeN_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, ReYMeN_home):
        from unittest.mock import patch
        from ReYMeN_cli import goals
