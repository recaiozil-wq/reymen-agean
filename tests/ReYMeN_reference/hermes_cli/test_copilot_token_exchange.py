...
class TestExchangeCopilotToken:
    ...
    def test_refreshes_expired_cache(self, mock_urlopen):
        from ReYMeN_cli.copilot_auth import (
...
class TestGetCopilotApiToken:
    ...
    @patch(
        "ReYMeN_cli.copilot_auth.exchange_copilot_token", side_effect=ValueError("fail")
    )
...
class TestCallerIntegration:
    ...
    @patch(
        "ReYMeN_cli.copilot_auth.get_copilot_api_token", return_value="exchanged_jwt"
    )
