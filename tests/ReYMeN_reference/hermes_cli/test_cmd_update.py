...
from ReYMeN_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        from ReYMeN_cli import main as hm

        mock_run.return_value = subprocess.CompletedProcess([], 0, stdout="", stderr="")
        monkeypatch.delenv("VIRTUAL_ENV", raising=False)
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/ReYMeN-launcher-venv")
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/ReYMeN-agent silently misses updates.
        """
        from ReYMeN_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/ReYMeN-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ReYMeN_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "ReYMeN_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("ReYMeN_cli.config.check_config_version", return_value=(1, 2)), patch(
            "ReYMeN_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("ReYMeN_cli.main.sys") as mock_sys:
            ...
            from ReYMeN_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ReYMeN_cli.config.get_missing_env_vars", return_value=[]
        ), patch("ReYMeN_cli.config.get_missing_config_fields", return_value=[]), patch(
            "ReYMeN_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "ReYMeN_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ReYMeN_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "ReYMeN_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch("ReYMeN_cli.config.check_config_version", return_value=(1, 24)), patch(
            "ReYMeN_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("ReYMeN_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.ReYMeN"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.ReYMeN/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.ReYMeN/profiles/work"))
        ...
        with (
            patch("ReYMeN_cli.profiles.list_profiles", return_value=all_profiles),
            patch("ReYMeN_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)

        assert (
            active_p.path in synced_paths
        ), f"Active profile 'bit' must be included in skill sync; got: {synced_paths}"
        assert set(synced_paths) == {
            p.path for p in all_profiles
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("ReYMeN_cli.profiles.list_profiles", return_value=[default_p]),
            patch("ReYMeN_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``ReYMeN update --branch <name>`` targets the requested branch.
    ...
    def test_branch_flag_pulls_against_named_branch(
        ...
        assert any(
            "bb/gui" in c and "main" not in c.split() for c in pull_cmds
        ), pull_cmds
...
class TestCmdUpdateCheckBranchFlag:
    """``ReYMeN update --check --branch <name>`` honors the branch override.
    ...
    @patch("ReYMeN_cli.config.detect_install_method", return_value="pip")
    @patch("ReYMeN_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``ReYMeN update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from ReYMeN_cli.main import _update_via_zip
