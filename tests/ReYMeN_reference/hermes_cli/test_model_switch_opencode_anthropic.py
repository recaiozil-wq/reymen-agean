...
``ReYMeN_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``ReYMeN_cli.model_switch.switch_model`` was missing the
...
from ReYMeN_cli.model_switch import switch_model
...
def _run_opencode_switch(
    ...
    with (
        patch("ReYMeN_cli.model_switch.resolve_alias", return_value=None),
