"""Tests for the ReYMeN plugin system (ReYMeN_cli.plugins)."""
...
from ReYMeN_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<ReYMeN_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<ReYMeN_home>/plugins/``; we derive
    ``<ReYMeN_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ReYMeN_HOME>/config.yaml.
        ...
        if ReYMeN_home_str:
            ...
            ReYMeN_home = base.parent
        ReYMeN_home.mkdir(parents=True, exist_ok=True)
        cfg_path = ReYMeN_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.ReYMeN/plugins/ are discovered."""
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".ReYMeN" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under ReYMeN_plugins.<name>."""
        plugins_dir = tmp_path / "ReYMeN_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path / "ReYMeN_test"))

        # Clean up any prior namespace module
        sys.modules.pop("ReYMeN_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "ReYMeN_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="ReYMeN_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `ReYMeN plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("ReYMeN_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "ReYMeN_test" / "plugins"
        # Set ReYMeN_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDebugLogging:
    """ReYMeN_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ReYMeN_PLUGINS_DEBUG", raising=False)
        from ReYMeN_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With ReYMeN_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("ReYMeN_PLUGINS_DEBUG", "1")
        from ReYMeN_cli import plugins as plugins_mod
