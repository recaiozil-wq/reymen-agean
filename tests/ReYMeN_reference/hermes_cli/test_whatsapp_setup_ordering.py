...
Before the fix, ``ReYMeN whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``ReYMeN gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    ReYMeN = home / ".ReYMeN"
    ReYMeN.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN))
...
def _env_value(ReYMeN_home: Path, key: str) -> str | None:
    env_file = ReYMeN_home / ".env"
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from ReYMeN_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("ReYMeN_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``ReYMeN whatsapp`` with an existing paired session and
