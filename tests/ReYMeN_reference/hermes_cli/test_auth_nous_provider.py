...
class TestResolveVerifyFallback:
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ReYMeN_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_ReYMeN_ca_bundle_env_falls_back(self, monkeypatch):
        from ReYMeN_cli.auth import _resolve_verify

        monkeypatch.setenv("ReYMeN_CA_BUNDLE", "/nonexistent/ReYMeN-ca.pem")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
    monkeypatch.setenv("ReYMeN_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="ReYMeN_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    ReYMeN_home = tmp_path / "ReYMeN"
    _setup_nous_auth(ReYMeN_home, access_token="at-123")
    monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    ReYMeN_home = tmp_path / "ReYMeN"
    _setup_nous_auth(ReYMeN_home, access_token="at-fresh")
    monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    ReYMeN_home = tmp_path / "ReYMeN"
    _setup_nous_auth(ReYMeN_home, refresh_token="refresh-old")
    monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `ReYMeN model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `ReYMeN auth add nous`
    ...
    """
    from ReYMeN_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "ReYMeN is not logged into Nous Portal").
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `ReYMeN auth add nous --type oauth --label <name>` silently
...
def test_persist_nous_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    """
    from ReYMeN_cli.auth import persist_nous_credentials
    from agent.credential_pool import load_pool
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", ReYMeN must surface an
    ...
    re-authenticate" string led users to report this as a ReYMeN persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from ReYMeN_cli.auth import _refresh_access_token
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ReYMeN_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("ReYMeN_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ReYMeN_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.ReYMeN/shared/`` across CI runs.
    """
    from ReYMeN_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ReYMeN_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """ReYMeN_SHARED_AUTH_DIR must redirect the path."""
    from ReYMeN_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("ReYMeN_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from ReYMeN_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from ReYMeN_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from ReYMeN_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from ReYMeN_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "ReYMeN-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    # Profile B: fresh ReYMeN_HOME, no auth yet, but the shared store
