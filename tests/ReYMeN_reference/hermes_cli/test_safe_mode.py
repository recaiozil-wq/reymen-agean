"""Tests for `ReYMeN chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("ReYMeN_SAFE_MODE", "ReYMeN_IGNORE_USER_CONFIG", "ReYMeN_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in ReYMeN_cli/main.py.
        class Args:
            safe_mode = True

        args = Args()
        if getattr(args, "safe_mode", False):
            os.environ["ReYMeN_SAFE_MODE"] = "1"
            os.environ["ReYMeN_IGNORE_USER_CONFIG"] = "1"
            os.environ["ReYMeN_IGNORE_RULES"] = "1"

        assert os.environ.get("ReYMeN_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under ReYMeN_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("ReYMeN_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("ReYMeN_SAFE_MODE", raising=False)
        from ReYMeN_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under ReYMeN_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `ReYMeN chat`."""
