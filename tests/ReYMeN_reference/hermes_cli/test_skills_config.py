"""Tests for ReYMeN_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("ReYMeN_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from ReYMeN_cli.skills_config import save_disabled_skills
...
class TestIsSkillDisabled:
    @patch("ReYMeN_cli.config.load_config")
    ...
    @patch("ReYMeN_cli.config.load_config")
    @patch.dict("os.environ", {"ReYMeN_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ReYMeN_SESSION_PLATFORM should be used when ReYMeN_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_ReYMeN_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ReYMeN_PLATFORM should win over ReYMeN_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        monkeypatch.setenv("ReYMeN_PLATFORM", "telegram")
        monkeypatch.setenv("ReYMeN_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from ReYMeN_cli.skills_config import _get_categories
