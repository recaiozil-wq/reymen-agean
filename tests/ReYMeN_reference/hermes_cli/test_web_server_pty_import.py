"""Test the platform-branched PTY bridge import in ReYMeN_cli.web_server.
...
def test_web_server_uses_win_pty_bridge_on_windows():
    ...
    from ReYMeN_cli.win_pty_bridge import WinPtyBridge
    ...
    from ReYMeN_cli.win_pty_bridge import PtyUnavailableError as WinErr
...
def test_web_server_uses_posix_pty_bridge_on_posix():
    ...
    from ReYMeN_cli.pty_bridge import PtyBridge as PosixBridge
    from ReYMeN_cli.pty_bridge import PtyUnavailableError as PosixErr
...
def test_pty_bridge_import_block_is_platform_branched():
    ...
    #       try:
    #           from ReYMeN_cli.win_pty_bridge import WinPtyBridge as PtyBridge, ...
    #       except ImportError:
    #           PtyBridge = None
    #           ...
    #   else:
    #       try:
    #           from ReYMeN_cli.pty_bridge import PtyBridge, PtyUnavailableError
    #       ...
    assert (
        'sys.platform.startswith("win")' in src
        or "sys.platform.startswith('win')" in src
    )
    assert "from ReYMeN_cli.win_pty_bridge import" in src
    assert "from ReYMeN_cli.pty_bridge import" in src
