...
import ReYMeN_cli.model_switch as ms
from ReYMeN_cli.model_switch import DirectAlias
from ReYMeN_cli.runtime_provider import _resolve_named_custom_runtime
...
def test_ensure_direct_aliases_mutates_in_place(monkeypatch):
    ...
    assert (
        id(ms.DIRECT_ALIASES) == initial_id
...
def test_chat_provider_argparse_acceptance(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.main.cmd_chat", mock_cmd_chat)
    monkeypatch.setattr(sys, "argv", ["ReYMeN", "chat", "--provider", "my-custom-key"])
...
def test_resolve_named_custom_runtime_honors_explicit_base_url(monkeypatch):
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.runtime_provider.has_usable_secret", lambda x: x == "test-api-key"
    )
