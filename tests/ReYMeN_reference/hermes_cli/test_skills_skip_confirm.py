...
class TestHandleSkillsSlashInstallFlags:
    ...
    def test_yes_flag_sets_skip_confirm(self):
        from ReYMeN_cli.skills_hub import handle_skills_slash

        with patch("ReYMeN_cli.skills_hub.do_install") as mock_install:
...
class TestHandleSkillsSlashUninstallFlags:
    ...
    def test_yes_flag_sets_skip_confirm(self):
        ...
        with patch("ReYMeN_cli.skills_hub.do_uninstall") as mock_uninstall:
...
class TestDoInstallSkipConfirm:
    ...
    @patch("ReYMeN_cli.skills_hub.input", return_value="n")
    def test_without_skip_confirm_prompts_user(self, mock_input):
        ...
        with patch("ReYMeN_cli.skills_hub._console"), patch(
            "tools.skills_hub.ensure_hub_dirs"
        ), patch("tools.skills_hub.GitHubAuth"), patch(
            "tools.skills_hub.create_source_router"
        ) as mock_router, patch(
            "ReYMeN_cli.skills_hub._resolve_short_name", return_value="test/skill"
        ), patch(
...
class TestDoUninstallSkipConfirm:
    ...
    def test_skip_confirm_bypasses_input(self):
        ...
        from ReYMeN_cli.skills_hub import do_uninstall

        with patch("ReYMeN_cli.skills_hub._console") as mock_console, patch(
            "tools.skills_hub.uninstall_skill", return_value=(True, "Removed")
        ) as mock_uninstall, patch("builtins.input") as mock_input:
    ...
    def test_without_skip_confirm_cancel(self):
        ...
        with patch("ReYMeN_cli.skills_hub._console"), patch(
            "tools.skills_hub.uninstall_skill"
        ) as mock_uninstall, patch("builtins.input", return_value="n"):
            do_uninstall("test-skill", skip_confirm=False)
