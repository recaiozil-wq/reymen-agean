"""Tests for Codex auth — tokens stored in ReYMeN auth store (~/.ReYMeN/auth.json)."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_ReYMeN_auth(ReYMeN_home)
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``ReYMeN auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``ReYMeN auth add openai-codex``.
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(
    ...
    Regression for #39236: ``ReYMeN auth add openai-codex`` for accounts B and C
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``ReYMeN auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to ReYMeN auth store, not ~/.codex/."""
    ReYMeN_home = tmp_path / "ReYMeN"
    codex_home = tmp_path / "codex-cli"
    ReYMeN_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "ReYMeN-at", "refresh_token": "ReYMeN-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches ReYMeN store
    assert not (codex_home / "auth.json").exists()

    # ReYMeN auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "ReYMeN-at"


def test_resolve_returns_ReYMeN_auth_store_source(tmp_path, monkeypatch):
    ReYMeN_home = tmp_path / "ReYMeN"
    _setup_ReYMeN_auth(ReYMeN_home)
    monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run ReYMeN auth".
    """
    from ReYMeN_cli.auth import (
    ...
    assert "ReYMeN auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from ReYMeN_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.auth.resolve_codex_runtime_credentials",
