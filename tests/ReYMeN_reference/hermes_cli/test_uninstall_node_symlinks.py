"""Tests for ReYMeN_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ReYMeN_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the ReYMeN-managed node dir.
"""
...
def _make_ReYMeN_node(ReYMeN_home: Path) -> Path:
    """Create a fake $ReYMeN_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = ReYMeN_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_ReYMeN_node(fake_home):
    ReYMeN_home = fake_home / ".ReYMeN"
    node_bin = _make_ReYMeN_node(ReYMeN_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(ReYMeN_home) == []


def test_removes_dangling_symlink_into_ReYMeN_node(fake_home):
    """A link into the ReYMeN node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the ReYMeN links that exist; ignores the ones that don't."""
    ReYMeN_home = fake_home / ".ReYMeN"
    node_bin = _make_ReYMeN_node(ReYMeN_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are ReYMeN-managed; node is a real user binary.
