...
A credential added via `ReYMeN auth add openrouter` lives in the credential
...
def _seed_openrouter_pool(token: str = "sk-or-FAKEKEY123") -> None:
    """Mimic `ReYMeN auth add openrouter <token>` — a manual pool entry."""
...
def test_no_credentials_still_raises(tmp_path, monkeypatch):
    ...
    from ReYMeN_cli.auth import AuthError, resolve_provider
