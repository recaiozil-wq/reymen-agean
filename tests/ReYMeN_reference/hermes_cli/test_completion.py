"""Tests for ReYMeN_cli/completion.py — shell completion script generation."""
...
from ReYMeN_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_ReYMeN_completion()" in out
        assert "complete -F _ReYMeN_completion ReYMeN" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_ReYMeN_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[ReYMeN]}} == _ReYMeN ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c ReYMeN -f" in out
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        ...
        assert 'profiles_dir="$HOME/.ReYMeN/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'ReYMeN profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _ReYMeN_profiles for name-taking actions
        lines = out.split("\n")
    ...
    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_ReYMeN_profiles()" in out
        assert "$HOME/.ReYMeN/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__ReYMeN_profiles" in out
        assert "$HOME/.ReYMeN/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__ReYMeN_profiles" in out
        count = out.count("(__ReYMeN_profiles)")
