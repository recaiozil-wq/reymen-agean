...
profile switcher can target any profile's ReYMeN_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via ReYMeN_HOME.
"""
import pytest
import yaml
...
def isolated_profiles(tmp_path, monkeypatch, _isolate_ReYMeN_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_ReYMeN_home", lambda: default_home)
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same ReYMeN_HOME the rest
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_ReYMeN_action",
            lambda subcommand, name: calls.append(list(subcommand)) or _FakeProc(),
        )
        monkeypatch.setattr(
            "ReYMeN_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_ReYMeN_home(self, isolated_profiles, monkeypatch):
        import ReYMeN_cli.web_server as web_server

        monkeypatch.setattr(
            "ReYMeN_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["ReYMeN_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "ReYMeN_TUI_GATEWAY_URL" not in env
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("ReYMeN_HOME") != str(isolated_profiles["worker_beta"])
