...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "ReYMeN_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
class TestSharedBoardPaths:
    ...
    def test_default_install_anchors_at_home_dot_ReYMeN(
    ...
    def test_explicit_override_via_ReYMeN_kanban_home(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_ReYMeN_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("ReYMeN_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
...
def test_resolve_ReYMeN_argv_ReYMeN_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("ReYMeN_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
