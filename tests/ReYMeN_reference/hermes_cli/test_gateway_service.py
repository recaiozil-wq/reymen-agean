...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway_cli,
            "_ReYMeN_home_for_target_user",
            lambda home: "/home/alice/.ReYMeN",
        )
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(
                    5, ["launchctl", "kickstart", "-k", "gui/501/ai.ReYMeN.gateway"]
                )
            ),
        )
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            gateway_cli, "get_reymen_home", lambda: root_home / ".ReYMeN"
        )
...
class TestLegacyReYMeNUnitDetection:
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (system_dir / "ReYMeN.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
...
class TestRemoveLegacyReYMeNUnits:
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(c.startswith("systemctl stop ReYMeN.service") for c in cmds_joined)
        assert any(
...
class TestServiceWorkingDirIsStable:
    ...
    def test_user_unit_workingdirectory_is_ReYMeN_home_not_checkout(
        self, tmp_path, monkeypatch
    ):
        home = tmp_path / ".ReYMeN"
        home.mkdir()
