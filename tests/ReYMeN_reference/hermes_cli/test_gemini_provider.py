...
from ReYMeN_cli.model_normalize import normalize_model_for_provider, detect_vendor
...
class TestGeminiAgentInit:
    ...
    def test_gemini_agent_uses_native_client(self, monkeypatch):
        ...
        ) as mock_client, patch("run_agent.OpenAI") as mock_openai, patch(
            "run_agent.ContextCompressor"
        ) as mock_compressor:
            mock_client.return_value = MagicMock()
    ...
    def test_gemini_resolve_provider_client_uses_native_client(self, monkeypatch):
        ...
        with patch(
            "agent.gemini_native_adapter.GeminiNativeClient"
        ) as mock_client, patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client.return_value = MagicMock()
