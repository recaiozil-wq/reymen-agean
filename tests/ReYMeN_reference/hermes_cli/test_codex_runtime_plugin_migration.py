...
from ReYMeN_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_ReYMeN_tools_mcp_entry,
    _format_toml_value,
...
class TestMigrate:
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from ReYMeN_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_expose_ReYMeN_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_ReYMeN_tools=True (production default), an
        [mcp_servers.ReYMeN-tools] entry is written so codex calls back
        into ReYMeN for browser/web/delegate_task/vision/memory tools.
        ...
        assert "ReYMeN-tools" in report.migrated
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(ReYMeN_cfg, codex_home=tmp_path, expose_ReYMeN_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If ReYMeN appends
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside ReYMeN' managed block, re-running
...
class TestStripUnmanagedPluginTables:
    ...
    ``ReYMeN codex-runtime migrate`` would re-emit them inside the managed
...
# ---- Bug C: ReYMeN_HOME tempdir leak into ~/.codex/config.toml ----


class TestReYMeNHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_ReYMeN_tools_mcp_entry()`` read ``ReYMeN_HOME``
    ...
    once codex spawned the ReYMeN-tools MCP subprocess.
    """
    ...
    def test_tempdir_detector_accepts_real_ReYMeN_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.ReYMeN")
        assert not _looks_like_test_tempdir("/home/bob/.ReYMeN")
        assert not _looks_like_test_tempdir("/opt/ReYMeN")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when ReYMeN_HOME points at a pytest
        tempdir, _build_ReYMeN_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "ReYMeN_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/ReYMeN_test",
        )
        entry = _build_ReYMeN_tools_mcp_entry()
        env = entry.get("env", {})
        assert "ReYMeN_HOME" not in env, (
            f"pytest-tempdir ReYMeN_HOME leaked into codex MCP entry: "
            f"{env.get('ReYMeN_HOME')!r}"
        )

    def test_real_ReYMeN_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate ReYMeN_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.ReYMeN"
        monkeypatch.setenv("ReYMeN_HOME", real_path)
        entry = _build_ReYMeN_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("ReYMeN_HOME") == real_path

    def test_unset_ReYMeN_home_omits_env_key(self, monkeypatch):
        """When ReYMeN_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever ReYMeN_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "ReYMeN_HOME" not in env, (
            f"ReYMeN_HOME should not be set when env var is unset, got: "
            f"{env.get('ReYMeN_HOME')!r}"
        )
