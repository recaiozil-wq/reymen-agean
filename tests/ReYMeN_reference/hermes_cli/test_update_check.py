"""Tests for the update check mechanism in ReYMeN_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    from ReYMeN_cli.banner import check_for_updates
    from ReYMeN_cli import __version__

    # Create a fake git repo and fresh cache
    repo_dir = tmp_path / "ReYMeN-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "ReYMeN_cli" / "banner.py"
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == [
            "git",
            "ls-remote",
            "https://github.com/NousResearch/ReYMeN-agent.git",
            "refs/heads/main",
        ]:
            return MagicMock(returncode=0, stdout="upstream-sha\trefs/heads/main\n")
        raise AssertionError(f"unexpected git command: {cmd!r}")

    with patch("ReYMeN_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
    with patch("ReYMeN_cli.banner.subprocess.run") as mock_run:
        with patch("ReYMeN_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when ReYMeN_HOME has no git repo."""
    import ReYMeN_cli.banner as banner
    ...
    # Point ReYMeN_HOME at a temp dir with no ReYMeN-agent/.git
    monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
    with patch("ReYMeN_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    ReYMeN_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `ReYMeN update` correctly refusing to run there. The guard
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from ReYMeN_cli.main import _invalidate_update_cache

    # Build a fake ~/.ReYMeN with default + two named profiles
