...
When ``ReYMeN_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.ReYMeN/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.ReYMeN            (has its own auth.json fixture)
    * Profile     -> tmp_path/.ReYMeN/profiles/coder   (active, ReYMeN_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"ReYMeN is not
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ReYMeN_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.ReYMeN as the "real home" — which is NOT equal
    # to the ReYMeN_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    ReYMeN_home = tmp_path / "classic"
