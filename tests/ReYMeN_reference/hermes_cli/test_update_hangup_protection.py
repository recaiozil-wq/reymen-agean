"""Tests for SIGHUP protection and stdout mirroring in ``ReYMeN update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``ReYMeN_cli/main.py``.  These exist so
that ``ReYMeN update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "ReYMeN update started" in contents
        finally:
            _finalize_update_output(state)
