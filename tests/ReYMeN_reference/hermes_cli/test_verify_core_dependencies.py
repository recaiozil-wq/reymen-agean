...
base deps) during ``ReYMeN update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point ReYMeN_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch(
            "ReYMeN_cli.main._resolve_install_target_python", return_value=py
        ), patch("ReYMeN_cli.main.subprocess.run") as mock_run, patch(
            "ReYMeN_cli.main._run_install_with_heartbeat"
        ) as mock_install:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")

            from ReYMeN_cli.main import _verify_core_dependencies_installed
    ...
    def test_triggers_reinstall_when_dep_missing(
        ...
        with patch(
            ...
            "ReYMeN_cli.main.subprocess.run", side_effect=fake_subprocess_run
        ), patch("ReYMeN_cli.main._run_install_with_heartbeat") as mock_install:
            from ReYMeN_cli.main import _verify_core_dependencies_installed
    ...
    def test_falls_back_to_per_package_install_when_reinstall_did_not_help(
        ...
        with patch(
            ...
            assert any(
                "pathspec==1.1.1" in str(a) for a in second_repair_args
    ...
    def test_skips_deps_excluded_by_environment_markers(
        ...
        with patch(
            ...
            "ReYMeN_cli.main.subprocess.run", side_effect=fake_subprocess_run
        ), patch("ReYMeN_cli.main._run_install_with_heartbeat"), patch(
            "sys.platform", "win32"
        ):
            from ReYMeN_cli.main import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch(
            "ReYMeN_cli.main._resolve_install_target_python"
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``ReYMeN.exe`` on Windows before installing.
        ...
        is left missing and ``ReYMeN`` drops off PATH. Previously this path
        ...
        with patch(
            ...
            "ReYMeN_cli.main.subprocess.run", side_effect=fake_subprocess_run
        ), patch("ReYMeN_cli.main._is_windows", return_value=True), patch(
            "ReYMeN_cli.main._venv_scripts_dir", return_value=fake_scripts
        ), patch("ReYMeN_cli.main._run_install_with_heartbeat"), patch(
            "ReYMeN_cli.main._quarantine_running_ReYMeN_exe", return_value=[]
        ) as mock_quar:
            from ReYMeN_cli.main import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `ReYMeN update`.
        ...
        with patch("ReYMeN_cli.main._is_windows", return_value=True):
            from ReYMeN_cli.main import _resolve_install_target_python
