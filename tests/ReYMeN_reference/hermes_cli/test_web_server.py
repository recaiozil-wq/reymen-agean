"""Tests for ReYMeN_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_ReYMeN_home):
    """Drop the example-dashboard fixture into the per-test ReYMeN_HOME
    ...
    isolated ``ReYMeN_HOME``.

    The user-plugin source is preferred over a transient
    ``ReYMeN_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, ReYMeN-achievements, model providers
    ...
    #      and start serving requests against a torn-down ReYMeN_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known ReYMeN vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-ReYMeN env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints ReYMeN_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("ReYMeN_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_ReYMeN_home):
        """Create a TestClient and isolate the state DB under the test ReYMeN_HOME."""
        try:
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("ReYMeN_state.SessionDB", _FakeDB)
    ...
    def test_sessions_endpoint_reads_requested_profile(self):
        ...
        from ReYMeN_state import SessionDB
        from ReYMeN_cli import profiles as profiles_mod
    ...
    def test_update_ReYMeN_returns_docker_guidance_without_spawning(self, monkeypatch):
        import ReYMeN_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn ReYMeN update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_ReYMeN_action", fail_spawn)
        web_server._ACTION_PROCS.pop("ReYMeN-update", None)
        web_server._ACTION_RESULTS.pop("ReYMeN-update", None)

        resp = self.client.post("/api/ReYMeN/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "ReYMeN-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/ReYMeN-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/ReYMeN-update/status")
    ...
    def test_update_ReYMeN_spawns_on_non_docker_install(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "_spawn_ReYMeN_action", fake_spawn)
        web_server._ACTION_PROCS.pop("ReYMeN-update", None)
        web_server._ACTION_RESULTS.pop("ReYMeN-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "ReYMeN-update"}
        assert calls == [(["update"], "ReYMeN-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("ReYMeN-update", None)
        web_server._ACTION_RESULTS.pop("ReYMeN-update", None)
        web_server._ACTION_PROCS["ReYMeN-update"] = proc

        resp = self.client.get("/api/actions/ReYMeN-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "ReYMeN-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["ReYMeN-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from ReYMeN_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from ReYMeN_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a ReYMeN provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `ReYMeN import` doesn't auto-abort at the overwrite prompt."""
        import ReYMeN_cli.web_server as ws
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from ReYMeN_cli.config import save_env_value
        from ReYMeN_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://ReYMeN-agent.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "ReYMeN Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("ReYMeNDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "ReYMeN_pair123_bot",
                "deep_link": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair123_bot",
                "qr_payload": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "ReYMeN_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "ReYMeN_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_ReYMeN_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "ReYMeN_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "ReYMeN_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_ReYMeN_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``ReYMeN gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "ReYMeN_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "ReYMeN_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "ReYMeN_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "ReYMeN_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/ReYMeN_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "ReYMeN-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from ReYMeN_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from ReYMeN_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``ReYMeN model`` custom flow. Regression for the desktop loop where a
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_ReYMeN_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("ReYMeN://")
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        named = ReYMeN_home / "profiles" / "multi-agent"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        from ReYMeN_cli.config import load_config
        from ReYMeN_cli.skills_config import get_disabled_skills
        import ReYMeN_cli.profiles as profiles_mod
        import ReYMeN_cli.web_server as web_server
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import ReYMeN_cli.skills_config as skills_config
        import ReYMeN_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import ReYMeN_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from ReYMeN_cli.web_server import _normalize_config_for_web
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.ReYMeN/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``ReYMeN_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in ReYMeN_cli/web_server.py."
        )
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_ReYMeN_home, _install_example_plugin):
        ...
        installs it into the per-test ``ReYMeN_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into ReYMeN_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, ReYMeN-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (ReYMeN-achievements is loaded by default).
        resp = self.client.get("/api/plugins/ReYMeN-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``ReYMeN --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["ReYMeN_TUI_INLINE"] == "1"
        assert env["ReYMeN_TUI_DISABLE_MOUSE"] == "1"

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_ReYMeN_home
    ):
        import ReYMeN_cli.main as main_mod

        config_path = Path(os.environ["ReYMeN_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/ReYMeN-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/ReYMeN-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"ReYMeN-ws-ok" in buf:
                    break
            assert b"ReYMeN-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from ReYMeN_cli.pty_bridge import PtyUnavailableError
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        ReYMeN_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from ReYMeN_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("ReYMeN_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_ReYMeN_home, _install_example_plugin):
        ...
        the per-test ``ReYMeN_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_ReYMeN_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("ReYMeN_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under ReYMeN_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_ReYMeN_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("ReYMeN_DESKTOP", raising=False)
