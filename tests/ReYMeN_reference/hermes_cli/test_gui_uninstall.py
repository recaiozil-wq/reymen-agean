"""Tests for ReYMeN_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import ReYMeN_cli.gui_uninstall as gu


def _make_agent(ReYMeN_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = ReYMeN_home / "ReYMeN-agent"
    (agent_root / "ReYMeN_cli").mkdir(parents=True)
    (agent_root / "ReYMeN_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(ReYMeN_home: Path) -> None:
    """Create the source-built GUI artifacts a `ReYMeN desktop` run produces."""
    desktop = ReYMeN_home / "ReYMeN-agent" / "apps" / "desktop"
    ...
    (ReYMeN_home / "ReYMeN-agent" / "node_modules").mkdir(parents=True)
    (ReYMeN_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(ReYMeN_home: Path) -> None:
    (ReYMeN_home / "config.yaml").write_text("x: 1\n")
    (ReYMeN_home / ".env").write_text("KEY=secret\n")
    (ReYMeN_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    ReYMeN_home = tmp_path / ".ReYMeN"
    ReYMeN_home.mkdir()
    assert gu.agent_is_installed(ReYMeN_home) is False
    _make_agent(ReYMeN_home)
    assert gu.agent_is_installed(ReYMeN_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    ReYMeN_home = tmp_path / ".ReYMeN"
    (ReYMeN_home / "ReYMeN-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(ReYMeN_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    ReYMeN_home = tmp_path / ".ReYMeN"
    _make_gui_build(ReYMeN_home)
    artifacts = gu.source_built_gui_artifacts(ReYMeN_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(ReYMeN_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(ReYMeN_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    ReYMeN_home = tmp_path / ".ReYMeN"
    agent_root = _make_agent(ReYMeN_home)
    _make_gui_build(ReYMeN_home)
    _make_user_data(ReYMeN_home)
    ...
    removed = gu.uninstall_gui(ReYMeN_home)
    ...
    assert not (ReYMeN_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "ReYMeN_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (ReYMeN_home / "config.yaml").exists()
    assert (ReYMeN_home / ".env").exists()
    assert (ReYMeN_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    ReYMeN_home = tmp_path / ".ReYMeN"
    _make_agent(ReYMeN_home)
    userdata = tmp_path / "ReYMeN-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(ReYMeN_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(ReYMeN_home)
    ...
    assert summary["ReYMeN_home"] == str(ReYMeN_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "ReYMeN"."""
    ...
    assert gu.desktop_userdata_dir() == home / ".config" / "ReYMeN"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    ReYMeN_HOME with all the destructive externals stubbed out.
    """
    import ReYMeN_cli.uninstall as uninstall
    ...
    assert not (desktop / "release").exists()
    assert (ReYMeN_home / "config.yaml").exists()
    assert ReYMeN_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole ReYMeN_HOME non-interactively."""
    import ReYMeN_cli.uninstall as uninstall

    ReYMeN_home = tmp_path / ".ReYMeN"
    (ReYMeN_home / "ReYMeN-agent" / "ReYMeN_cli").mkdir(parents=True)
    (ReYMeN_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not ReYMeN_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m ReYMeN_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    assert not (desktop / "release").exists()
    assert not (ReYMeN_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "ReYMeN_cli").exists()
    assert (ReYMeN_home / "config.yaml").exists()
