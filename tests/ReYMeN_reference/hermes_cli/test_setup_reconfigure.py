...
- Bare `ReYMeN setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `ReYMeN setup --quick` runs the narrower "fill in missing items" flow.
- `ReYMeN setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("ReYMeN_cli.setup.ensure_ReYMeN_home", {}),
        ("ReYMeN_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("ReYMeN_cli.config.is_managed", {"return_value": False}),
        ("ReYMeN_cli.setup.load_config", {"return_value": {}}),
        ("ReYMeN_cli.setup.save_config", {}),
        ("ReYMeN_cli.setup.get_env_value", {"return_value": None}),
        ("ReYMeN_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("ReYMeN_cli.setup._print_setup_summary", {}),
        ("ReYMeN_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("ReYMeN_cli.setup.load_config", {"return_value": {}}),
        ("ReYMeN_cli.setup.save_config", {}),
        ("ReYMeN_cli.auth.get_active_provider", {"return_value": None}),
        ("ReYMeN_cli.setup.get_env_value", {"return_value": None}),
        ("ReYMeN_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `ReYMeN setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="ReYMeN_cli.setup.prompt_choice",
                quick="ReYMeN_cli.setup._run_quick_setup",
                model="ReYMeN_cli.setup.setup_model_provider",
                terminal="ReYMeN_cli.setup.setup_terminal_backend",
                agent="ReYMeN_cli.setup.setup_agent_settings",
                gateway="ReYMeN_cli.setup.setup_gateway",
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`ReYMeN setup --reconfigure` behaves the same as bare `ReYMeN setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("ReYMeN_cli.setup.prompt_choice", {"return_value": 0}),
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from ReYMeN_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "ReYMeN_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["ReYMeN", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["ReYMeN", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["ReYMeN", "setup"])
        try:
            main()
        except SystemExit:
            pass
