...
``$ReYMeN_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/ReYMeN_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        _enable(ReYMeN_home, "my-tts-plugin")

        mgr = PluginManager()
        mgr.discover_and_load()

        assert (
            mgr._plugins["my-tts-plugin"].enabled is True
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(ReYMeN_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(ReYMeN_home, "shadow-tts-plugin")
