"""Tests for --ignore-user-config and --ignore-rules flags on `ReYMeN chat`.
...
files. In ReYMeN the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.ReYMeN/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → ReYMeNCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("ReYMeN_IGNORE_USER_CONFIG", "ReYMeN_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``ReYMeN_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<ReYMeN_home>/config.yaml`` is
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("ReYMeN_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``ReYMeNCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting ReYMeN_IGNORE_RULES=1 flips ReYMeNCLI.ignore_rules True."""
        monkeypatch.setenv("ReYMeN_IGNORE_RULES", "1")
        ...
        # Replicate the exact logic from cli.py ReYMeNCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``ReYMeN_cli/main.py`` must set
    ...
    def _simulate_cmd_chat_env_setup(self, args):
        ...
        if getattr(args, "ignore_user_config", False):
            os.environ["ReYMeN_IGNORE_USER_CONFIG"] = "1"
        if getattr(args, "ignore_rules", False):
            os.environ["ReYMeN_IGNORE_RULES"] = "1"

    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("ReYMeN_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("ReYMeN_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("ReYMeN_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("ReYMeN_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("ReYMeN_IGNORE_USER_CONFIG") == "1"
        assert "ReYMeN_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "ReYMeN_IGNORE_USER_CONFIG" not in os.environ
        assert "ReYMeN_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real ReYMeN parser accepts both flags."""
        from ReYMeN_cli._parser import build_top_level_parser
