...
`ReYMeN_REDACT_SECRETS` at module-import time. `ReYMeN_cli/main.py` at
...
in .env), the toggle was silently ignored in both `ReYMeN chat` and
`ReYMeN gateway run`.
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports ReYMeN_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("ReYMeN_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import ReYMeN_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('ReYMeN_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["ReYMeN_HOME"] = str(ReYMeN_home)
    env.pop("ReYMeN_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env ReYMeN_REDACT_SECRETS takes precedence over config.yaml."""
    ReYMeN_home = tmp_path / ".ReYMeN"
    ReYMeN_home.mkdir()
    (ReYMeN_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (ReYMeN_home / ".env").write_text("ReYMeN_REDACT_SECRETS=true\n")
