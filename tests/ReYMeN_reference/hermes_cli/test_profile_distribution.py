"""Tests for ReYMeN_cli.profile_distribution — git-based profile installs.
...
from ReYMeN_cli.profile_distribution import (
    DEFAULT_DIST_OWNED,
    DistributionError,
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/ReYMeN_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            ...
            with pytest.raises(DistributionError, match="requires ReYMeN"):
                check_ReYMeN_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "ReYMeN distribution" in out
