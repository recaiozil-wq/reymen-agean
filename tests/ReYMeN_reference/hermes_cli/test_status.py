...
from ReYMeN_cli.status import show_status
...
def test_show_status_termux_gateway_section_skips_systemctl(
    ...
    assert "Start with:   ReYMeN gateway" in output
...
def test_show_status_reports_nous_auth_error(monkeypatch, capsys, tmp_path):
    ...
    output = capsys.readouterr().out
    assert "Nous Portal   ✗ not logged in (run: ReYMeN portal)" in output
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in ReYMeN status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            auth_mod,
            "get_xai_oauth_auth_status",
            lambda: {"logged_in": True, "auth_store": "/home/u/.ReYMeN/auth.json"},
            raising=False,
        )
        ...
        assert "Auth file:  /home/u/.ReYMeN/auth.json" in out
    ...
    def test_not_logged_in_shows_login_command(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: ReYMeN auth add xai-oauth)" in out
