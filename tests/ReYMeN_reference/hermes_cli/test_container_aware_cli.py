...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ReYMeN_HOME with .container-mode file."""
    ReYMeN_home = tmp_path / ".ReYMeN"
    ReYMeN_home.mkdir()
    monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
    monkeypatch.delenv("ReYMeN_DEV", raising=False)

    container_mode = ReYMeN_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=ReYMeN-agent\n"
        "exec_user=ReYMeN\n"
        "ReYMeN_bin=/data/current-package/bin/ReYMeN\n"
    )
    return ReYMeN_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    assert info["container_name"] == "ReYMeN-agent"
    assert info["exec_user"] == "ReYMeN"
    assert info["ReYMeN_bin"] == "/data/current-package/bin/ReYMeN"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("ReYMeN_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_ReYMeN_dev(container_env, monkeypatch):
    """Returns None when ReYMeN_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ReYMeN_DEV", "1")
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=ReYMeN-custom\n"
        "exec_user=myuser\n"
        "ReYMeN_bin=/opt/ReYMeN/bin/ReYMeN\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "ReYMeN-custom"
    assert info["exec_user"] == "myuser"
    assert info["ReYMeN_bin"] == "/opt/ReYMeN/bin/ReYMeN"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "ReYMeN"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "ReYMeN-agent" in cmd
    assert "/data/current-package/bin/ReYMeN" in cmd
    assert "chat" in cmd
