"""Tests for ReYMeN_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
from ReYMeN_cli.telegram_managed_bot import (
    DEFAULT_MANAGER_BOT,
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("ReYMeN_")
        assert name.endswith("_bot")
        assert len(name) == len("ReYMeN_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("ReYMeN_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestGenerateDeepLink:
    ...
    def test_defaults(self):
        ...
        assert "ReYMeN_" in link

    def test_name_url_encoded(self):
        link = generate_deep_link(
            manager_bot="Bot",
            suggested_username="test_bot",
            suggested_name="ReYMeN & Friends",
        )
        assert "ReYMeN+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "ReYMeN_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_abcdefghijklmnop_bot?name=ReYMeN+Agent",
            "qr_payload": "https://t.me/newbot/ReYMeNSetupBot/ReYMeN_abcdefghijklmnop_bot?name=ReYMeN+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "ReYMeN_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="ReYMeN Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="ReYMeN_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/ReYMeNSetupBot/ReYMeN_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/ReYMeNSetupBot/ReYMeN_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "ReYMeN_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "ReYMeN_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("ReYMeN_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("ReYMeN_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("ReYMeN_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("ReYMeN_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("ReYMeN_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "ReYMeN_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("ReYMeN_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("ReYMeN_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from ReYMeN_cli.setup import _setup_telegram_auto
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "ReYMeN_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from ReYMeN_cli.setup import _profile_name_from_ReYMeN_home

        assert (
            _profile_name_from_ReYMeN_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\ReYMeN\profiles\oracle")
            )
            == "oracle"
        )
