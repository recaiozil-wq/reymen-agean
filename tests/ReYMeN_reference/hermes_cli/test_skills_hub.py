...
from ReYMeN_cli.skills_hub import (
    do_check,
    do_install,
    do_list,
    do_update,
...
def _capture_update(monkeypatch, results) -> tuple[str, list[tuple[str, str, bool]]]:
    import tools.skills_hub as hub
    import ReYMeN_cli.skills_hub as cli_hub
...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`ReYMeN skills list` reads the active profile's config via
    ReYMeN_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` — otherwise per-platform overrides
    would silently leak in from ReYMeN_PLATFORM env."""
    from agent import skill_utils
...
def test_handle_skills_slash_search_accepts_chatconsole_without_status_errors():
    ...
    with patch("tools.skills_hub.unified_search", return_value=results), patch(
        "tools.skills_hub.create_source_router", return_value={}
...
def test_existing_categories_skips_top_level_skills(monkeypatch, tmp_path, hub_env):
    import tools.skills_hub as hub
    from ReYMeN_cli.skills_hub import _existing_categories
...
def test_browse_skills_dedup_uses_identifier_not_name(monkeypatch):
    ...
    # so the patch must target the source module, not ReYMeN_cli.skills_hub.
...
def test_do_browse_reports_live_per_source_progress():
    ...
    from ReYMeN_cli.skills_hub import do_browse
    from tools.skills_hub import SkillMeta
    ...
    with patch("tools.skills_hub.create_source_router", return_value=[]), patch(
        "tools.skills_hub.GitHubAuth"
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `ReYMeN skills search`
...
def test_do_search_identifier_column_does_not_truncate_long_slug():
    ...
    """
    from ReYMeN_cli.skills_hub import do_search
    ...
    with patch("tools.skills_hub.unified_search", return_value=[_LONG_RESULT]), patch(
        "tools.skills_hub.create_source_router", return_value={}
    ), patch("tools.skills_hub.GitHubAuth"):
