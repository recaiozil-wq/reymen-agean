"""Tests for IRC gateway configuration via `ReYMeN setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh ReYMeN install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new ReYMeN install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#ReYMeN")
            monkeypatch.setenv("IRC_NICKNAME", "ReYMeN-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `ReYMeN setup gateway` experience with IRC present."""
