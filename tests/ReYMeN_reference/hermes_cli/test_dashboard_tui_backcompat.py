"""Regression test: `ReYMeN dashboard --tui` must not hard-crash.

Older ReYMeN desktop app shells (<= 0.15.x) spawn the backend as::

    ReYMeN dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "ReYMeN couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real ReYMeN_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "ReYMeN_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
...
def test_dashboard_without_tui_still_parses():
    ...
    result = _run_cli(
        ["dashboard", "--no-open", "--host", "127.0.0.1", "--port", "39996", "--status"]
    )
    combined = (result.stdout or "") + (result.stderr or "")
    assert "unrecognized arguments" not in combined, combined
    assert result.returncode != 2, combined
