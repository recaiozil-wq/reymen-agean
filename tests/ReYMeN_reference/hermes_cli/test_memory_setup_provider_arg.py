"""Tests for `ReYMeN memory setup [provider]` routing.
...
``ReYMeN memory setup honcho``) without the interactive picker — which matters
because the per-provider ``ReYMeN <provider>`` subcommand is only registered
...
from ReYMeN_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    def test_setup_with_provider_arg_skips_picker(self):
        ...
        with patch.object(memory_setup, "cmd_setup_provider") as direct, patch.object(
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "ReYMeN memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect):
        ...
        with patch("plugins.memory.find_provider_dir", return_value=tmp_path), patch(
