...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    .env `ReYMeN_MAX_ITERATIONS` fallback was removed because it was
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    from an old `ReYMeN setup` run shadowed `agent.max_turns: 500` in
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.setup.remove_env_value",
    ...
    assert "ReYMeN_MAX_ITERATIONS" in removed_keys
