"""Tests for the `ReYMeN memory reset` CLI command.
...
- Profile-scoped reset (uses ReYMeN_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ReYMeN_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nReYMeN repo is at ~/.ReYMeN/ReYMeN-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        (ReYMeN_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ReYMeN_HOME."""
