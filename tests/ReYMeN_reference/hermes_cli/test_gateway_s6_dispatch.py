"""Tests for the Phase 4 s6 dispatch helper in ReYMeN_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`ReYMeN gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`ReYMeN -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from ReYMeN_cli import gateway as gw
    from ReYMeN_cli.service_manager import GatewayNotRegisteredError
    ...
    assert "ReYMeN profile create typo" in out
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("ReYMeN_cli.gateway._profile_suffix", lambda: "")
    ...
    monkeypatch.setattr("ReYMeN_cli.gateway.os.execvp", fake_execvp)
    # If the fallback ran, the normal sleep path was wrongly skipped.
    monkeypatch.setattr(
        "ReYMeN_cli.gateway._block_until_terminated",
    ...
    assert "ReYMeN_GATEWAY_NO_SUPERVISE" in err
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("ReYMeN_cli.gateway.os.execvp", missing_sleep)
...
def test_block_until_terminated_installs_sigterm_handler_and_blocks(
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.gateway.signal.signal",
    ...
    monkeypatch.setattr("ReYMeN_cli.gateway.signal.pause", fake_pause)
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `ReYMeN gateway run --replace`, the
    ReYMeN_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
...
def test_redirect_respects_no_supervise_env(
    ...
    monkeypatch.delenv("ReYMeN_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("ReYMeN_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of ReYMeN_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `ReYMeN_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        execvp_calls.clear()
        monkeypatch.setenv("ReYMeN_GATEWAY_NO_SUPERVISE", falsy)
