"""Tests for ReYMeN_cli.service_manager — the abstract ServiceManager
...
from ReYMeN_cli.service_manager import (
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `ReYMeN gateway restart` spawned a
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from ReYMeN_cli.service_manager import _s6_running
...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the ReYMeN user. The new
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `ReYMeN_cli` package succeeds — gateway_windows is
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
# unprivileged ReYMeN user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from ReYMeN_cli.service_manager import _seed_supervise_skeleton
...
def test_seed_supervise_skeleton_handles_log_subservice(tmp_path) -> None:
    ...
    slot's supervise/ was ReYMeN-owned.
    """
    import stat
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "ReYMeN -p coder gateway run" in run_text
    assert "s6-setuidgid ReYMeN" in run_text
    ...
    # in ReYMeN_cli/gateway.py::_gateway_command_inner.
    assert "export ReYMeN_S6_SUPERVISED_CHILD=1" in run_text

    log_run = svc_dir / "log" / "run"
    assert log_run.is_file()
    log_text = log_run.read_text()
    # CRITICAL: ReYMeN_HOME must be a runtime env-var expansion, NOT
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid ReYMeN ReYMeN -p coder gateway run" in run_text
...
def test_s6_lifecycle_persists_named_profile_desired_state(
    ...
    profile_dir = ReYMeN_home / "profiles" / "coder"
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home / "profiles" / "coder"))

    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.start("gateway-default")
    state = json.loads((ReYMeN_home / "gateway_state.json").read_text())
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "ReYMeN profile create typo" in msg
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from ReYMeN_cli.service_manager import S6CommandError
...
#
# `ReYMeN gateway stop` inside a container dispatches through
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    dropped ReYMeN user and s6-log crash-loops on `mkdir: Permission denied`.
    """
    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.register_profile_gateway("coder")
    ...
    parent_chown = 'chown ReYMeN:ReYMeN "$ReYMeN_HOME/logs/gateways"'
    ...
    assert 'chown -R ReYMeN:ReYMeN "$ReYMeN_HOME/logs/gateways"' not in log_text
    ...
    leaf_idx = log_text.index('chown -R ReYMeN:ReYMeN "$log_dir"')
