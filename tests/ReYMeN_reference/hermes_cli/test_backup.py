"""Tests for ReYMeN backup and import commands."""
...
def _make_ReYMeN_tree(root: Path) -> None:
    ...
    (root / "ReYMeN_state.db").write_bytes(b"fake-state")
    ...
    (root / "ReYMeN-agent").mkdir(exist_ok=True)
    (root / "ReYMeN-agent" / "run_agent.py").write_text("# big file\n")
    (root / "ReYMeN-agent" / ".git").mkdir()
    (root / "ReYMeN-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestBackup:
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = ReYMeN_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, ReYMeN_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_ReYMeN_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include ReYMeN-agent/ directory."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        _make_ReYMeN_tree(ReYMeN_home)
    ...
    def test_includes_nested_ReYMeN_agent_in_skills(self, tmp_path, monkeypatch):
        ...
        # Add a nested ReYMeN-agent directory inside skills (like the real layout)
        nested = ReYMeN_home / "skills" / "autonomous-ai-agents" / "ReYMeN-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# ReYMeN Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root ReYMeN-agent must be excluded
            root_agent = [n for n in names if n.startswith("ReYMeN-agent/")]
            assert root_agent == [], f"root ReYMeN-agent leaked: {root_agent}"
            # Nested skill ReYMeN-agent must be included
            assert "skills/autonomous-ai-agents/ReYMeN-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/ReYMeN-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/ReYMeN-backup-*.zip."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        (ReYMeN_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("ReYMeN-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ReYMeN_HOME."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        _make_ReYMeN_tree(ReYMeN_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(ReYMeN_home / "skills" / "outside-link.txt", outside)
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into ReYMeN home."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_strips_ReYMeN_prefix(self, tmp_path, monkeypatch):
        """Import strips .ReYMeN/ prefix if all entries share it."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (ReYMeN_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_ReYMeN_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a ReYMeN backup."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside ReYMeN home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (ReYMeN_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (ReYMeN_home / "config.yaml").read_text() == "existing: true\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".ReYMeN"
        src_home.mkdir(parents=True)
        _make_ReYMeN_tree(src_home)

        monkeypatch.setenv("ReYMeN_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from ReYMeN_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".ReYMeN"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ReYMeN_HOME", str(dst_home))
        ...
        # ReYMeN-agent should NOT be present
        assert not (dst_home / "ReYMeN-agent").exists()
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without ReYMeN markers fails validation."""
        import io
        from ReYMeN_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_ReYMeN(self):
        """Detects .ReYMeN/ prefix wrapping all entries."""
        import io
        from ReYMeN_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".ReYMeN/config.yaml", "test")
            zf.writestr(".ReYMeN/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".ReYMeN/"
...
class TestBackupEdgeCases:
    def test_nonexistent_ReYMeN_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "nonexistent" / ".ReYMeN"
    ...
    def test_empty_ReYMeN_home(self, tmp_path, monkeypatch):
        """Backup handles empty ReYMeN home (no files to back up)."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        # Only excluded dirs, no actual files
        (ReYMeN_home / "__pycache__").mkdir()
        (ReYMeN_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = ReYMeN_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = ReYMeN_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_ReYMeN(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside ReYMeN root."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        (ReYMeN_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside ReYMeN home
        out_zip = ReYMeN_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        (ReYMeN_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        (ReYMeN_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (ReYMeN_home / "config.yaml").exists()
        assert (ReYMeN_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (ReYMeN_home / "profiles" / "coder" / "config.yaml").exists()
        assert (ReYMeN_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "ReYMeN -p coder" in coder_wrapper
...
class TestQuickSnapshot:
    @pytest.fixture
    def ReYMeN_home(self, tmp_path):
        """Create a fake ReYMeN_HOME with critical state files."""
    ...
    def test_list_snapshots(self, ReYMeN_home):
        ...
        snaps = list_quick_snapshots(ReYMeN_home=ReYMeN_home)
        assert len(snaps) == 2
    ...
    def test_restore_config(self, ReYMeN_home):
        ...
        assert "anthropic" in (ReYMeN_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, ReYMeN_home=ReYMeN_home)
        assert result is True
        assert "openrouter" in (ReYMeN_home / "config.yaml").read_text()
    ...
    def test_restore_state_db(self, ReYMeN_home):
        ...
        restore_quick_snapshot(snap_id, ReYMeN_home=ReYMeN_home)
    ...
    def test_manual_prune(self, ReYMeN_home):
        ...
        assert deleted == 7
        assert len(list_quick_snapshots(ReYMeN_home=ReYMeN_home)) == 3

    def test_snapshot_includes_pairing_directories(self, ReYMeN_home):
        ...
        (ReYMeN_home / "platforms" / "pairing").mkdir(parents=True)
        (ReYMeN_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (ReYMeN_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (ReYMeN_home / "pairing").mkdir()
        (ReYMeN_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (ReYMeN_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, ReYMeN_home):
        ...
        assert restore_quick_snapshot(snap_id, ReYMeN_home=ReYMeN_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, ReYMeN_home):
...
class TestPreUpdateBackup:
    ...
    @pytest.fixture
    def ReYMeN_home(self, tmp_path):
        root = tmp_path / ".ReYMeN"
        root.mkdir()
        _make_ReYMeN_tree(root)
        return root
    ...
    def test_creates_backup_under_backups_dir(self, ReYMeN_home):
        ...
        assert out.exists()
        assert out.parent == ReYMeN_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, ReYMeN_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # ReYMeN-agent repo excluded
        assert not any(n.startswith("ReYMeN-agent/") for n in names)
    ...
    def test_does_not_recurse_into_prior_backups(self, ReYMeN_home):
        ...
        out2 = create_pre_update_backup(ReYMeN_home=ReYMeN_home)
    ...
    def test_rotation_keeps_only_n(self, ReYMeN_home):
    ...
    def test_rotation_preserves_manual_files(self, ReYMeN_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from ReYMeN_cli.backup import create_pre_update_backup

        (ReYMeN_home / "backups").mkdir(exist_ok=True)
        manual = ReYMeN_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(ReYMeN_home=ReYMeN_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, ReYMeN_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, ReYMeN_home):
        ...
        first = create_pre_update_backup(ReYMeN_home=ReYMeN_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(ReYMeN_home=ReYMeN_home, keep=5)
        _t.sleep(1.05)
    ...
    def test_skips_symlinked_files(self, ReYMeN_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ReYMeN_HOME."""
        from ReYMeN_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def ReYMeN_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".ReYMeN"
        root.mkdir()
        _make_ReYMeN_tree(root)
        # Point ReYMeN_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ReYMeN_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for ReYMeN_cli.config + ReYMeN_constants so they pick up ReYMeN_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("ReYMeN_cli.config") or mod == "ReYMeN_constants":
    ...
    def test_backup_flag_creates_backup(self, ReYMeN_home, capsys):
        ...
        assert "ReYMeN import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((ReYMeN_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, ReYMeN_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (ReYMeN_home / "backups").exists() or not list(
            (ReYMeN_home / "backups").glob("pre-update-*.zip")
        )
    ...
    def test_config_enabled_creates_backup(self, ReYMeN_home, capsys):
    ...
    def test_config_disabled_is_silent(self, ReYMeN_home, capsys):
...
class TestPreMigrationBackup:
    ...
    ``ReYMeN claw migrate`` runs before mutating ~/.ReYMeN/."""
    ...
    def test_creates_backup_under_backups_dir(self, ReYMeN_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `ReYMeN
        # import` and the update-backup listing both pick them up.
        assert out.parent == ReYMeN_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, ReYMeN_home):
    ...
    def test_restorable_with_ReYMeN_import(self, ReYMeN_home, tmp_path):
    ...
    def test_rotation_keeps_only_n(self, ReYMeN_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(ReYMeN_home=ReYMeN_home, keep=3)
            if out is not None:
                created.append(out)
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_restores_when_emptied_after_migration(self, tmp_path):
        ...
        snap_id = self._make_snapshot(ReYMeN_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, ReYMeN_home=ReYMeN_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = ReYMeN_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, ReYMeN_home=ReYMeN_home) is None
        assert restore_cron_jobs_if_emptied("", ReYMeN_home=ReYMeN_home) is None
