"""Tests for the decomposer module + `ReYMeN kanban decompose` CLI surface.
...
from ReYMeN_cli import kanban_decompose as decomp
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("ReYMeN_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("ReYMeN_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch(
...
def test_decompose_fanout_false_assigns_default_when_unassigned(kanban_home):
    ...
    try:
        with _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "ReYMeN_cli.kanban_decompose._load_config",
...
def test_decompose_unknown_assignee_falls_back_to_default(kanban_home):
    ...
    try:
        with patch.dict(
            ...
            {},
            clear=False,
        ), _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "ReYMeN_cli.kanban_decompose._load_config",
            return_value={
                "kanban": {
