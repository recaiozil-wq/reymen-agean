"""Tests for ReYMeN_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate ReYMeN_HOME + reset any module-level catalog cache per test."""
...
class TestFallbackChain:
    ...
    PRIMARY = "https://ReYMeN-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/NousResearch/ReYMeN-agent"
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in ReYMeN_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from ReYMeN_cli import model_catalog
        from ReYMeN_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``ReYMeN model`` CLI — not the live
        ...
        # ``ReYMeN_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` ReYMeN_HOME directly instead.
        import importlib
        from ReYMeN_cli import model_catalog
        from ReYMeN_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            ...
            active_home = Path(os.environ["ReYMeN_HOME"])
            ...
            with patch.object(
                ...
                "ReYMeN_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
