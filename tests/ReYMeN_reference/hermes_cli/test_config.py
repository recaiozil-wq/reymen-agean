"""Tests for ReYMeN_cli configuration management."""
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``ReYMeN logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"ReYMeN_HOME": str(tmp_path)}):
            ...
            # stands out at ReYMeN startup before logging is configured)
            captured = capsys.readouterr()
            assert "ReYMeN config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"ReYMeN_HOME": str(tmp_path)}):
            ...
            assert "ReYMeN config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"ReYMeN_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ReYMeN_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ReYMeN_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ReYMeN_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `ReYMeN setup`) lets a
        ...
        via config.yaml; ReYMeN_MAX_ITERATIONS remains a read-only backward-compat
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``ReYMeN_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next ReYMeN
    process on next startup via the dotenv → ``os.environ`` chain in
    ``ReYMeN_cli/env_loader.py``.
    ...
    def _ReYMeN_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "ReYMeN_GEMINI_CLIENT_ID",
            "ReYMeN_LANGFUSE_PUBLIC_KEY",
            "ReYMeN_SPOTIFY_CLIENT_ID",
            "ReYMeN_QWEN_BASE_URL",
            "ReYMeN_MAX_ITERATIONS",
        ],
    )
    def test_ReYMeN_integration_keys_still_writable(self, allowed_key):
        """``ReYMeN_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``ReYMeN_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``ReYMeN_*``) keep working. The denylist
