...
import ReYMeN_cli.doctor as doctor
import ReYMeN_cli.gateway as gateway_cli
from ReYMeN_cli import doctor as doctor_mod
from ReYMeN_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `ReYMeN doctor` crashed on Windows
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("ReYMeN_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ReYMeN_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_ReYMeN_home(self, tmp_path, provider=""):
        """Create a minimal ReYMeN_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_ReYMeN_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "ReYMeN_HOME", home)
        ...
        try:
            from ReYMeN_cli import auth as _auth_mod
...
def test_run_doctor_accepts_ReYMeN_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from ReYMeN_cli.doctor import run_doctor
        import io, contextlib
    ...
    def test_gh_authenticated_without_env_token_shows_ok(self, monkeypatch, tmp_path):
        ...
        assert "gh auth" in str(call_log) or any(
            c[0] == "gh" for c in call_log
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from ReYMeN_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("ReYMeN_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "ReYMeN_cli.doctor", raising=False)
        from ReYMeN_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorCodexCliHintPlacement:
    ...
    def _run(
        ...
        monkeypatch.setattr(
            doctor_mod.shutil,
            "which",
            lambda cmd: ("/usr/local/bin/codex" if codex_cli_present else None)
            if cmd == "codex"
            else real_which(cmd),
        )
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ReYMeN_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir(parents=True)
        (ReYMeN_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"ReYMeN_MAX_ITERATIONS={ghost}\n")
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ReYMeN_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("ReYMeN_MAX_ITERATIONS", raising=False)
        ...
        return buf.getvalue(), ReYMeN_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, ReYMeN_home = self._run_config_section(
            monkeypatch,
            tmp_path,
            fix=False,
            ghost=90,
        ...
        assert "ReYMeN_MAX_ITERATIONS=90" in out
        assert "shadows" in out
        # Warn-only must NOT mutate .env.
        assert "ReYMeN_MAX_ITERATIONS=90" in (ReYMeN_home / ".env").read_text(
            encoding="utf-8"
        )
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ReYMeN_MAX_ITERATIONS" in out
        env_after = (ReYMeN_home / ".env").read_text(encoding="utf-8")
        assert "ReYMeN_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`ReYMeN doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken ReYMeN install.
