"""Regression tests for #34107 — Docker UID/GID handling in ensure_ReYMeN_home.

When ReYMeN runs in Docker with ``ReYMeN_UID=1000`` / ``ReYMeN_GID=911``,
the entrypoint chowns the top-level ``ReYMeN_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_ReYMeN_home()`` — especially
...
The fix is a ``_chown_to_ReYMeN_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_ReYMeN_uid_gid
# ---------------------------------------------------------------------------


class TestResolveReYMeNUidGid:
...
# ---------------------------------------------------------------------------
# _chown_to_ReYMeN_uid
# ---------------------------------------------------------------------------


class TestChownToReYMeNUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ReYMeN_UID", "1000")
        monkeypatch.setenv("ReYMeN_GID", "911")
        from ReYMeN_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_ReYMeN_uid(d)
