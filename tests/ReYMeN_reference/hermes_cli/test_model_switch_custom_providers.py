...
The terminal `ReYMeN model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import ReYMeN_cli.providers as providers_mod
from ReYMeN_cli.model_switch import list_authenticated_providers, switch_model
from ReYMeN_cli.providers import resolve_provider_full
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "ReYMeN_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("ReYMeN_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("ReYMeN_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: ReYMeN's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_custom_providers_uses_live_models_for_multi_model_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.models.fetch_api_models", fake_fetch_api_models)
