"""Tests for ReYMeN_cli.gateway_windows."""

from pathlib import Path

import pytest

import ReYMeN_cli.gateway as gateway
import ReYMeN_cli.gateway_windows as gateway_windows
import ReYMeN_cli.setup as setup
...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "ReYMeN_Gateway"])

    assert (code, out, err) == (0, "ok", "")
    assert (
        captured["errors"] == "replace"
    ), "schtasks output must decode with errors='replace'"
    assert (
        isinstance(captured["encoding"], str) and captured["encoding"]
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda ReYMeN_home: "")
    monkeypatch.setattr("ReYMeN_cli.config.get_reymen_home", lambda: str(ReYMeN_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "ReYMeN_cli.main"]
    assert cwd == str(ReYMeN_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_ReYMeN_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".ReYMeN"
        home.mkdir()
        monkeypatch.setattr("ReYMeN_cli.config.get_reymen_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(
        ...
        missing = tmp_path / "missing" / ".ReYMeN"
        project = tmp_path / "checkout"
        monkeypatch.setattr("ReYMeN_cli.config.get_reymen_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_ReYMeN_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    ReYMeN_home = tmp_path / "ReYMeN-home"
    ReYMeN_home.mkdir()
    ...
    assert (
        f"cd /d {gateway_windows._quote_cmd_script_arg(str(ReYMeN_home.resolve()))}"
        in content
    )
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "ReYMeN_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "ReYMeN_Gateway_alice.cmd"
    calls = []
    ...
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(
        gateway_windows, "get_task_name", lambda: "ReYMeN_Gateway_alice"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda force=False, start_now=None, start_on_login=None: calls.append(
            ("elevate", force, start_now, start_on_login)
        )
        or True,
    )
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\ReYMeN\\ReYMeN-agent\\venv\\Scripts\\python.exe",
        r"C:\\ReYMeN\\ReYMeN-agent",
        r"C:\\ReYMeNHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_elevated_gateway_command_uses_pythonw_hidden_console(monkeypatch):
    ...
    monkeypatch.setattr(
        gateway_windows.sys, "executable", r"C:\ReYMeN\venv\Scripts\python.exe"
    )
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task(
        "ReYMeN_Gateway_alice", script_path
    )

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "ReYMeN_Gateway_alice")
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(
    ...
    monkeypatch.setattr(
        ...
        lambda task_name, script_path: (
            True,
            "Created Scheduled Task 'ReYMeN_Gateway_alice'",
        ),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(
    ...
    assert "Launched elevated ReYMeN gateway install prompt" in out
...
def test_install_prompts_start_choices_before_uac(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        setup,
        "prompt_yes_no",
        lambda prompt, default=True: calls.append(("prompt", prompt, default))
        or next(answers),
    )
    monkeypatch.setattr(
        gateway_windows,
...
def test_install_start_now_without_login_autostart_never_escalates(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda *args, **kwargs: calls.append(("install_task", args))
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(
    ...
    assert "ReYMeN --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_exec_schtasks",
        lambda args: calls.append(("schtasks", tuple(args)))
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated ReYMeN gateway uninstall prompt" in out
...
# resume_pending=True) never fires when `ReYMeN gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("ReYMeN_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `ReYMeN gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
    ...
    monkeypatch.setattr("ReYMeN_cli.gateway._get_restart_drain_timeout", lambda: 1.0)
...
def test_stop_no_running_gateway_skips_drain(monkeypatch):
    ...
    assert all(
        e[0] != "write_marker" for e in events
