...
class TestMem0UserIdScoping:
    ...
    def test_no_user_id_no_config_uses_ReYMeN_user(self):
        """Without user_id or config override, should default to 'ReYMeN-user'."""
        from plugins.memory.mem0 import Mem0MemoryProvider
...
class TestHonchoUserIdScoping:
    ...
    def test_session_manager_prefers_runtime_user_id_over_config_peer_name(self):
        ...
        with patch.object(
            manager, "_get_or_create_peer", return_value=MagicMock()
        ), patch.object(
            manager,
...
class TestAIAgentUserIdPropagation:
    ...
    def test_user_id_stored_on_agent(self):
        ...
        with patch.dict(os.environ, {"ReYMeN_HOME": "/tmp/test_ReYMeN"}):
