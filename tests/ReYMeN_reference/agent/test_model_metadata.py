...
class TestDefaultContextLengths:
    ...
    def test_deepseek_v4_models_1m_context(self):
        ...
        for key, value in expected_keys.items():
            ...
            assert (
                DEFAULT_CONTEXT_LENGTHS[key] == value
    ...
    def test_openrouter_live_metadata_beats_hardcoded_catchall(self):
        ...
        with mock_patch(
            "agent.model_metadata.fetch_model_metadata", return_value=live
        ), mock_patch(
            "agent.model_metadata._query_ollama_api_show", return_value=None
        ), mock_patch(
            "agent.model_metadata.get_cached_context_length", return_value=None
...
class TestCodexOAuthContextLength:
    ...
    def test_non_codex_providers_unaffected(self):
        ...
        with patch("agent.model_metadata.fetch_model_metadata", return_value={}), patch(
            "agent.model_metadata.fetch_endpoint_model_metadata", return_value={}
        ), patch(
            "agent.model_metadata.get_cached_context_length", return_value=None
...
class TestNousPortalContextResolution:
    ...
    def test_portal_value_is_persisted_to_disk(
        ...
        assert (
            persisted.get(f"qwen3.6-plus@{base_url}") == 262_144
    ...
    def test_stale_cache_is_bypassed_and_overwritten_by_portal(
        ...
        assert (
            remaining.get(stale_key) == 262_144
    ...
    def test_stale_cache_survives_when_portal_unreachable(
        ...
        remaining = yaml.safe_load(cache_file.read_text()).get("context_lengths", {})
        assert (
            remaining.get(existing_key) == 1_000_000
...
class TestStripProviderPrefix:
    ...
    def test_ollama_model_tag_not_mangled_in_context_lookup(self, mock_fetch):
        ...
        with patch(
            ...
            "agent.model_metadata._is_custom_endpoint", return_value=True
        ):
