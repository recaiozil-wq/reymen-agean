"""Tests for the cross-ReYMeN-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another ReYMeN profile's
...
Reference: May 2026 incident — a ReYMeN-security profile session
accidentally edited skills under both ~/.ReYMeN/profiles/ReYMeN-security/skills/
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake ReYMeN root with two profiles, monkeypatch the
...
class TestResolveActiveProfileName:
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_ReYMeN, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_ReYMeN_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_ReYMeN, monkeypatch):
        ...
        assert result["active_profile"] == "ReYMeN-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_ReYMeN, monkeypatch):
        ...
        assert result["target_profile"] == "ReYMeN-security"
...
class TestGetCrossProfileWarning:
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_ReYMeN, monkeypatch):
        ...
        assert "ReYMeN-security" in warn
