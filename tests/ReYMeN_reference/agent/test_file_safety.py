...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_ReYMeN_env(self):
        """ReYMeN' own .env inside ReYMeN_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: ReYMeN internal .env is in ~/.ReYMeN/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.ReYMeN/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.ReYMeN/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal ReYMeN cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        cache = ReYMeN_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._ReYMeN_home_path", return_value=ReYMeN_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal ReYMeN cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        hub = ReYMeN_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_ReYMeN_home(self, tmp_path):
        """The env basename guard does not depend on ReYMeN_HOME resolution."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        cache = ReYMeN_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
