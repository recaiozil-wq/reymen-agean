...
def backup_env(monkeypatch, tmp_path):
    """Isolate ReYMeN_HOME + reload modules so every test starts clean."""
...
def test_snapshot_excludes_backups_dir_itself(backup_env):
    ...
    with tarfile.open(snap2 / "skills.tar.gz") as tf:
        names = tf.getnames()
    assert not any(
        n.startswith(".curator_backups") for n in names
...
def test_resolve_backup_newest_when_no_id(backup_env, monkeypatch):
    ...
    assert (
        resolved.name == "2026-05-02T00-00-00Z"
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd ReYMeN_constants
    ...
    rows = cb.list_backups()
    assert any(
        r.get("reason") == "pre-curator-run" for r in rows
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under ReYMeN_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level ReYMeN_DIR picks up the tmp HOME."""
    import ReYMeN_constants
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.ReYMeN/cron/jobs.json
