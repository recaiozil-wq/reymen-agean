...
engine plugins (e.g. ReYMeN-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``ReYMeN plugins`` picker."""
    from ReYMeN_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "ReYMeN_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "ReYMeN_cli.plugins.get_plugin_context_engine",
