...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "ReYMeN-4", "https://inference-api.nousresearch.com/v1"),
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(
        ...
        with patch(
            ...
            "ReYMeN_cli.auth._read_codex_tokens",
            return_value={
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(
        ...
        ``ReYMeN auth status`` already reports these as logged in; compression
        ...
        from ReYMeN_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(
        ...
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        monkeypatch.setenv("ReYMeN_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestAnthropicOAuthFlag:
    ...
    def test_api_key_no_oauth_flag(self, monkeypatch):
        ...
        ), patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, patch(
            "agent.auxiliary_client._select_pool_entry", return_value=(False, None)
        ):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``ReYMeN model``, no per-task override is
...
class TestExpiredCodexFallback:
    ...
    def test_ReYMeN_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch(
            "agent.anthropic_adapter.resolve_anthropic_token",
            return_value="sk-ant-oat-ReYMeN-token",
        ), patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, patch(
            "agent.auxiliary_client._select_pool_entry", return_value=(False, None)
        ):
            mock_build.return_value = MagicMock()
...
class TestGetTextAuxiliaryClient:
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_nous_auth", return_value=None), patch(
            "agent.auxiliary_client._read_codex_access_token", return_value=None
        ), patch(
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(
        ...
        ), patch("agent.auxiliary_client._read_nous_auth", return_value=None), patch(
...
class TestAuxiliaryPoolAwareness:
    def test_try_nous_uses_pool_entry(self):
        ...
        with (
            ...
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch(
                "ReYMeN_cli.models.get_nous_recommended_aux_model", return_value=None
            ),
        ):
    ...
    def test_try_nous_uses_portal_recommendation_for_text(self):
        ...
        with (
            ...
            patch(
                "ReYMeN_cli.models.get_nous_recommended_aux_model",
...
class TestRefreshNousRecommendedModel:
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr("ReYMeN_cli.models.get_nous_recommended_aux_model", _boom)
        out = _refresh_nous_recommended_model(
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch(
            ...
            "agent.auxiliary_client._try_nous", return_value=(mock_client, "nous-model")
        ), patch(
    ...
    def test_returns_none_when_no_fallback(self):
        with patch(
            "agent.auxiliary_client._try_openrouter", return_value=(None, None)
        ), patch("agent.auxiliary_client._try_nous", return_value=(None, None)), patch(
...
class TestTryMainAgentModelFallback:
    ...
    def test_returns_none_when_main_provider_is_auto(self):
        ...
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="auto"
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "ReYMeN_cli.auth.is_provider_explicitly_configured",
        lambda pid: False,
    )
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("ReYMeN_cli.config.load_config", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_no_warning_when_provider_is_custom(self, monkeypatch, caplog):
        ...
        with patch(
            ...
            "agent.auxiliary_client._read_main_model", return_value="llama3"
        ), patch(
            "agent.auxiliary_client._resolve_custom_runtime",
            return_value=("http://localhost:11434/v1", "test-key", None),
        ), patch("agent.auxiliary_client.OpenAI") as mock_openai, caplog.at_level(
            logging.WARNING, logger="agent.auxiliary_client"
        ):
            mock_openai.return_value = MagicMock()
            _resolve_auto()
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/NousResearch/ReYMeN-agent/issues/23432.
    """
    ...
    def test_evict_cached_client_instance_walks_async_wrapper(self):
        ...
        try:
            ...
            assert ("openai-codex", False, None, None, None) not in _client_cache
            assert (
                ("openai-codex", True, None, None, None) not in _client_cache
    ...
    def test_codex_timeout_evicts_cached_wrapper(self):
        ...
        try:
            ...
            assert (
                cache_key not in _client_cache
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/NousResearch/ReYMeN-agent/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "ReYMeNAgent"
...
class TestOpenRouterExplicitApiKey:
    ...
    def test_resolve_provider_client_passes_explicit_api_key_to_openrouter(
        ...
        with patch("agent.auxiliary_client.OpenAI", mock_openai):
            ...
            call_kwargs = mock_openai.call_args[1]
            assert (
                call_kwargs["api_key"] == "explicit-pool-key"
            ), f"Expected explicit_api_key to be passed, got: {call_kwargs['api_key']}"
            assert (
                call_kwargs["api_key"] != "env-fallback-key"
...
class TestAnthropicExplicitApiKey:
    ...
    def test_try_anthropic_uses_explicit_api_key_over_env(self):
        ...
        assert client is not None
        assert (
            mock_build.call_args.args[0] == "explicit-pool-key"
    ...
    def test_resolve_provider_client_passes_explicit_api_key_to_anthropic(self):
        ...
        with patch(
            "agent.anthropic_adapter.resolve_anthropic_token", return_value="env-key"
        ), patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, patch(
            "agent.auxiliary_client._select_pool_entry", return_value=(False, None)
        ):
            mock_build.return_value = MagicMock()
        ...
        assert client is not None
        assert (
            mock_build.call_args.args[0] == "explicit-fallback-key"
...
class TestAuxUnhealthyCache:
    ...
    def test_resolve_auto_skips_unhealthy_step2(self):
        ...
        ), patch("agent.auxiliary_client._read_main_model", return_value=""), patch(
            "agent.auxiliary_client._try_openrouter"
        ) as or_try, patch(
            "agent.auxiliary_client._try_nous", return_value=(nous_client, "nous-model")
        ), patch(
            "agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)
        ), patch(
            "agent.auxiliary_client._resolve_api_key_provider",
    ...
    def test_resolve_auto_skips_unhealthy_main_in_step1(self):
        ...
        ), patch("agent.auxiliary_client.resolve_provider_client") as step1, patch(
            "agent.auxiliary_client._try_openrouter"
        ) as or_try, patch(
            "agent.auxiliary_client._try_nous", return_value=(nous_client, "n-model")
        ), patch(
            "agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)
        ), patch(
            "agent.auxiliary_client._resolve_api_key_provider",
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="openrouter"
        ), patch("agent.auxiliary_client._try_openrouter") as or_try, patch(
            "agent.auxiliary_client._try_nous", return_value=(nous_client, "n-model")
        ), patch("agent.auxiliary_client._try_custom_endpoint") as custom_try, patch(
            "agent.auxiliary_client._resolve_api_key_provider",
            return_value=(None, None),
        ):
            client, model, label = _try_payment_fallback(
