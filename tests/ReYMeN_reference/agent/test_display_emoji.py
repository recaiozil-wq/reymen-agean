...
class TestGetToolEmoji:
    ...
    def test_returns_registry_emoji_when_no_skin(self):
        ...
        with mock_patch("agent.display._get_skin", return_value=None), mock_patch(
            "agent.display.registry", mock_registry, create=True
        ):
    ...
    def test_skin_empty_dict_falls_through(self):
        ...
        mock_module = MagicMock()
        mock_module.registry = mock_reg
        with mock_patch("agent.display._get_skin", return_value=skin), mock_patch.dict(
...
class TestSkinConfigToolEmojis:
    ...
    def test_build_skin_config_includes_tool_emojis(self):
        from ReYMeN_cli.skin_engine import _build_skin_config
