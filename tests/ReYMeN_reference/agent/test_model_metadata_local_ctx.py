...
class TestQueryLocalContextLengthOllama:
    ...
    def test_ollama_num_ctx_wins_over_model_info(self):
        ...
        training max — using it would let ReYMeN grow conversations past
        the runtime limit and Ollama would silently truncate.

        Concrete example: ReYMeN-brain:qwen3-14b-ctx32k is a Modelfile
        derived from qwen3:14b with `num_ctx 32768`, but the underlying
        GGUF reports `qwen3.context_length: 40960` (training max). If
        ReYMeN used 40960 it would let the conversation grow past 32768
        ...
        assert result == 32768, (
            ...
            "If ReYMeN uses the GGUF training max, conversations will silently truncate."
        )
