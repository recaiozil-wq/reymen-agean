...
``…/sandboxes/<backend>/<task>/home/.ReYMeN/…`` prefix. This covers the
complementary inner-container case: when file tools execute inside Docker,
the bind-mount strips that prefix and the guard sees plain ``/root/.ReYMeN/…``.
...
class TestClassifyContainerMirrorTarget:
    ...
    def test_catches_soul_md_with_context(self):
        ...
        result = classify_container_mirror_target(
            "/root/.ReYMeN/profiles/group1/SOUL.md",
            mirror_prefix="/root/.ReYMeN",
        )
    ...
    def test_non_ReYMeN_path_not_flagged(self):
        """/root/workspace/… is not .ReYMeN state and must not be blocked."""
        from agent.file_safety import classify_container_mirror_target
