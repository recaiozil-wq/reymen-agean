...
def test_shell_linter_skipped_when_lsp_will_handle(ext, tmp_path):
    ...
    with patch.object(fops, "_lsp_will_handle", return_value=True), patch.object(
        fops, "_exec", side_effect=_exec_must_not_run
...
def test_shell_linter_runs_when_lsp_inactive(ext, tmp_path):
    ...
    with patch.object(fops, "_lsp_will_handle", return_value=False), patch.object(
        fops, "_exec", return_value=fake_result
    ) as exec_mock, patch.object(fops, "_has_command", return_value=True):
        result = fops._check_lint(str(src))
...
def test_lsp_will_handle_returns_false_when_service_is_none(tmp_path):
    ...
    with patch.object(fops, "_lsp_local_only", return_value=True), patch(
        "agent.lsp.get_service", return_value=None
    ):
...
def test_lsp_will_handle_returns_false_on_remote_backend(tmp_path):
    ...
    with patch.object(fops, "_lsp_local_only", return_value=False), patch(
...
def test_lsp_will_handle_swallows_enabled_for_exception(tmp_path):
    ...
    with patch.object(fops, "_lsp_local_only", return_value=True), patch(
        "agent.lsp.get_service", return_value=fake_svc
    ):
        assert fops._lsp_will_handle(str(src)) is False
