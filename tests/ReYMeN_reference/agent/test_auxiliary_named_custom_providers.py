...
def _isolate(tmp_path, monkeypatch):
    """Redirect ReYMeN_HOME and clear module caches."""
...
def _write_config(tmp_path, config_dict):
    """Write a config.yaml to the test ReYMeN_HOME."""
    import yaml
...
class TestProvidersDictApiModeAnthropicMessages:
    ...
    def test_providers_dict_propagates_api_mode(self, tmp_path, monkeypatch):
        ...
        from ReYMeN_cli.runtime_provider import _get_named_custom_provider
    ...
    def test_resolve_provider_client_returns_anthropic_client(
        ...
        assert isinstance(
            async_client, AsyncAnthropicAuxiliaryClient
...
class TestCustomProviderAliasCollision:
    ...
    *alias* (not a canonical provider) must win over the built-in.
    ...
    def test_bare_kimi_without_custom_still_routes_to_builtin(
        ...
        assert (
            "moonshot" in base_url or "kimi" in base_url
