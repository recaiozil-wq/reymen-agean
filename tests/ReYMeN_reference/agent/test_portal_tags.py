...
def test_ReYMeN_client_tag_includes_current_version():
    """The client tag must reflect ReYMeN_cli.__version__ verbatim."""
    from ReYMeN_cli import __version__
    from agent.portal_tags import ReYMeN_client_tag

    assert ReYMeN_client_tag() == f"client=ReYMeN-client-v{__version__}"


def test_ReYMeN_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import ReYMeN_client_tag

    tag = ReYMeN_client_tag()
    assert tag.startswith("client=ReYMeN-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import ReYMeN_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=ReYMeN-agent" in tags
    assert ReYMeN_client_tag() in tags
    assert len(tags) == 2
