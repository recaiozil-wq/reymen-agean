...
class CopilotACPClientSafetyTests(unittest.TestCase):
    ...
    def test_read_text_file_blocks_internal_ReYMeN_hub_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(
                os.environ,
                {"HOME": str(home), "ReYMeN_HOME": str(home / ".ReYMeN")},
                clear=False,
            ):
                response = self._dispatch(
                    {
    ...
    def test_read_text_file_redacts_sensitive_content(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # agent.redact snapshots ReYMeN_REDACT_SECRETS at import time into
...
def test_run_prompt_preserves_real_home_when_profile_home_available(
    ...
    assert captured["kwargs"]["env"]["ReYMeN_REAL_HOME"] == str(real_home)
