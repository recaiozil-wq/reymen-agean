...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._ReYMeN_home / "logs" / "errors.log").resolve()
...
class TestSessionJsonSnapshotOptIn:
    ...
    def test_session_json_disabled_by_default(self, agent):
        ...
        assert (
            getattr(agent, "_session_json_enabled", False) is False
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host ReYMeN_REDACT_SECRETS state.
...
class TestInit:
    ...
    def test_session_id_auto_generated(self):
        ...
        with (
            ...
            assert re.match(
                r"^\d{8}_\d{6}_[0-9a-f]{6}$", a.session_id
    ...
    def test_datetime_is_date_only_not_minute_precision(self, agent):
        ...
        for line in prompt.splitlines():
            if line.startswith("Conversation started:"):
                ...
                assert (
                    " AM" not in line and " PM" not in line
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(
        ...
        monkeypatch.setattr("ReYMeN_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("ReYMeN_cli.plugins.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(
        ...
        hook_calls = []
        monkeypatch.setattr(
            "ReYMeN_cli.plugins.get_pre_tool_call_block_message",
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(
        ...
        monkeypatch.setattr(
            "ReYMeN_cli.plugins.invoke_middleware",
            lambda kind, **kwargs: [request_middleware(**kwargs)]
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for ReYMeN tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "ReYMeN_cli.plugins.has_hook",
                side_effect=lambda name: name
                in {"pre_api_request", "post_api_request"},
            ),
            patch("ReYMeN_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(
        ...
        monkeypatch.setattr("ReYMeN_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("ReYMeN_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(
        ...
        monkeypatch.setattr("ReYMeN_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("ReYMeN_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_stub_stall_mid_tool_call_recovers_within_3_retries(self, agent):
        ...
        from ReYMeN_constants import PARTIAL_STREAM_STUB_ID
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("ReYMeN_cli.kanban_db._record_task_failure", mock_record_failure),
            patch("ReYMeN_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when ReYMeN_KANBAN_TASK
        ...
        assert (
            mock_record_failure.call_count == 0
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "ReYMeN_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "ReYMeN Agent" in agent._cached_system_prompt
