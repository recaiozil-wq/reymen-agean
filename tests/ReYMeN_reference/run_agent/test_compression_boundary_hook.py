...
old_session_id=<old>). This lets plugin engines (e.g. ReYMeN-lcm) preserve
DAG lineage across the split instead of treating it as a fresh /new.

See ReYMeN-lcm#68: after ReYMeN compresses and mints a new physical session,
...
class TestCompressionBoundaryHook:
    ...
    def test_on_session_start_called_with_compression_boundary(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                call.args and call.args[0] == agent.session_id
            ), f"Expected new session_id as first positional arg, got {call!r}"
            assert (
                call.kwargs.get("old_session_id") == original_sid
