...
class TestTryRecoverPrimaryTransport:
    ...
    def test_wait_time_scales_with_retry_count(self):
        ...
        with patch("run_agent.OpenAI", return_value=MagicMock()), patch(
            "time.sleep"
        ) as mock_sleep:
    ...
    def test_survives_rebuild_failure(self):
        ...
        with patch("run_agent.OpenAI", side_effect=Exception("socket error")), patch(
            "time.sleep"
        ):
            result = agent._try_recover_primary_transport(
                error,
...
class TestRestoreInRunConversation:
    ...
    def test_restore_called_at_turn_start(self):
        ...
        ) as mock_restore, patch.object(agent, "run_conversation", wraps=None) as _:
