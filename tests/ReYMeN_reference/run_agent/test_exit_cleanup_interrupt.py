...
def _mock_runtime_provider(monkeypatch):
    ...
    is mocked too."""
    import ReYMeN_cli.runtime_provider as rp
...
class TestCronJobCleanup:
    ...
    def test_keyboard_interrupt_in_end_session_does_not_skip_close(self):
        ...
        with patch("ReYMeN_state.SessionDB", return_value=mock_db), patch.object(
            scheduler, "_build_job_prompt", return_value="hello"
        ), patch.object(scheduler, "_resolve_origin", return_value=None), patch.object(
            scheduler, "_resolve_delivery_target", return_value=None
        ), patch("dotenv.load_dotenv", return_value=None), patch(
            "run_agent.AIAgent"
        ) as MockAgent:
