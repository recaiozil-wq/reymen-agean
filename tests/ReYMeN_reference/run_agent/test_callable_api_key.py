...
ReYMeN preserves the callable end-to-end so the SDK refreshes tokens
...
class TestCliEnsureRuntimeCredentialsCallable:
    ...
    ``ReYMeNCLI`` instance) — the predicate change is the load-bearing
...
class TestInlinedDisplayMasks:
    ...
    def test_cli_show_config_handles_callable(self):
        """``cli.ReYMeNCLI.show_config`` previously did
        ...
        assert '"Microsoft Entra ID"' in src, (
            "cli.ReYMeNCLI.show_config must print the static "
