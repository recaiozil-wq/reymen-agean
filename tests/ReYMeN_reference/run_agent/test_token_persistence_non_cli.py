...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    ReYMeN_state = ModuleType("ReYMeN_state")
    ReYMeN_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "ReYMeN_state", ReYMeN_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "ReYMeN"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "ReYMeN"
