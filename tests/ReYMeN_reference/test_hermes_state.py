"""Tests for ReYMeN_state.py — SessionDB SQLite CRUD, FTS5 search, export."""

import sqlite3
import time
import pytest

from ReYMeN_state import SCHEMA_SQL, SCHEMA_VERSION, SessionDB
...
class TestSessionLifecycle:
    ...
    def test_db_initializes_without_fts5_module(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("ReYMeN_state.sqlite3.connect", connect_without_fts)
    ...
    def test_fts_runtime_restores_triggers_after_no_fts_open(
        ...
        monkeypatch.setattr("ReYMeN_state.sqlite3.connect", real_connect)
...
class TestSchemaInit:
    ...
    def test_schema_version(self, db):
        from ReYMeN_state import SCHEMA_VERSION
    ...
    def test_topic_mode_schema_is_not_auto_migrated_on_open(self, tmp_path):
        ...
        The gateway must remain rollback-safe: simply upgrading ReYMeN and starting
    ...
    def test_v9_migration_skips_v10_trigram_backfill_before_v11_rebuild(
        ...
        monkeypatch.setattr("ReYMeN_state.sqlite3.connect", connect_with_trace)
        migrated_db = SessionDB(db_path=db_path)
        try:
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from ReYMeN_state import SCHEMA_SQL
...
class TestConcurrentWriteSafety:
    ...
    def test_sqlite_timeout_is_at_least_30s(self, db):
        ...
        import inspect
        from ReYMeN_state import SessionDB as _SessionDB
...
class TestFTS5ToolCallMigration:
    ...
    def test_v10_to_v11_upgrade_backfills_tool_fields(self, tmp_path):
        ...
        try:
            assert (
                len(session_db.search_messages("LEGACYTOOL")) == 1
            ), "v11 migration must backfill tool_name into FTS"
            assert (
                len(session_db.search_messages("LEGACYARG")) == 1
...
class TestApplyWalProbe:
    ...
    def test_skips_set_pragma_when_already_wal(self, tmp_path):
        ...
        import sqlite3
        from ReYMeN_state import apply_wal_with_fallback
        ...
        assert any(
            "PRAGMA journal_mode" == sql.strip() for sql in conn.executed
        ), "probe PRAGMA should have run"
        assert not any(
            "journal_mode=WAL" in sql for sql in conn.executed
