...
The shim (docker/ReYMeN-exec-shim.sh, installed at /opt/ReYMeN/bin/ReYMeN)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> ReYMeN login` would write /opt/data/auth.json as
...
failed: ReYMeN is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> ReYMeN …`` (defaulting to root) gets dropped to the
   ReYMeN user before the real binary runs.
2. ``docker exec --user ReYMeN <c> ReYMeN …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $ReYMeN_HOME from a ``docker exec`` session land
   as ReYMeN:ReYMeN — the actual user-visible invariant.
4. The ReYMeN_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def test_shim_drops_root_to_ReYMeN_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `ReYMeN` with a Python-style `-c` shim equivalent — there's no
    pure-ReYMeN "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `ReYMeN` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `ReYMeN`
    do anything that writes to disk, then check the file's owner.

    Use `ReYMeN config set` which writes config.yaml under ReYMeN_HOME.
    ...
    assert r.stdout.strip() == "ReYMeN:ReYMeN", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected ReYMeN:ReYMeN. "
        "The shim did not drop privileges before invoking ReYMeN."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user ReYMeN already runs as 10000; shim should be a no-op.
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """ReYMeN_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``ReYMeN_DOCKER_EXEC_AS_ROOT=0``) doesn't
    ...
    assert r.stdout.strip() == "ReYMeN:ReYMeN", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected ReYMeN:ReYMeN"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/ReYMeN/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid ReYMeN ReYMeN <args>` which resolves `ReYMeN`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> ReYMeN login`` (root) wrote
    ...
    "ReYMeN is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `ReYMeN config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is ReYMeN-readable.

    Specifically: pretend the operator ran `ReYMeN login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the ReYMeN UID. We use `ReYMeN auth list` since that touches the
    ...
    # `ReYMeN auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `ReYMeN config set` first to
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the ReYMeN user "
