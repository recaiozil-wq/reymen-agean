"""Harness: dashboard opt-in via ReYMeN_DASHBOARD.

Today (tini): dashboard starts once when ReYMeN_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``ReYMeN`` user
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    false-positive `ReYMeN doctor` and any other health check).
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without ReYMeN_DASHBOARD; "
...
def test_dashboard_restarts_after_crash(
    ...
    # Kill the dashboard. The dashboard process runs as ReYMeN, so the
    # ReYMeN user can kill it (same UID).
...
# whenever `ReYMeN_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``ReYMeN_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    assert "nous" in provider_names, (
        ...
        f"ReYMeN_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and ReYMeN_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_opts_out_of_gate(
    ...
    assert status.get("auth_required") is False, (
        "ReYMeN_DASHBOARD_INSECURE=1 must disable the auth gate (explicit "
