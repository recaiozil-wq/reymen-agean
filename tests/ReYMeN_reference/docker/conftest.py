...
Override the image with ``ReYMeN_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ReYMeN_TEST_IMAGE", "ReYMeN-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ReYMeN_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ReYMeN_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged ReYMeN user
# ---------------------------------------------------------------------------
#
# Background: every ReYMeN runtime path inside the container drops to UID
# 10000 (the ``ReYMeN`` user) via ``s6-setuidgid ReYMeN``. ``docker exec``
...
# fails (PermissionError swallowed) for ReYMeN, so a test that ran as root
...
# helpers below run every probe under ``-u ReYMeN`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: ReYMeN).
