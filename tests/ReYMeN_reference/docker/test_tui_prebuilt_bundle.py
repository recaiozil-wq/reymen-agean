...
package-lock.json. Without ReYMeN_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV ReYMeN_TUI_DIR=/opt/ReYMeN/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the ReYMeN user, return stdout."""
    inner = (
        "source /opt/ReYMeN/.venv/bin/activate && "
        "cd /opt/ReYMeN && "
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from ReYMeN_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/ReYMeN/ui-tui')\n"
