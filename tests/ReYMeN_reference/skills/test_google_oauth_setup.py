...
class TestReYMeNConstantsFallback:
    """Tests for _ReYMeN_home.py fallback when ReYMeN_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_ReYMeN_home.py"
    )
    ...
    def test_fallback_uses_ReYMeN_home_env_var(self, monkeypatch, tmp_path):
    ...
    def test_delegates_to_ReYMeN_constants_when_available(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "_ReYMeN_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
...
class TestInstallDeps:
    ...
    Regression coverage for the ReYMeN Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "ReYMeN-agent[google]" in out
