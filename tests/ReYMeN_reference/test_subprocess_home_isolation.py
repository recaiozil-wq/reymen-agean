...
ReYMeN state stays profile-scoped through ReYMeN_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/ReYMeN-agent/issues/25114
See: https://github.com/NousResearch/ReYMeN-agent/issues/36144
See: https://github.com/NousResearch/ReYMeN-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    ...
    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(ReYMeN_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("ReYMeN_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        ReYMeN_home = real_home / ".ReYMeN" / "profiles" / "coder"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_REAL_HOME", str(real_home))

        from ReYMeN_constants import get_subprocess_home, get_real_home
    ...
    def test_real_home_falls_back_to_os_account_when_home_is_profile(self, tmp_path, monkeypatch):
        ...
        from ReYMeN_constants import get_real_home

        assert get_real_home() != str(profile_home)

    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".ReYMeN" / "profiles"
        ...
        monkeypatch.setenv("ReYMeN_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ReYMeN_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_no_injection_when_ReYMeN_home_unset(self, monkeypatch):
        monkeypatch.delenv("ReYMeN_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["ReYMeN_HOME"] == str(profile)
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(ReYMeN_home / "home"), original_home)
