...
binary download are exercised manually by `ReYMeN secrets bitwarden
...
def ReYMeN_home(tmp_path, monkeypatch):
    """Point ReYMeN at an isolated home directory."""
...
def test_platform_asset_name(system, machine, libc_text, expected):
    with mock.patch.object(
        bw.platform, "system", return_value=system
    ), mock.patch.object(
        bw.platform, "machine", return_value=machine
    ), mock.patch.object(
        bw.subprocess,
        "run",
...
def test_install_bws_rejects_malicious_member(ReYMeN_home, monkeypatch):
...
def test_install_bws_happy_path(ReYMeN_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(ReYMeN_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(ReYMeN_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from ReYMeN_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under ReYMeN_home/cache/."""
    home = tmp_path / ".ReYMeN"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
