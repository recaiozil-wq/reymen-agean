...
class TestWriteDenyExactPaths:
    ...
    def test_ReYMeN_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.ReYMeN/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "ReYMeN_root"
    ...
    def test_shell_profiles_are_writable(self):
        ...
        for name in [".bashrc", ".zshrc", ".profile", ".bash_profile", ".zprofile"]:
            assert (
                _is_write_denied(os.path.join(home, name)) is False
    ...
    def test_credential_config_files_denied(self):
        ...
        for name in [".netrc", ".pgpass", ".npmrc", ".pypirc"]:
            assert (
                _is_write_denied(os.path.join(home, name)) is True
...
class TestWriteAllowed:
    ...
    def test_ReYMeN_control_files_requested_writable(self):
        ...
        for name in ["auth.json", "config.yaml", "webhook_subscriptions.json"]:
            assert (
                _is_write_denied(str(home / name)) is False
            ), f"{name} should be writable"
