...
from tools.environments.local import _ReYMeN_PROVIDER_ENV_FORCE_PREFIX
...
class TestOrphanedPipeReconciliation:
    ...
    `ReYMeN update` in Feishu spawned a background subprocess that restarted
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(
        ...
        ), patch("threading.Thread", return_value=fake_thread), patch.object(
            ...
            registry.spawn_local(
                ...
                env_vars={
                    ...
                    f"{_ReYMeN_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN": "forced-bot-token",
                },
            )
        ...
        assert f"{_ReYMeN_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN" not in env
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/ReYMeN_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/ReYMeN_bg_" not in bg_command
        assert "cat /tmp/ReYMeN_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch(
            "tools.process_registry.time.sleep", return_value=None
        ), patch.object(registry, "_move_to_finished"):
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/ReYMeN_bg.log",
                "/path with spaces/ReYMeN_bg.pid",
                "/path with spaces/ReYMeN_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/ReYMeN_bg.log' 2>/dev/null"
        assert (
            env.commands[1][0]
            == "kill -0 \"$(cat '/path with spaces/ReYMeN_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        )
        assert (
            env.commands[2][0] == "cat '/path with spaces/ReYMeN_bg.exit' 2>/dev/null"
        )
...
class TestPopenLeakOnSetupFailure:
    ...
    def test_popen_killed_when_thread_creation_fails(self, registry):
        ...
        with patch(
            "tools.process_registry._find_shell", return_value="/bin/bash"
        ), patch("subprocess.Popen", return_value=proc), patch(
            "threading.Thread", side_effect=boom
        ), patch("os.getpgid", side_effect=ProcessLookupError), patch.object(
            registry, "_write_checkpoint"
        ):
    ...
    def test_popen_killed_when_write_checkpoint_fails(self, registry):
        ...
        with patch(
            ...
            "threading.Thread", return_value=fake_thread
        ), patch("os.getpgid", side_effect=ProcessLookupError), patch.object(
...
class TestKillProcess:
    ...
    def test_kill_detached_session_uses_host_pid(self, registry):
        ...
        try:
            ...
            with patch("gateway.status._pid_exists", return_value=True), patch.object(
