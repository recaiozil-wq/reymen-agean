...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across ReYMeN processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/ReYMeN-snap-test123.sh"
    env._cwd_file = "/tmp/ReYMeN-cwd-test123.txt"
    env._cwd_marker = "__ReYMeN_CWD_test123__"
...
def test_init_env_args_uses_ReYMeN_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _ReYMeN_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
...
def test_init_env_args_prefers_shell_env_over_ReYMeN_dotenv(monkeypatch):
...
def test_init_env_args_uses_ReYMeN_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.ReYMeN/.env.
    """
    env = _make_execute_only_env(["MY_SECRET"])

    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(
        docker_env, "_load_ReYMeN_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"}
    )
...
def test_init_env_args_never_forwards_blank_secret(monkeypatch):
    ...
    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_ReYMeN_env_vars", lambda: {})
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled ReYMeN image, or ``gosu``/``su``
...
def test_run_command_tags_ReYMeN_agent_label(monkeypatch):
    """Every container ReYMeN-agent starts must carry the ReYMeN-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=ReYMeN-agent=1`` call. Regression test
    ...
    assert (
        "ReYMeN-agent=1" in labels
    ), f"ReYMeN-agent=1 label missing; got labels: {sorted(labels)}"
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root ReYMeN home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert (
        "ReYMeN-task-id=kanban-42" in labels
    ), f"ReYMeN-task-id=kanban-42 missing; got: {sorted(labels)}"
    assert (
        "ReYMeN-profile=research-bot" in labels
    ), f"ReYMeN-profile=research-bot missing; got: {sorted(labels)}"
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert (
        "ReYMeN-task-id=task_with_weird_chars" in labels
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every ReYMeN process spawning a new container
    ...
    assert not start_invocations, f"docker start should be skipped when container already running, got: {start_invocations}"
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new ReYMeN process uses it. Without this
...
def test_no_reuse_when_persist_across_processes_disabled(monkeypatch):
    ...
    assert not ps_invocations, f"docker ps probe should be skipped when persist_across_processes=False, got: {ps_invocations}"
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    ReYMeN startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    SIGKILL'd ReYMeN processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    ...
    container belongs to a ReYMeN process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=ReYMeN-profile=<profile>`` to
    ...
    assert (
        "label=ReYMeN-profile=research-bot" in flat
    ), f"profile filter not applied to docker ps; got args: {ps_calls[0][0]}"
    assert (
        "label=ReYMeN-agent=1" in flat
    ), f"ReYMeN-agent label filter must also be applied; got: {ps_calls[0][0]}"
    assert "status=exited" in flat, (
        "must filter to exited containers only — running containers may "
        "belong to a sibling ReYMeN process and must NEVER be reaped"
    )
    ...
    fake_mounts = [
        {
            ...
            "container_path": "/root/.ReYMeN/google_token.json",
        },
    ]
    monkeypatch.setattr(
    ...
    fake_mounts = [
        {
            ...
            "container_path": "/root/.ReYMeN/deleted_token.json",
        },
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
...
def test_credential_mount_works_when_source_is_valid_file(monkeypatch, tmp_path):
    ...
    fake_mounts = [
        {"host_path": str(valid_file), "container_path": "/root/.ReYMeN/token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_image_uses_init_entrypoint_detects_s6_init(monkeypatch):
    ...
    monkeypatch.setattr(docker_env.subprocess, "run", _run)
    assert (
        docker_env._image_uses_init_entrypoint("/usr/bin/docker", "ReYMeN-agent:latest")
        is True
    )
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="ReYMeN-agent:latest")
    ...
    assert (
        "exec" in run_mounts[0] and "noexec" not in run_mounts[0]
...
def test_is_container_gone_matches_removal_errors(monkeypatch):
    ...
    assert env._is_container_gone(
        "Error response from daemon: No such container: ReYMeN-abc123"
    )
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    outputs = iter(
        [
            {
                "output": "Error response from daemon: No such container: ReYMeN-x",
                "returncode": 1,
            },
