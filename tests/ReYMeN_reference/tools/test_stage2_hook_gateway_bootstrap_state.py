...
ReYMeN_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
ReYMeN_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
/etc/cont-init.d/01-ReYMeN-setup, which runs lexicographically BEFORE
...
This mirrors the existing ReYMeN_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def _seed_block(text: str) -> str:
    ...
    assert m, (
        ...
        "guarded on ReYMeN_GATEWAY_BOOTSTRAP_STATE"
    )
    return m.group(1)
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $ReYMeN_HOME.

    ``env_value`` is the ReYMeN_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export ReYMeN_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset ReYMeN_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
            f'ReYMeN_HOME="{home}"\n'
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when ReYMeN_GATEWAY_BOOTSTRAP_STATE is unset"
