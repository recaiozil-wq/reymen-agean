...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        _tirith_mod._resolved_path = None
        with patch("tools.tirith_security.shutil.which", return_value=None), patch(
            "tools.tirith_security._ReYMeN_bin_dir", return_value="/nonexistent"
        ), patch(
...
class TestUnsupportedPlatform:
    ...
    def test_is_platform_supported_true_on_linux_x86_64(self):
        with patch(
            "tools.tirith_security.platform.system", return_value="Linux"
    ...
    def test_is_platform_supported_true_on_darwin_arm64(self):
        with patch(
            "tools.tirith_security.platform.system", return_value="Darwin"
        ), patch("tools.tirith_security.platform.machine", return_value="arm64"):
            assert _tirith_mod.is_platform_supported() is True

    def test_is_platform_supported_false_on_windows(self):
        with patch(
            "tools.tirith_security.platform.system", return_value="Windows"
    ...
    def test_ensure_installed_unsupported_returns_none_no_thread(self, mock_cfg):
        ...
        _tirith_mod._resolved_path = None
        with patch(
            "tools.tirith_security.is_platform_supported", return_value=False
        ), patch("tools.tirith_security.threading.Thread") as MockThread, patch(
            "tools.tirith_security._mark_install_failed"
        ) as mock_mark, patch("tools.tirith_security.shutil.which") as mock_which:
            result = ensure_installed()
            assert result is None
    ...
    def test_check_command_security_unsupported_allows_silently(self, mock_cfg):
        ...
        with patch(
            "tools.tirith_security.is_platform_supported", return_value=False
        ), patch("tools.tirith_security.subprocess.run") as mock_run, patch(
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(
        ...
        assert reason == ""
        assert path == str(ReYMeN_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(
        ...
        assert not os.path.lexists(ReYMeN_home / "bin" / "tirith")
...
class TestBackgroundInstall:
    def test_ensure_installed_non_blocking(self):
        ...
        ), patch("tools.tirith_security.shutil.which", return_value=None), patch(
            "tools.tirith_security._ReYMeN_bin_dir", return_value="/nonexistent"
        ), patch(
            "tools.tirith_security._is_install_failed_on_disk", return_value=False
        ), patch("tools.tirith_security.threading.Thread") as MockThread:
    ...
    def test_ensure_installed_skips_on_disk_marker(self):
        ...
        with patch(
            ...
            "tools.tirith_security._read_failure_reason", return_value="download_failed"
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_still_checks_local_paths(self):
        ...
        with patch(
            "tools.tirith_security.shutil.which", return_value="/usr/local/bin/tirith"
    ...
    def test_install_failed_recovers_from_ReYMeN_bin(self):
        """After _INSTALL_FAILED, manual install in ReYMeN_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile

        tmpdir = tempfile.mkdtemp()
        ReYMeN_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(ReYMeN_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(ReYMeN_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), patch(
            "tools.tirith_security._ReYMeN_bin_dir", return_value=tmpdir
        ), patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == ReYMeN_bin
            assert _tirith_mod._resolved_path == ReYMeN_bin
            mock_clear.assert_called_once()
    ...
    def test_cosign_missing_disk_marker_allows_retry(self):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), patch(
            ...
            "tools.tirith_security._install_tirith", return_value=("/new/tirith", "")
        ) as mock_install, patch("tools.tirith_security._clear_install_failed"):
            result = _resolve_tirith_path("tirith")
    ...
    def test_in_memory_cosign_missing_retries_when_cosign_appears(self):
        ...
        with patch(
            "tools.tirith_security.shutil.which", side_effect=_which_side_effect
        ), patch(
            "tools.tirith_security._ReYMeN_bin_dir", return_value="/nonexistent"
        ), patch(
            "tools.tirith_security._is_install_failed_on_disk", return_value=False
    ...
    def test_disk_marker_reason_preserved_in_memory(self):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), patch(
            ...
            "tools.tirith_security._read_failure_reason", return_value="cosign_missing"
        ), patch("tools.tirith_security._is_install_failed_on_disk", return_value=True):
            _resolve_tirith_path("tirith")
            assert _tirith_mod._resolved_path is _INSTALL_FAILED
...
# ---------------------------------------------------------------------------
# ReYMeN_HOME isolation
# ---------------------------------------------------------------------------


class TestReYMeNHomeIsolation:
    def test_ReYMeN_bin_dir_respects_ReYMeN_home(self):
        """_ReYMeN_bin_dir must use ReYMeN_HOME, not hardcoded ~/.ReYMeN."""
        from tools.tirith_security import _ReYMeN_bin_dir
        import tempfile

        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ReYMeN_HOME": tmpdir}):
            result = _ReYMeN_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_ReYMeN_home(self):
        """_failure_marker_path must use ReYMeN_HOME, not hardcoded ~/.ReYMeN."""
        from tools.tirith_security import _failure_marker_path

        with patch.dict(os.environ, {"ReYMeN_HOME": "/custom/ReYMeN"}):
            result = _failure_marker_path()
        assert result == "/custom/ReYMeN/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ReYMeN_HOME; verify it's active."""
        ReYMeN_home = os.getenv("ReYMeN_HOME")
...
class TestIsAppTldFinding:
    ...
    def test_matching_message_field(self):
        assert self.fn(
            {"rule_id": "lookalike_tld", "message": "Domain uses '.app' TLD"}
        )
