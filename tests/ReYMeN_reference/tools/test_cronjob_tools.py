...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because ReYMeN uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("ReYMeN_INTERACTIVE", raising=False)
        monkeypatch.delenv("ReYMeN_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("ReYMeN_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("ReYMeN_INTERACTIVE", false_like_value)
        monkeypatch.delenv("ReYMeN_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ReYMeN_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["ReYMeN_INTERACTIVE", "ReYMeN_GATEWAY_SESSION", "ReYMeN_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("ReYMeN_INTERACTIVE", "ReYMeN_GATEWAY_SESSION", "ReYMeN_EXEC_ASK"):
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import ReYMeN_cli.runtime_provider as rp_mod
