...
class TestFindDocker:
    ...
    def test_not_in_path_falls_back_to_known_locations(self, tmp_path):
        ...
        with patch("tools.environments.docker.shutil.which", return_value=None), patch(
            "tools.environments.docker._DOCKER_SEARCH_PATHS", [str(fake_docker)]
        ):
            result = docker_mod.find_docker()
    ...
    def test_returns_none_when_not_found(self):
        with patch("tools.environments.docker.shutil.which", return_value=None), patch(
            "tools.environments.docker._DOCKER_SEARCH_PATHS", ["/nonexistent/docker"]
        ):
            result = docker_mod.find_docker()
    ...
    def test_env_var_override_takes_precedence(self, tmp_path):
        """ReYMeN_DOCKER_BINARY overrides PATH and known-location discovery."""
        fake_binary = tmp_path / "podman"
        fake_binary.write_text("#!/bin/sh\n")
        fake_binary.chmod(0o755)

        with patch.dict(os.environ, {"ReYMeN_DOCKER_BINARY": str(fake_binary)}), patch(
            "tools.environments.docker.shutil.which", return_value="/usr/bin/docker"
        ):
            result = docker_mod.find_docker()
    ...
    def test_env_var_override_ignored_if_not_executable(self, tmp_path):
        """Non-executable ReYMeN_DOCKER_BINARY falls through to normal discovery."""
        fake_binary = tmp_path / "podman"
        fake_binary.write_text("#!/bin/sh\n")
    ...
    def test_env_var_override_ignored_if_nonexistent(self):
        """Non-existent ReYMeN_DOCKER_BINARY path falls through."""
        with patch.dict(
            os.environ, {"ReYMeN_DOCKER_BINARY": "/nonexistent/podman"}
        ), patch(
            "tools.environments.docker.shutil.which", return_value="/usr/bin/docker"
        ):
            result = docker_mod.find_docker()
