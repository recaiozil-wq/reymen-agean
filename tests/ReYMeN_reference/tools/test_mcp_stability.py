...
class TestStdioPidTracking:
    ...
    def test_kill_orphaned_uses_sigkill_when_available(self, monkeypatch):
        ...
        with patch("tools.mcp_tool.os.kill") as mock_kill, patch(
...
class TestStdioPgroupReaping:
    ...
    def test_killpg_used_when_pgid_tracked(self, monkeypatch):
        ...
        with patch("tools.mcp_tool.os.killpg") as mock_killpg, patch(
            "tools.mcp_tool.os.kill"
        ) as mock_kill, patch("gateway.status._pid_exists", return_value=True), patch(
            "time.sleep"
        ):
            _kill_orphaned_mcp_children()
    ...
    def test_killpg_failure_falls_back_to_kill(self, monkeypatch):
        ...
        ) as mock_killpg, patch("tools.mcp_tool.os.kill") as mock_kill, patch(
            "gateway.status._pid_exists", return_value=False
        ), patch("time.sleep"):
            _kill_orphaned_mcp_children()
...
class TestMCPReloadTimeout:
    ...
    def test_reload_timeout_does_not_block_forever(self, tmp_path, monkeypatch):
        ...
        # Create a mock ReYMeNCLI-like object with the needed attributes
        class FakeCLI:
            _config_mtime = 0.0
            _config_mcp_servers = {}
        ...
        source = inspect.getsource(ReYMeNCLI._check_config_mcp_changes)
        # The fix adds threading.Thread for _reload_mcp
        assert (
            "Thread" in source or "thread" in source.lower()
