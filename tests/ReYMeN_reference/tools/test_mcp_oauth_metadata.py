...
Covers:
- :class:`ReYMeNTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.ReYMeNMCPOAuthProvider`) restoring metadata
...
# ---------------------------------------------------------------------------
# ReYMeNTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = ReYMeNTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (ReYMeNMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: ReYMeNTokenStorage, **context_attrs):
    ...
    provider = _ReYMeN_PROVIDER_CLS.__new__(_ReYMeN_PROVIDER_CLS)
    provider._ReYMeN_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ReYMeN_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("noop-srv")
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("flow-srv")
        ...
        with patch.object(
            _ReYMeN_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
