...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``ReYMeN tools`` repaint and at
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.ReYMeN/auth.json
            ...
            json.dumps(
                {
                    ...
                    "providers": {
                        "xai-oauth": {
                            "tokens": {"access_token": "ya29.fake-access-token"}
                        },
                    },
                }
            )
...
class TestXAIProviderSearchJSONPath:
    ...
    def test_happy_path_normalizes_results(self):
        ...
        ), patch.object(xai_provider, "_load_xai_web_config", return_value={}), patch(
            "httpx.post", return_value=_mock_resp(_responses_payload(self._GROK_JSON))
        ):
    ...
    def test_parses_json_with_leading_prose(self):
        ...
        with patch.object(
            ...
            "httpx.post", return_value=_mock_resp(_responses_payload(text))
        ):
    ...
    def test_drops_rows_without_url(self):
        ...
        with patch.object(
            ...
            "httpx.post", return_value=_mock_resp(_responses_payload(bad_json))
        ):
            result = xai_provider.XAIWebSearchProvider().search("q", limit=5)
...
class TestXAIProviderSearchFallbacks:
    def test_falls_back_to_annotations_when_json_missing(self):
        ...
        with patch.object(
            ...
            "httpx.post",
            return_value=_mock_resp(_responses_payload(body, annotations=annotations)),
        ):
    ...
    def test_falls_back_to_citations_list(self):
        ...
        with patch.object(
            ...
            "httpx.post", return_value=_mock_resp(payload)
        ):
            result = xai_provider.XAIWebSearchProvider().search("q", limit=5)
...
class TestXAIProviderRequestShape:
    ...
    def test_allowed_domains_passes_through_as_filters(self):
        ...
        with patch.object(
            xai_provider, "resolve_xai_http_credentials", return_value=_creds()
        ), patch.object(xai_provider, "_load_xai_web_config", return_value=cfg), patch(
            "httpx.post", side_effect=fake_post
        ):
            xai_provider.XAIWebSearchProvider().search("q", limit=5)
...
class TestXAIProviderSearchErrors:
    ...
    def test_request_error_returns_failure(self):
        ...
        with patch.object(
            ...
            "httpx.post", side_effect=httpx.RequestError("boom")
        ):
            result = xai_provider.XAIWebSearchProvider().search("q", limit=5)
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `ReYMeN tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``ReYMeN tools``,
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``ReYMeN auth`` sign-in) rather than an env-var key.
    Patches at the ``ReYMeN_cli.runtime_provider.resolve_runtime_provider``
