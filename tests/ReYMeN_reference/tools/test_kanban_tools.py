...
Verifies:
  - Tools are gated on ReYMeN_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ReYMeN_KANBAN_TASK", "t_fake")
    home = tmp_path / ".ReYMeN"
    home.mkdir()
    monkeypatch.setenv("ReYMeN_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the ReYMeN_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: ReYMeN_HOME isolated, ReYMeN_KANBAN_TASK set
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["ReYMeN_cli/kanban.py"],
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to ReYMeN_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``ReYMeN_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``ReYMeN-system`` and
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from ReYMeN_PROFILE in the fixture, not the
        # caller-supplied "ReYMeN-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no ReYMeN_KANBAN_TASK) still default to
...
def test_create_stamps_session_id_from_env(monkeypatch, worker_env):
    ...
    originating chat session id via ReYMeN_SESSION_ID. ``kanban_create``
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (ReYMeN_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has ReYMeN_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no ReYMeN_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # ReYMeN_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    monkeypatch.setenv("ReYMeN_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no ReYMeN_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via ReYMeN_KANBAN_BOARD env var,
...
# restarting ReYMeN. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated ReYMeN home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    ReYMeN_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".ReYMeN"
    home.mkdir()
    monkeypatch.setenv("ReYMeN_HOME", str(home))
    # Make sure neither ReYMeN_KANBAN_DB nor ReYMeN_KANBAN_BOARD pin a
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning ReYMeN_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".ReYMeN"
    home.mkdir()
    monkeypatch.setenv("ReYMeN_HOME", str(home))
    monkeypatch.setenv("ReYMeN_PROFILE", "alt-worker")
    monkeypatch.delenv("ReYMeN_KANBAN_DB", raising=False)
    monkeypatch.delenv("ReYMeN_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
