...
def _restore_tool_modules():
    original_ReYMeN_home = os.environ.get("ReYMeN_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_ReYMeN_home is None:
            os.environ.pop("ReYMeN_HOME", None)
        else:
            os.environ["ReYMeN_HOME"] = original_ReYMeN_home
        _reset_modules(("tools", "ReYMeN_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    ReYMeN_cli = types.ModuleType("ReYMeN_cli")
    ...
    return {
        "snapshot_store": ReYMeN_home / "modal_snapshots.json",
