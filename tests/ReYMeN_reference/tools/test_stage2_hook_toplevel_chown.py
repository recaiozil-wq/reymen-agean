"""Contract test: the s6-overlay stage2 hook resets ownership of ReYMeN-owned
top-level state files in $ReYMeN_HOME — but only those, never arbitrary
host-owned files.

Regression guard for the gateway restart loop reported in #35098: files such
as gateway.lock / state.db / auth.json live directly under $ReYMeN_HOME (not in
a subdir), so the targeted subdir chown misses them. When created or rewritten
by `docker exec <container> ReYMeN …` (root unless `-u` is passed) they land
root-owned and the unprivileged ReYMeN runtime then hits PermissionError on next
startup.
...
bind-mounted $ReYMeN_HOME may contain host-owned files ReYMeN does not manage,
...
def _toplevel_chown_loop(text: str) -> str:
    """Extract the `for f in … chown ReYMeN:ReYMeN "$ReYMeN_HOME/$f" … done`
...
def _run_loop(text: str, present_files: list[str]) -> list[str]:
    """Run the extracted chown loop in a sandbox $ReYMeN_HOME, with `chown`
