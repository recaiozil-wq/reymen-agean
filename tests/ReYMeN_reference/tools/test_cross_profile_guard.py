...
def fake_ReYMeN(tmp_path, monkeypatch):
    """Build a two-profile ReYMeN layout and point ReYMeN_HOME at
    the ReYMeN-security profile (matching the original-incident shape).
    """
    ...
    sec_home = root / "profiles" / "ReYMeN-security"
...
class TestWriteFileCrossProfileGuard:
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_ReYMeN):
        ...
        assert "ReYMeN-security" in result["error"]
        # File untouched.
...
class TestPatchCrossProfileGuard:
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_ReYMeN):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        self._make_skill_in_profile(fake_ReYMeN["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up ReYMeN_HOME (set in
        ...
        assert "not found in active profile 'ReYMeN-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_ReYMeN, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_ReYMeN["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_ReYMeN["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_prompt_text(self, fake_ReYMeN):
        """When active profile is 'ReYMeN-security', the prompt warns
        ...
        assert "~/.ReYMeN/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active ReYMeN profile: default" in src
        assert "Active ReYMeN profile: {active_profile}" in src
