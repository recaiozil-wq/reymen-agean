...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        ReYMeN_home.mkdir()
        (ReYMeN_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(ReYMeN_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.ReYMeN/token.json"
    ...
    def test_string_entry(self, tmp_path):
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        (ReYMeN_home / "secret.key").write_text("key")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.ReYMeN/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ReYMeN_home = tmp_path / ".ReYMeN"
        (ReYMeN_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ReYMeN_HOME": str(ReYMeN_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.ReYMeN")

        assert mounts[0]["container_path"] == "/home/user/.ReYMeN/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.ReYMeN/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.ReYMeN/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ReYMeN_HOME."""
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path / ".ReYMeN"))
        (tmp_path / ".ReYMeN").mkdir()

        # Create a sensitive file one level above ReYMeN_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside ReYMeN_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ReYMeN_HOME must still be registered."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        (ReYMeN_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_ReYMeN_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ReYMeN_HOME must be allowed."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        subdir = ReYMeN_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ReYMeN_HOME pointing outside must be rejected."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))

        # Create a sensitive file outside ReYMeN_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside ReYMeN_home pointing outside
        symlink = ReYMeN_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ReYMeN_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(ReYMeN_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(ReYMeN_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ReYMeN_HOME via config must still mount."""
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))

        (ReYMeN_home / "oauth.json").write_text("{}")
        self._write_config(ReYMeN_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        (ReYMeN_home / "cache" / "documents").mkdir(parents=True)
        (ReYMeN_home / "cache" / "audio").mkdir(parents=True)
        (ReYMeN_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.ReYMeN/cache/documents" in paths
        assert "/root/.ReYMeN/cache/audio" in paths
        assert "/root/.ReYMeN/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.ReYMeN/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(ReYMeN_home / "document_cache") in host_paths
        assert str(ReYMeN_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.ReYMeN/cache/documents" in container_paths
        assert "/root/.ReYMeN/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        ReYMeN_home = tmp_path / ".ReYMeN"
        img_dir = ReYMeN_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.ReYMeN/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.ReYMeN")
            == "/home/agent/.ReYMeN/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        ReYMeN_home = tmp_path / ".ReYMeN"
        (ReYMeN_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        doc_dir = ReYMeN_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        ss_dir = ReYMeN_home / "cache" / "screenshots"
