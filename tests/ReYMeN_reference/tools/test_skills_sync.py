...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "ReYMeN skills reset new-skill" in captured
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """ReYMeN_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("ReYMeN_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("ReYMeN_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty ReYMeN_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("ReYMeN_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `ReYMeN profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.ReYMeN) rely on so bundled
    skills are never seeded at install time NOR re-injected by `ReYMeN update`.
    """
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        ReYMeN_home = tmp_path / "home"
        ReYMeN_home.mkdir()
        (ReYMeN_home / ".no-bundled-skills").write_text("opted out\n")

        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), patch(
            "tools.skills_sync.SKILLS_DIR", skills_dir
        ), patch("tools.skills_sync.MANIFEST_FILE", manifest_file), patch(
            "tools.skills_sync.ReYMeN_HOME", ReYMeN_home
        ):
            result = sync_skills(quiet=True)
    ...
    def test_no_marker_seeds_normally(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), patch(
            "tools.skills_sync._get_optional_dir",
            return_value=bundled.parent / "optional-skills",
        ), patch("tools.skills_sync.SKILLS_DIR", skills_dir), patch(
            "tools.skills_sync.MANIFEST_FILE", manifest_file
        ), patch("tools.skills_sync.ReYMeN_HOME", ReYMeN_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`ReYMeN skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.ReYMeN_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), patch(
            ...
            "tools.skills_sync.MANIFEST_FILE", manifest_file
        ), patch("tools.skills_sync.ReYMeN_HOME", home):
            sync_skills(quiet=True)
            # User edits 'beta'
...
class TestUpdateBackupRecovery:
    ...
    def test_stale_backup_does_not_poison_failed_update(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file), patch(
            "tools.skills_sync.shutil.copytree", side_effect=_boom
        ):
            sync_skills(quiet=True)
    ...
    def test_partial_copy_failure_restores_original(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file), patch(
            "tools.skills_sync.shutil.copytree", side_effect=_partial_then_fail
        ):
            sync_skills(quiet=True)
