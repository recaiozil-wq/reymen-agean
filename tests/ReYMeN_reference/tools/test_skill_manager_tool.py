...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("ReYMeN_cli.config.load_config", return_value={"skills": {}}):
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.ReYMeN/skills/ as a
    workaround.
    """
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "ReYMeN curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
