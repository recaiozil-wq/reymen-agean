...
class TestGetBrowserEngine:
    ...
    def test_default_is_auto(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            with patch("ReYMeN_cli.config.read_raw_config", return_value={}):
    ...
    def test_config_lightpanda(self):
        ...
        with patch("ReYMeN_cli.config.read_raw_config", return_value=cfg):
    ...
    def test_caching(self):
        ...
        with patch("ReYMeN_cli.config.read_raw_config", mock_read):
            assert _get_browser_engine() == "lightpanda"
            assert _get_browser_engine() == "lightpanda"
...
class TestShouldInjectEngine:
    ...
    def test_lightpanda_injects_in_local_mode(self):
        ...
        with patch("tools.browser_tool._is_camofox_mode", return_value=False), patch(
            "tools.browser_tool._get_cdp_override", return_value=""
    ...
    def test_no_inject_with_cdp_override(self):
        ...
        with patch("tools.browser_tool._is_camofox_mode", return_value=False), patch(
            "tools.browser_tool._get_cdp_override", return_value="ws://localhost:9222"
        ):
            assert _should_inject_engine("lightpanda") is False
...
class TestLightpandaRequirements:
    ...
    def test_lightpanda_local_mode_does_not_require_chromium(self):
        ...
        with patch("tools.browser_tool._is_camofox_mode", return_value=False), patch(
            ...
            return_value=False,
        ), patch("tools.browser_tool._get_cloud_provider", return_value=None), patch(
            "tools.browser_tool._get_browser_engine", return_value="lightpanda"
    ...
    def test_chrome_local_mode_still_requires_chromium(self):
        ...
        with patch("tools.browser_tool._is_camofox_mode", return_value=False), patch(
            ...
            "tools.browser_tool._get_browser_engine", return_value="auto"
        ), patch("tools.browser_tool._chromium_installed", return_value=False):
...
class TestLightpandaFallbackWarning:
    ...
    def test_browser_vision_lightpanda_uses_chrome_capture_and_normal_call_llm_shape(
        ...
        with patch(
            "tools.browser_tool._get_browser_engine", return_value="lightpanda"
        ), patch("tools.browser_tool._should_inject_engine", return_value=True), patch(
            "tools.browser_tool._chrome_fallback_screenshot",
            return_value={"success": True, "data": {"path": str(chrome_shot)}},
        ), patch("ReYMeN_constants.get_reymen_dir", return_value=tmp_path), patch(
            "tools.browser_tool.call_llm", side_effect=fake_call_llm
        ):
            response = json.loads(
    ...
    def test_browser_vision_lightpanda_response_has_structured_fallback(self, tmp_path):
        ...
        with patch(
            ...
            "tools.browser_tool.call_llm", return_value=_Response()
        ):
            response = json.loads(
...
class TestEngineOverride:
    ...
    def test_override_prevents_engine_injection(
        ...
        with patch("subprocess.Popen", side_effect=capture_popen), patch(
            "os.open", return_value=99
        ), patch("os.close"), patch("os.unlink"), patch("os.makedirs"), patch(
            "builtins.open",
            MagicMock(
                return_value=MagicMock(
                    __enter__=MagicMock(
                        return_value=MagicMock(
                            read=MagicMock(return_value='{"success": true, "data": {}}')
                        )
                    ),
    ...
    def test_no_override_uses_cached_engine(
        ...
        with patch("subprocess.Popen", side_effect=capture_popen), patch(
            ...
            MagicMock(
                return_value=MagicMock(
                    __enter__=MagicMock(
                        return_value=MagicMock(read=MagicMock(return_value=mock_stdout))
                    ),
                    __exit__=MagicMock(return_value=False),
                )
            ),
    ...
    def test_hybrid_local_sidecar_injects_engine_even_with_cloud_provider(self):
        ...
        with patch(
            ...
            "tools.browser_tool._find_agent_browser",
            return_value="/usr/bin/agent-browser",
        ), patch("tools.browser_tool._is_local_mode", return_value=False), patch(
            "tools.browser_tool._chromium_installed", return_value=True
        ), patch(
            "tools.browser_tool._get_cloud_provider", return_value=mock_provider
        ), patch("tools.browser_tool._get_cdp_override", return_value=""), patch(
            "tools.browser_tool._is_camofox_mode", return_value=False
        ), patch("subprocess.Popen", side_effect=capture_popen), patch(
            "os.open", return_value=99
        ), patch("os.close"), patch("os.unlink"), patch("os.makedirs"), patch(
            "builtins.open",
            MagicMock(
