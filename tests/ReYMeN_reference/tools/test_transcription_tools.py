...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("ReYMeN_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("ReYMeN_LOCAL_STT_LANGUAGE", raising=False)
...
class TestGetProviderGroq:
    ...
    def test_groq_nothing_available(self, monkeypatch):
        ...
        with patch("tools.transcription_tools._HAS_FASTER_WHISPER", False), patch(
            "tools.transcription_tools._HAS_OPENAI", False
        ):
            from tools.transcription_tools import _get_provider
...
class TestTranscribeLocalCommand:
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("ReYMeN_LOCAL_STT_LANGUAGE", "en")
...
class TestTranscribeLocalExtended:
    def test_model_reuse_on_second_call(self, tmp_path):
        ...
        with patch("tools.transcription_tools._HAS_FASTER_WHISPER", True), patch(
            "faster_whisper.WhisperModel", mock_whisper_cls
        ), patch("tools.transcription_tools._local_model", None), patch(
            "tools.transcription_tools._local_model_name", None
        ):
    ...
    def test_load_time_cuda_lib_failure_falls_back_to_cpu(self, tmp_path):
        ...
        with patch("tools.transcription_tools._HAS_FASTER_WHISPER", True), patch(
            "faster_whisper.WhisperModel", side_effect=fake_whisper
        ), patch("tools.transcription_tools._local_model", None), patch(
            "tools.transcription_tools._local_model_name", None
        ):
            from tools.transcription_tools import _transcribe_local
...
class TestLoadSttConfig:
    ...
    def test_real_load_returns_dict(self):
        ...
        with patch.dict("sys.modules", {"ReYMeN_cli": None, "ReYMeN_cli.config": None}):
            from tools.transcription_tools import _load_stt_config
...
class TestValidateAudioFileEdgeCases:
    ...
    def test_all_supported_formats_accepted(self, tmp_path):
        ...
        for fmt in SUPPORTED_FORMATS:
            ...
            f.write_bytes(b"data")
            assert (
                _validate_audio_file(str(f)) is None
...
class TestTranscribeAudioDispatch:
    def test_dispatches_to_groq(self, sample_ogg):
        ...
        ), patch("tools.transcription_tools._get_provider", return_value="groq"), patch(
    ...
    def test_explicit_openai_no_key_returns_error(self, monkeypatch, sample_ogg):
        ...
        with patch(
            "tools.transcription_tools._load_stt_config",
            return_value={"provider": "openai"},
        ), patch("tools.transcription_tools._HAS_FASTER_WHISPER", False), patch(
            "tools.transcription_tools._HAS_OPENAI", True
        ):
            from tools.transcription_tools import transcribe_audio
...
class TestGetProviderMistral:
    ...
    def test_auto_detect_mistral_after_openai(self, monkeypatch):
        ...
        with patch("tools.transcription_tools._HAS_FASTER_WHISPER", False), patch(
            "tools.transcription_tools._has_local_command", return_value=False
        ), patch("tools.transcription_tools._HAS_OPENAI", False), patch(
            "tools.transcription_tools._HAS_MISTRAL", True
        ):
            from tools.transcription_tools import _get_provider
    ...
    def test_auto_detect_openai_preferred_over_mistral(self, monkeypatch):
        ...
        with patch("tools.transcription_tools._HAS_FASTER_WHISPER", False), patch(
            "tools.transcription_tools._has_local_command", return_value=False
        ), patch("tools.transcription_tools._HAS_OPENAI", True), patch(
            "tools.transcription_tools._HAS_MISTRAL", True
        ):
            from tools.transcription_tools import _get_provider
    ...
    def test_auto_detect_skips_mistral_without_sdk(self, monkeypatch):
        ...
        with patch("tools.transcription_tools._HAS_FASTER_WHISPER", False), patch(
            ...
            "tools.transcription_tools._HAS_MISTRAL", False
        ):
            from tools.transcription_tools import _get_provider
...
class TestTranscribeAudioMistralDispatch:
    def test_dispatches_to_mistral(self, sample_ogg):
        with patch(
            ...
            "tools.transcription_tools._get_provider", return_value="mistral"
        ), patch(
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.ReYMeN_xai_user_agent = MagicMock(return_value="ReYMeN-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(
        ...
        monkeypatch.setenv("ReYMeN_LOCAL_STT_LANGUAGE", "fr")
...
class TestTranscribeAudioXAIDispatch:
    def test_dispatches_to_xai(self, sample_ogg):
        ...
        ), patch("tools.transcription_tools._get_provider", return_value="xai"), patch(
...
class TestTranscribeAudioElevenLabsDispatch:
    def test_dispatches_to_elevenlabs(self, sample_ogg):
        with patch(
            ...
            "tools.transcription_tools._get_provider", return_value="elevenlabs"
        ), patch(
...
class TestShellSafety:
    ...
    def test_env_var_template_uses_shell_path(self, monkeypatch):
        """When ReYMeN_LOCAL_STT_COMMAND is set, use_shell should be True."""
        import os
