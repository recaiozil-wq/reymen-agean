...
class TestProbeMcpServerTools:
    ...
    def test_returns_tools_from_successful_server(self):
        ...
        with patch("tools.mcp_tool._MCP_AVAILABLE", True), patch(
            "tools.mcp_tool._load_mcp_config", return_value=config
        ), patch("tools.mcp_tool._connect_server", side_effect=fake_connect), patch(
            "tools.mcp_tool._ensure_mcp_loop"
        ), patch("tools.mcp_tool._run_on_mcp_loop") as mock_run, patch(
            "tools.mcp_tool._stop_mcp_loop"
        ):
    ...
    def test_cleanup_called_even_on_failure(self):
        ...
        with patch("tools.mcp_tool._MCP_AVAILABLE", True), patch(
            "tools.mcp_tool._load_mcp_config", return_value=config
        ), patch("tools.mcp_tool._ensure_mcp_loop"), patch(
            "tools.mcp_tool._run_on_mcp_loop", side_effect=RuntimeError("boom")
