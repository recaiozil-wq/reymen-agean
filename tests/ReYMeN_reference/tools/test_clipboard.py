...
Coverage:
  ReYMeN_cli/clipboard.py  — platform-specific image extraction (macOS, WSL, Wayland, X11)
...
from ReYMeN_cli.clipboard import (
...
class TestSaveClipboardImage:
    def test_dispatches_to_macos_on_darwin(self, tmp_path):
        ...
        with patch("ReYMeN_cli.clipboard.sys") as mock_sys:
            ...
            with patch("ReYMeN_cli.clipboard._macos_save", return_value=False) as m:
    ...
    def test_dispatches_to_windows_on_win32(self, tmp_path):
        ...
        with patch("ReYMeN_cli.clipboard.sys") as mock_sys:
            ...
            with patch("ReYMeN_cli.clipboard._windows_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
    ...
    def test_dispatches_to_linux_on_linux(self, tmp_path):
        ...
        with patch("ReYMeN_cli.clipboard.sys") as mock_sys:
            ...
            with patch("ReYMeN_cli.clipboard._linux_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
    ...
    def test_creates_parent_dirs(self, tmp_path):
        ...
        with patch("ReYMeN_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("ReYMeN_cli.clipboard._linux_save", return_value=False):
                save_clipboard_image(dest)
...
class TestIsWsl:
    def setup_method(self):
        # _is_wsl is ReYMeN_constants.is_wsl; reset the function's own module
    ...
    def test_regular_linux(self):
        ...
        # supposed to intercept ReYMeN_constants.is_wsl's `open` call,
...
class TestWaylandSave:
    ...
    def test_non_png_conversion_failure_cleans_up(self, tmp_path):
        ...
        with patch("ReYMeN_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch("ReYMeN_cli.clipboard._convert_to_png", return_value=True):
...
class TestLinuxSave:
    ...
    def test_wsl_tried_first(self, tmp_path):
        ...
        with patch("ReYMeN_cli.clipboard._is_wsl", return_value=True):
            with patch("ReYMeN_cli.clipboard._wsl_save", return_value=True) as m:
...
class TestWindowsHasImage:
    ...
    def test_no_powershell_available(self):
        with patch("ReYMeN_cli.clipboard._get_ps_exe", return_value=None):
            assert _windows_has_image() is False
...
class TestHasClipboardImage:
    ...
    def test_macos_dispatch(self):
        with patch("ReYMeN_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "darwin"
            with patch("ReYMeN_cli.clipboard._macos_has_image", return_value=True) as m:
                assert has_clipboard_image() is True
                m.assert_called_once()
...
class TestTryAttachClipboardImage:
    ...
    def test_image_found_attaches(self, cli):
        with patch("ReYMeN_cli.clipboard.save_clipboard_image", return_value=True):
    ...
    def test_no_image_doesnt_attach(self, cli):
        with patch("ReYMeN_cli.clipboard.save_clipboard_image", return_value=False):
            result = cli._try_attach_clipboard_image()
    ...
    def test_mixed_success_and_failure(self, cli):
        ...
        with patch("ReYMeN_cli.clipboard.save_clipboard_image", side_effect=results):
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
    ...
    def test_image_path_follows_naming_convention(self, cli):
        ...
        assert path.parent == Path(os.environ["ReYMeN_HOME"]) / "images"
