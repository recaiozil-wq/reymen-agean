...
``~/.ReYMeN/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.ReYMeN/.env`` would be told
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        """Importing STT while ReYMeN_cli.config.get_env_value is patched must
        ...
        try:
            with patch.object(tt, "_HAS_FASTER_WHISPER", False), patch.object(
                tt, "_HAS_OPENAI", True
            ), patch.object(tt, "_has_local_command", return_value=False), patch(
                "ReYMeN_cli.config.load_env",
    ...
    def test_xai_resolver_import_after_config_env_patch_uses_restored_dotenv_loader(
        ...
        try:
            with patch(
                ...
                "ReYMeN_cli.auth.resolve_xai_oauth_runtime_credentials",
                return_value={},
            ), patch(
                "ReYMeN_cli.config.load_env",
    ...
    def test_explicit_groq_sees_dotenv(self):
        ...
        with patch.object(tt, "_HAS_FASTER_WHISPER", False), patch.object(
            ...
            "ReYMeN_cli.config.load_env", return_value={"GROQ_API_KEY": "dotenv-secret"}
        ):
            assert tt._get_provider({"enabled": True, "provider": "groq"}) == "groq"
    ...
    def test_explicit_xai_sees_dotenv(self):
        ...
        with patch.object(tt, "_HAS_FASTER_WHISPER", False), patch.object(
            ...
            "ReYMeN_cli.config.load_env", return_value={"XAI_API_KEY": "dotenv-secret"}
        ):
    ...
    def test_auto_detect_sees_dotenv_groq(self):
        ...
        with patch.object(tt, "_HAS_FASTER_WHISPER", False), patch.object(
            tt, "_HAS_OPENAI", True
        ), patch.object(tt, "_HAS_MISTRAL", False), patch.object(
            tt, "_has_local_command", return_value=False
        ), patch.object(tt, "_has_openai_audio_backend", return_value=False), patch(
            "ReYMeN_cli.config.load_env", return_value={"GROQ_API_KEY": "dotenv-secret"}
        ):
...
class TestTranscribeCallSitesReadDotenv:
    ...
    def test_transcribe_groq_forwards_dotenv_key(self):
        ...
        with patch.object(
            tt, "get_env_value", return_value="groq-dotenv-key"
        ), patch.object(tt, "_HAS_OPENAI", True), patch.dict(
    ...
    def test_transcribe_mistral_forwards_dotenv_key(self):
        ...
        with patch.object(
            tt, "get_env_value", return_value="mistral-dotenv-key"
        ), patch.dict("sys.modules", {"mistralai.client": fake_client_module}), patch(
    ...
    def test_transcribe_xai_forwards_dotenv_key(self):
        ...
        with patch.object(
            xai_http, "get_env_value", side_effect=fake_get_env_value
    ...
    def test_transcribe_elevenlabs_forwards_dotenv_key(self):
        ...
        ), patch.object(tt, "_load_stt_config", return_value={}), patch(
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``ReYMeN_cli.config.load_env`` to simulate
    ``~/.ReYMeN/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch(
            ...
            # os.environ is empty.
            from ReYMeN_cli.config import get_env_value as live_get
