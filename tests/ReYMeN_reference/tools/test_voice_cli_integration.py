...
def _make_voice_cli(**overrides):
    """Create a minimal ReYMeNCLI with only voice-related attrs initialized.
...
class TestStreamingTTSActivation:
    ...
    def test_activates_when_elevenlabs_and_sounddevice_available(self):
        ...
        with patch("tools.tts_tool._load_tts_config") as mock_cfg, patch(
            "tools.tts_tool._get_provider", return_value="elevenlabs"
        ), patch("tools.tts_tool._import_elevenlabs") as mock_el, patch(
    ...
    def test_does_not_activate_when_elevenlabs_missing(self):
        ...
        use_streaming_tts = False
        with patch(
            "tools.tts_tool._load_tts_config", return_value={"provider": "elevenlabs"}
        ), patch("tools.tts_tool._get_provider", return_value="elevenlabs"), patch(
    ...
    def test_does_not_activate_when_sounddevice_missing(self):
        ...
        with patch(
            ...
            "tools.tts_tool._import_elevenlabs", return_value=MagicMock()
        ), patch(
            "tools.tts_tool._import_sounddevice", side_effect=OSError("no PortAudio")
        ):
            try:
                from tools.tts_tool import (
                    _load_tts_config as load_cfg,
...
class TestDisableVoiceModeStopsTTS:
    ...
    def test_disable_voice_mode_calls_stop_playback(self):
        ...
        source = inspect.getsource(ReYMeNCLI._disable_voice_mode)
        assert (
            "stop_playback" in source
        ), "_disable_voice_mode must call stop_playback()"
        assert (
            "_voice_tts_done.set()" in source
...
class TestVoiceStatusUsesConfigKey:
    ...
    def test_show_voice_status_reads_config(self):
        ...
        method_body = "\n".join(method_lines)
        assert (
            "load_config" in method_body or "record_key" in method_body
                ...
                for child in _ast.walk(node):
                    if isinstance(child, _ast.Try) and child.finalbody:
                        ...
                        if "text_queue" in finally_text:
                            assert (
                                "stop_event" in finally_text
                            ), "finally must also handle stop_event"
                            assert (
                                "tts_thread" in finally_text
...
class TestEnableVoiceModeReal:
    ...
    @patch("cli._cprint")
    @patch("ReYMeN_cli.config.load_config", return_value={"voice": {}})
    @patch(
    ...
    @patch("cli._cprint")
    @patch("ReYMeN_cli.config.load_config", return_value={"voice": {"auto_tts": True}})
    @patch(
        "tools.voice_mode.check_voice_requirements",
        return_value={"available": True, "details": "OK"},
    )
    ...
    @patch("cli._cprint")
    @patch("ReYMeN_cli.config.load_config", side_effect=Exception("broken config"))
    @patch(
        "tools.voice_mode.check_voice_requirements",
        return_value={"available": True, "details": "OK"},
    )
...
class TestVoiceBeepConfigReal:
    ...
    @patch(
        "ReYMeN_cli.config.load_config", return_value={"voice": {"beep_enabled": False}}
    )
...
class TestVoiceStopAndTranscribeReal:
    ...
    @patch("cli.os.unlink")
    @patch("cli.os.path.isfile", return_value=True)
    @patch("ReYMeN_cli.config.load_config", return_value={"stt": {}})
    @patch(
