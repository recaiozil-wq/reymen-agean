...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_ReYMeN_home_override(self, tmp_path):
        """Auth lookup should read from ReYMeN_HOME/auth.json, not ~/.ReYMeN/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".ReYMeN").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``ReYMeN tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_config_exa(self):
        ...
        with patch(
            "tools.web_tools._load_web_config", return_value={"backend": "exa"}
    ...
    def test_fallback_parallel_only_key(self):
        ...
        with patch("tools.web_tools._load_web_config", return_value={}), patch.dict(
    ...
    def test_fallback_tavily_beats_firecrawl_direct(self):
        ...
        with patch("tools.web_tools._load_web_config", return_value={}), patch.dict(
            os.environ, {"TAVILY_API_KEY": "tvly-test", "FIRECRAWL_API_KEY": "fc-test"}
        ):
            assert _get_backend() == "tavily"
    ...
    def test_fallback_tavily_beats_parallel(self):
        ...
        with patch("tools.web_tools._load_web_config", return_value={}), patch.dict(
            os.environ, {"TAVILY_API_KEY": "tvly-test", "PARALLEL_API_KEY": "par-test"}
        ):
            assert _get_backend() == "tavily"
    ...
    def test_fallback_parallel_beats_firecrawl_direct(self):
        ...
        with patch("tools.web_tools._load_web_config", return_value={}), patch.dict(
            os.environ, {"PARALLEL_API_KEY": "test-key", "FIRECRAWL_API_KEY": "fc-test"}
        ):
            assert _get_backend() == "parallel"
    ...
    def test_fallback_no_keys_defaults_to_firecrawl(self):
        ...
        with patch("tools.web_tools._load_web_config", return_value={}), patch(
            "tools.web_tools._ddgs_package_importable", return_value=False
        ):
            assert _get_backend() == "firecrawl"
    ...
    def test_invalid_config_falls_through_to_fallback(self):
        ...
        with patch(
            "tools.web_tools._load_web_config", return_value={"backend": "nonexistent"}
        ), patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            assert _get_backend() == "parallel"
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        ...
        with patch("tools.web_tools._load_web_config", return_value={}), patch(
            "tools.web_tools._is_tool_gateway_ready", return_value=True
...
class TestWebSearchSchema:
    ...
    def test_web_search_clamps_limit_before_backend_call(self):
        ...
        with patch(
            "tools.web_tools._get_search_backend", return_value="parallel"
        ), patch(
            "agent.web_search_registry.get_provider", return_value=fake_provider
        ), patch("tools.interrupt.is_interrupted", return_value=False), patch.object(
            tools.web_tools._debug, "log_call"
...
class TestWebSearchErrorHandling:
    ...
    def test_search_error_response_does_not_expose_diagnostics(self):
        ...
        with patch(
            "tools.web_tools._get_search_backend", return_value="firecrawl"
        ), patch(
            "agent.web_search_registry.get_provider", return_value=fake_provider
        ), patch("tools.interrupt.is_interrupted", return_value=False), patch.object(
            tools.web_tools._debug, "log_call"
        ) as mock_log_call, patch.object(tools.web_tools._debug, "save"):
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "ReYMeN_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
