"""Regression tests for ReYMeN_HOME override propagation onto the MCP loop.
...
def test_override_propagates_to_mcp_loop(tmp_path, monkeypatch, mcp_loop):
    ...
    token = set_ReYMeN_home_override(str(profile_home))
    try:
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: ReYMeNTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(ReYMeNTokenStorage("probe-srv")._tokens_path())
...
def test_concurrent_scopes_do_not_interfere(tmp_path, monkeypatch, mcp_loop):
    ...
    def scoped_call(key, home):
        token = set_ReYMeN_home_override(str(home))
        try:
