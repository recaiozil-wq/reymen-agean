...
the volume.  ReYMeN_UID/ReYMeN_GID must still take precedence when both are
set.
...
def _alias_lines(text: str) -> list[str]:
    """The stage2 hook lines that resolve ReYMeN_UID/ReYMeN_GID from aliases."""
    return [
        line.strip()
        for line in text.splitlines()
        if line.strip().startswith(("ReYMeN_UID=", "ReYMeN_GID="))
    ]
...
def _resolve(stage2_text: str, env: dict[str, str]) -> str:
    ...
    resolved ``ReYMeN_UID:ReYMeN_GID`` pair."""
    bash = shutil.which("bash")
    if bash is None:
        pytest.skip("bash not available")
    script = "\n".join(_alias_lines(stage2_text))
    script += '\necho "${ReYMeN_UID:-}:${ReYMeN_GID:-}"\n'
    proc = subprocess.run(
        [bash, "-ec", script],
...
def test_puid_pgid_populate_ReYMeN_uid_gid(stage2_text: str) -> None:
    assert _resolve(stage2_text, {"PUID": "1000", "PGID": "10"}) == "1000:10"


def test_ReYMeN_uid_gid_take_precedence_over_aliases(stage2_text: str) -> None:
    resolved = _resolve(
        stage2_text,
        {"ReYMeN_UID": "2000", "ReYMeN_GID": "2001", "PUID": "1000", "PGID": "10"},
    )
    assert resolved == "2000:2001"


def test_no_uid_vars_leaves_values_empty(stage2_text: str) -> None:
    # An empty resolution means the stage2 hook keeps the default ReYMeN user.
...
def test_stage2_hook_runs_config_migration_as_ReYMeN(stage2_text: str) -> None:
    assert "scripts/docker_config_migrate.py" in stage2_text
    assert 's6-setuidgid ReYMeN "$INSTALL_DIR/.venv/bin/python"' in stage2_text


def test_stage2_hook_documents_config_migration_opt_out(stage2_text: str) -> None:
    assert "ReYMeN_SKIP_CONFIG_MIGRATION" in stage2_text
