...
def test_strip_responses_format_strips_format_keyword():
    ...
    assert (
        "format" not in result[0]["parameters"]["properties"]["ts"]
    ), "format should be stripped"
    assert (
        result[0]["parameters"]["properties"]["ts"]["type"] == "string"
...
def test_strip_responses_idempotent():
    ...
    assert (
        "pattern" in result[0]["parameters"]["properties"]
