...
code paths ReYMeN takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``ReYMeN_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect ReYMeN_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("ReYMeN_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("ReYMeN_DISABLE_WINDOWS_UTF8", optout)
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("ReYMeN_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestEntryPointsConfigureStdio:
    """cli.py, ReYMeN_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "ReYMeN_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call ReYMeN_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """ReYMeN_cli/_subprocess_compat.py POSIX + Windows behaviour."""
    ...
    def test_windows_flags_zero_on_posix(self):
        from ReYMeN_cli._subprocess_compat import (
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``ReYMeN update``
        (which runs under ReYMeN-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# ReYMeN_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "ReYMeN_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the ReYMeN_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / ReYMeN_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "ReYMeN_cli/tools_config.py",
            "ReYMeN_cli/doctor.py",
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "ReYMeN_cli" / "gateway.py").read_text(encoding="utf-8")
