...
class TestLoadMCPConfig:
    def test_no_config_returns_empty(self):
        ...
        with patch("ReYMeN_cli.config.load_config", return_value={"model": "test"}):
    ...
    def test_valid_config_parsed(self):
        ...
        with patch(
            "ReYMeN_cli.config.load_config", return_value={"mcp_servers": servers}
        ):
            from tools.mcp_tool import _load_mcp_config
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch(
            "ReYMeN_cli.config.load_config", return_value={"mcp_servers": "invalid"}
        ):
            from tools.mcp_tool import _load_mcp_config
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_ReYMeN_schema(self):
...
class TestMCPServerTask:
    ...
    def test_empty_env_gets_safe_defaults(self):
        ...
        async def _test():
            with patch(
                "tools.mcp_tool.StdioServerParameters"
            ) as mock_params, p_stdio, p_cs, patch.dict(
                "os.environ", {"PATH": "/usr/bin", "HOME": "/home/test"}, clear=False
            ):
                server = MCPServerTask("srv")
...
class TestToolsetInjection:
    def test_mcp_tools_resolve_through_server_aliases(self):
        ...
        with patch("tools.mcp_tool._MCP_AVAILABLE", True), patch(
            "tools.mcp_tool._servers", fresh_servers
        ), patch("tools.mcp_tool._load_mcp_config", return_value=fake_config), patch(
            "tools.mcp_tool._connect_server", side_effect=fake_connect
    ...
    def test_server_toolset_skips_builtin_collision(self):
        ...
        fake_toolsets = {
            "ReYMeN-cli": {"tools": ["terminal"], "description": "CLI", "includes": []},
        ...
        with patch("tools.mcp_tool._MCP_AVAILABLE", True), patch(
            ...
            "tools.mcp_tool._connect_server", side_effect=fake_connect
        ), patch("tools.registry.registry", mock_registry), patch(
            "toolsets.TOOLSETS", fake_toolsets
        ):
            from tools.mcp_tool import discover_mcp_tools
    ...
    def test_server_connection_failure_skipped(self):
        ...
        fake_toolsets = {
            "ReYMeN-cli": {"tools": [], "description": "CLI", "includes": []},
        }

        with patch("tools.mcp_tool._MCP_AVAILABLE", True), patch(
            "tools.mcp_tool._servers", fresh_servers
        ), patch("tools.mcp_tool._load_mcp_config", return_value=fake_config), patch(
            "tools.mcp_tool._connect_server", side_effect=flaky_connect
...
class TestHTTPConfig:
    ...
    def test_http_seeds_initial_protocol_header(self):
        ...
        async def _run(config, *, new_http):
            captured.clear()
            with patch("tools.mcp_tool._MCP_HTTP_AVAILABLE", True), patch(
                "tools.mcp_tool._MCP_NEW_HTTP", new_http
            ), patch("httpx.AsyncClient", DummyAsyncClient), patch(
            ...
            ), patch("tools.mcp_tool.ClientSession", DummySession), patch.object(
                MCPServerTask, "_discover_tools", _discover_tools
            ):
                await server._run_http(config)
...
class TestReconnection:
    ...
    def test_reconnect_on_disconnect(self):
        ...
        async def _test():
            ...
            with patch.object(MCPServerTask, "_run_stdio", patched_run_stdio), patch(
    ...
    def test_initial_oauth_failure_does_not_retry(self):
        ...
        async def _test():
            ...
            with patch.object(MCPServerTask, "_run_stdio", patched_run_stdio), patch(
                "tools.mcp_tool._is_auth_error", return_value=True
    ...
    def test_preflight_probe_runs_on_initial_http_connect(self):
        ...
        async def _test():
            ...
            with patch.object(
                MCPServerTask, "_run_http", patched_run_http
            ), patch.object(MCPServerTask, "_preflight_content_type", probe), patch(
                "asyncio.sleep", new_callable=AsyncMock
            ):
...
class TestDiscoveryFailedCount:
    ...
    def test_failed_server_increments_failed_count(self):
        ...
        with patch("tools.mcp_tool._load_mcp_config", return_value=fake_config), patch(
            "tools.mcp_tool._discover_and_register_server", side_effect=fake_register
    ...
    def test_all_servers_fail_still_prints_summary(self):
        ...
        with patch("tools.mcp_tool._load_mcp_config", return_value=fake_config), patch(
            ...
            "tools.mcp_tool._existing_tool_names", return_value=[]
        ):
            _ensure_mcp_loop()
...
class TestMCPSelectiveToolLoading:
    ...
    def test_existing_tool_names_reflect_registered_subset(self):
        ...
        async def run():
            with patch(
                "tools.mcp_tool._connect_server", side_effect=fake_connect
            ), patch.dict("tools.mcp_tool._servers", {}, clear=True), patch(
                "tools.registry.registry", mock_registry
    ...
    def test_no_toolset_created_when_everything_is_filtered_out(self):
        ...
        async def run():
            with patch(
                ...
                "toolsets.create_custom_toolset", mock_create
            ):
...
class TestRegisterMcpServers:
    ...
    def test_connects_new_servers(self):
        ...
        with patch("tools.mcp_tool._MCP_AVAILABLE", True), patch(
            ...
            "tools.mcp_tool._existing_tool_names", return_value=["mcp_my_server_tool1"]
        ):
            _ensure_mcp_loop()
...
class TestMcpParallelToolCalls:
    ...
    def test_register_mcp_servers_tracks_parallel_flag(self):
        ...
        ), patch("tools.mcp_tool._run_on_mcp_loop"), patch(
            "tools.mcp_tool._existing_tool_names", return_value=[]
        ):
            register_mcp_servers(fake_config)
