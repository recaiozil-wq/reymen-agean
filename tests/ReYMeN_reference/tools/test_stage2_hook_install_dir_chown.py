...
When ReYMeN_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the ReYMeN user's home directory ($ReYMeN_HOME == /opt/data).
Runtime-writable trees under ``/opt/ReYMeN`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/ReYMeN/gateway`` is one such
