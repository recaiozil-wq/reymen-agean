...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in ReYMeN-discord
# ---------------------------------------------------------------------------


class TestToolsetInclusion:
    def test_discord_tools_in_ReYMeN_discord_toolset(self):
        from toolsets import TOOLSETS

        assert "discord" in TOOLSETS["ReYMeN-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["ReYMeN-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _ReYMeN_CORE_TOOLS

        assert "discord" not in _ReYMeN_CORE_TOOLS
        assert "discord_admin" not in _ReYMeN_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS

        for name, ts in TOOLSETS.items():
            if name in {"ReYMeN-discord", "ReYMeN-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
            assert (
                "discord" not in tools or name == "discord"
            ), f"discord tool should not be in toolset '{name}'"
            assert (
                "discord_admin" not in tools or name == "discord_admin"
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("ReYMeN_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(
            enabled_toolsets=["ReYMeN-discord"], quiet_mode=True
        )
