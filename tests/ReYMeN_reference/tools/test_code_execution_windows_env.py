...
``ReYMeN_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``ReYMeN_`` prefix was dropped
...
# ``ReYMeN_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_ReYMeN_tools_module(["terminal"], transport="uds")
...
# let the child import ReYMeN_tools, a user script that printed non-ASCII
