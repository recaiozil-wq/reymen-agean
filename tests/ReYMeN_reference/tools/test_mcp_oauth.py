...
class TestReYMeNTokenStorage:
    def test_roundtrip_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("test-server")
    ...
    def test_token_file_created_with_0o600(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("perm-test-server")
    ...
    def test_has_cached_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("my-server")
    ...
    def test_corrupt_tokens_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("bad-server")
...
class TestPathTraversal:
    ...
    def test_path_traversal_blocked(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("../../.ssh/config")
    ...
    def test_dots_and_slashes_sanitized(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ReYMeN_HOME", str(tmp_path))
        storage = ReYMeNTokenStorage("../../../etc/passwd")
        path = storage._tokens_path()
...
def test_build_oauth_auth_preserves_server_url_path():
    ...
    ``OAuthContext.get_authorization_base_url``; ReYMeN must not pre-strip.
    """
    ...
    with patch.object(mcp_oauth, "_OAUTH_AVAILABLE", True), patch.object(
        mcp_oauth, "OAuthClientProvider", _FakeProvider
    ), patch.object(mcp_oauth, "_is_interactive", return_value=True), patch.object(
        mcp_oauth, "_maybe_preregister_client"
    ), patch.object(mcp_oauth, "ReYMeNTokenStorage") as mock_storage_cls:
...
class TestPasteCallbackSkipToken:
    ...
    def test_skip_message_printed(self, monkeypatch, capsys):
        ...
        assert "ReYMeN mcp login" in err
