...
def test_resolve_stdio_command_falls_back_to_ReYMeN_node_bin(tmp_path):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$ReYMeN_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the ReYMeN Docker image
    ...
    # the other candidates ($ReYMeN_HOME/node/bin/npx and ~/.local/bin/npx)
