"""Contract test: the s6-overlay stage2 hook seeds $ReYMeN_HOME/logs/gateways
as the ReYMeN user.
...
ReYMeN user and s6-log crash-loops on `mkdir: Permission denied`.
...
def _seed_mkdir_block(text: str) -> str:
    """Extract the `as_ReYMeN mkdir -p \\ ...` seed block."""
    m = re.search(r"as_ReYMeN mkdir -p \\\n(?:[^\n]*\\\n)*[^\n]*\n", text)
    assert m, "stage2-hook.sh must contain the as_ReYMeN mkdir -p seed block"
    return m.group(0)


def test_logs_gateways_is_seeded(stage2_text: str) -> None:
    block = _seed_mkdir_block(stage2_text)
    assert '"$ReYMeN_HOME/logs/gateways"' in block, (
        "logs/gateways must be seeded ReYMeN-owned in stage2 so profiles "
    ...
    assert '"$ReYMeN_HOME/logs"' in block
