...
def test_ReYMeN_provider_subclass_exists():
    """ReYMeNMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ReYMeN_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ReYMeN_PROVIDER_CLS is not None
    assert issubclass(_ReYMeN_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_ReYMeN_provider_subclass(tmp_path, monkeypatch):
    ...
    assert _ReYMeN_PROVIDER_CLS is not None
    assert isinstance(provider, _ReYMeN_PROVIDER_CLS)
    assert provider._ReYMeN_server_name == "srv"
