...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ReYMeN_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ReYMeN_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("ReYMeN_CRON_SESSION", raising=False)
...
def test_env_scrub_ReYMeN_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ReYMeN_HOME": "/h", "ReYMeN_PROFILE": "p",
        "ReYMeN_CONFIG": "/c.yaml", "ReYMeN_ENV": "/e",
        # other ReYMeN_* → dropped (broad prefix removed)
        "ReYMeN_BASE_URL": "https://x", "ReYMeN_INTERACTIVE": "1",
        "ReYMeN_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ReYMeN_HOME", "ReYMeN_PROFILE", "ReYMeN_CONFIG", "ReYMeN_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ReYMeN_BASE_URL", "ReYMeN_INTERACTIVE", "ReYMeN_KANBAN_DB",
...
def test_env_scrub_logs_dropped_ReYMeN_vars(caplog):
    """Dropping a non-allowlisted, non-secret ReYMeN_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ReYMeN_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ReYMeN_HOME": "/h",          # allowlisted → kept, not logged
        "ReYMeN_BASE_URL": "https://x",   # dropped → logged
        "ReYMeN_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ReYMeN_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ReYMeN_HOME" in out and "PATH" in out
    assert "ReYMeN_BASE_URL" not in out and "ReYMeN_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ReYMeN_BASE_URL" in msgs and "ReYMeN_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ReYMeN_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ReYMeN_* vars."""
    import logging
