...
class TestSendMattermost:
    def test_success(self):
        ...
        with patch("aiohttp.ClientSession", return_value=session_ctx), patch.dict(
...
class TestSendMatrix:
    def test_success(self):
        ...
        assert url.startswith(
            "https://matrix.example.com/_matrix/client/v3/rooms/%21room%3Aexample.com/send/m.room.message/"
        )
...
class TestSendHomeAssistant:
    def test_success(self):
        ...
        with patch("aiohttp.ClientSession", return_value=session_ctx), patch.dict(
            os.environ, {"HASS_URL": "", "HASS_TOKEN": ""}, clear=False
        ):
...
class TestSendDingtalk:
    ...
    def test_env_var_fallback(self):
        ...
        with patch("httpx.AsyncClient", return_value=client_ctx), patch.dict(
            os.environ,
            {
