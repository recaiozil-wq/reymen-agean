...
class TestParseFrontmatterQuick:
    ...
    def test_nested_yaml(self):
        ...
        fm = GitHubSource._parse_frontmatter_quick(content)
        assert fm["metadata"]["ReYMeN"]["tags"] == ["a", "b"]
...
class TestSkillsShGroupings:
        ...
        content = json.dumps(
            {
                ...
                "groupings": [
                    {
                        "title": "Inference AI",
                        "skills": ["dynamo-router", "dynamo-recipe"],
                    },
    ...
    def test_list_skills_stamps_category_from_sidecar(self):
        ...
        with patch.object(src, "_read_cache", return_value=None), patch.object(
            src, "_write_cache"
        ), patch.object(
            src, "_get_skillsh_groupings", return_value=groupings
        ), patch.object(src, "inspect", return_value=meta), patch(
            "tools.skills_hub.httpx.get", return_value=resp
        ):
    ...
    def test_list_skills_no_sidecar_leaves_extra_empty(self):
        ...
        with patch.object(src, "_read_cache", return_value=None), patch.object(
            src, "_write_cache"
        ), patch.object(src, "_get_skillsh_groupings", return_value=None), patch.object(
            src, "inspect", return_value=meta
...
class TestTrustLevelFor:
    ...
    def test_nvidia_skills_tap_is_registered_and_trusted(self):
        ...
        # browseable/searchable through `ReYMeN skills browse` must also
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `ReYMeN skills browse`."
            )
...
class TestQuarantineBundleBinaryAssets:
    def test_quarantine_bundle_writes_binary_files(self, tmp_path):
        ...
        with patch.object(hub, "SKILLS_DIR", tmp_path / "skills"), patch.object(
            hub, "HUB_DIR", hub_dir
        ), patch.object(hub, "LOCK_FILE", hub_dir / "lock.json"), patch.object(
            hub, "QUARANTINE_DIR", hub_dir / "quarantine"
        ), patch.object(hub, "AUDIT_LOG", hub_dir / "audit.log"), patch.object(
            hub, "TAPS_FILE", hub_dir / "taps.json"
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_symlinks(self, tmp_path):
        ...
        with patch.object(hub, "SKILLS_DIR", skills_dir), patch.object(
            hub, "QUARANTINE_DIR", quarantine_root
        ):
