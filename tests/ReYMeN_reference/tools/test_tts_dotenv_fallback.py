...
TTS provider tools must resolve API keys from ``~/.ReYMeN/.env`` (via
``ReYMeN_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``ReYMeN_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.ReYMeN/.env`` carries the
    ...
    ``ReYMeN_cli.config.load_env`` integration. Before the fix, ``os.getenv``
    ...
    def test_elevenlabs_reads_dotenv_key(self, tmp_path):
        ...
        with patch.object(
            tts_tool, "get_env_value", return_value="el-dotenv-key"
    ...
    def test_xai_reads_dotenv_key(self, tmp_path):
        ...
        with patch.object(
            xai_http, "get_env_value", return_value="xai-dotenv-key"
    ...
    def test_minimax_reads_dotenv_key(self, tmp_path):
        ...
        with patch.object(
            tts_tool, "get_env_value", return_value="mm-dotenv-key"
        ), patch("requests.post", side_effect=fake_post):
    ...
    def test_mistral_reads_dotenv_key(self, tmp_path):
        ...
        with patch.object(
            tts_tool, "get_env_value", return_value="mistral-dotenv-key"
        ), patch.object(
    ...
    def test_gemini_reads_dotenv_key(self, tmp_path):
        ...
        with patch.object(
            tts_tool, "get_env_value", side_effect=fake_get_env_value
        ), patch("requests.post", side_effect=fake_post):
...
class TestRegressionGuard:
    ...
    ``ReYMeN_cli.config.load_env`` to simulate ``~/.ReYMeN/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(
        ...
        """Importing TTS while ReYMeN_cli.config.get_env_value is patched must
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(
        ...
        # Simulate ~/.ReYMeN/.env carrying the key (load_env returns the dict
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.ReYMeN/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
        ...
        with patch(
            ...
            tts_tool, "_import_edge_tts", side_effect=ImportError
        ), patch.object(
            tts_tool, "_import_elevenlabs", side_effect=ImportError
        ), patch.object(
            tts_tool, "_import_openai_client", side_effect=ImportError
        ), patch.object(
            tts_tool, "_check_neutts_available", return_value=False
