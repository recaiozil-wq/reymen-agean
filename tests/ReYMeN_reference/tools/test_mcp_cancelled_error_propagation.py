...
server stayed dead until ReYMeN was restarted. See #9930.
...
class TestCancelledErrorPropagation:
    def test_cancelled_error_is_not_swallowed_by_except_exception(self):
        ...
        async def drive():
            with patch.object(MCPServerTask, "_run_stdio", _hanging_run), patch.object(
                MCPServerTask, "_is_http", lambda self: False
            ):
