...
Verifies that ReYMeN-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed ReYMeN secrets.

See: https://github.com/NousResearch/ReYMeN-agent/issues/1002
See: https://github.com/NousResearch/ReYMeN-agent/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _ReYMeN_PROVIDER_ENV_BLOCKLIST,
    _ReYMeN_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.ReYMeN/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a ReYMeN inference secret
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _ReYMeN_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        ReYMeN-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "ReYMeN_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _ReYMeN_FORCE_ prefix."""
    ...
    def test_force_prefix_passes_blocked_var(self):
        ...
        assert f"{_ReYMeN_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_ReYMeN_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their ReYMeN-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _ReYMeN_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to ReYMeN, and blocklisting
        ...
        leaked_block = general_chain & _ReYMeN_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_ReYMeN_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_gateway_runtime_vars_are_in_blocklist(self):
        extras = {
            ...
            "ReYMeN_DASHBOARD_SESSION_TOKEN",
