...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document/video cache to tmp_path so tests don't touch ~/.ReYMeN."""
    monkeypatch.setattr(
...
class TestDocumentDownloadBlock:
    ...
    async def test_png_document_is_routed_as_image(self, adapter):
        ...
        with patch.object(
            adapter, "_photo_batch_key", return_value="batch-1"
    ...
    async def test_spoofed_png_document_falls_back_with_error(self, adapter):
        ...
        with patch.object(
            adapter, "_photo_batch_key", return_value="batch-2"
        ), patch.object(adapter, "_enqueue_photo_event") as enqueue_mock:
            await adapter._handle_media_message(update, MagicMock())
