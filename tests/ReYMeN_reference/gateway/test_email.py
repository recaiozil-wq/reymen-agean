...
class TestConfigEnvOverrides(unittest.TestCase):
    ...
    def test_email_config_loaded_from_env(self):
        ...
        self.assertEqual(
            config.platforms[Platform.EMAIL].extra["address"], "ReYMeN@test.com"
        )
...
class TestDispatchMessage(unittest.TestCase):
    ...
    def test_non_allowlisted_sender_dropped(self):
        ...
        with patch.dict(
            os.environ,
            {
                "EMAIL_ALLOWED_USERS": "ReYMeN@test.com,admin@test.com",
            },
        ):
            adapter = self._make_adapter()
            adapter._message_handler = MagicMock()
...
class TestThreadContext(unittest.TestCase):
    ...
    def test_no_thread_context_uses_default_subject(self):
        """Without thread context, subject should be 'Re: ReYMeN Agent'."""
        adapter = self._make_adapter()
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            send_call = mock_server.send_message.call_args[0][0]
            self.assertEqual(send_call["Subject"], "Re: ReYMeN Agent")
            self.assertIn("Date", send_call)
...
class TestSendMethods(unittest.TestCase):
    ...
    def test_send_calls_smtp(self):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            result = asyncio.run(adapter.send("user@test.com", "Hello from ReYMeN!"))

            self.assertTrue(result.success)
            mock_server.starttls.assert_called_once()
            mock_server.login.assert_called_once_with("ReYMeN@test.com", "secret")
...
class TestConnectDisconnect(unittest.TestCase):
    ...
    def test_connect_smtp_failure(self):
        ...
        with patch("imaplib.IMAP4_SSL", return_value=mock_imap), patch(
            "smtplib.SMTP", side_effect=Exception("SMTP down")
        ):
            result = asyncio.run(adapter.connect())
            self.assertFalse(result)
...
class TestSendEmailStandalone(unittest.TestCase):
    ...
    def test_send_email_tool_success(self):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            result = asyncio.run(
                _send_email(
                    {"address": "ReYMeN@test.com", "smtp_host": "smtp.test.com"},
                    "user@test.com",
                    "Hello",
                )
            )
            ...
            send_call = mock_server.send_message.call_args[0][0]
            self.assertEqual(send_call["Subject"], "ReYMeN Agent")
            self.assertIn("Date", send_call)
            self.assertEqual(send_call["To"], "user@test.com")
            self.assertEqual(send_call["From"], "ReYMeN@test.com")
...
class TestImapIdExtensionForNetEase(unittest.TestCase):
    ...
    def test_connect_sends_imap_id_after_login(self):
        ...
        self.assertIn("ReYMeN-agent", payload)
...
class TestConnectSmtp(unittest.TestCase):
    ...
    def test_ipv6_timeout_falls_back_to_ipv4(self):
        ...
        with patch(
            "smtplib.SMTP", side_effect=_socket.timeout("timed out")
    ...
    def test_port_465_ipv6_fallback(self):
        ...
        with patch(
            "smtplib.SMTP_SSL", side_effect=_socket.timeout("timed out")
    ...
    def test_tls_verification_error_does_not_retry_ipv4(self):
        ...
        with patch(
            "smtplib.SMTP_SSL", side_effect=_ssl.SSLError("cert verify failed")
        ), patch.object(email_mod, "_IPv4SMTP_SSL") as mock_ipv4_smtp_ssl:
