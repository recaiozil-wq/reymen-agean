...
class TestOpusFindLibrary:
    ...
    def test_homebrew_fallback_is_conditional(self):
        ...
        assert (
            "/opt/homebrew" in source or "homebrew" in source
        ...
        assert (
            "sys.platform" in source or "darwin" in source
        ), "Homebrew fallback must be guarded by macOS platform check"

    def test_windows_bundled_discord_opus_dll_is_discovered(
