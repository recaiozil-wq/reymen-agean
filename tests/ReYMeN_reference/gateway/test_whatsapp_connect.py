...
class TestDataInitialized:
    ...
    async def test_no_name_error_when_json_always_fails(self):
        ...
        with patches[0], patches[1], patches[2], patches[3], patches[4], patches[
            ...
            type(adapter), "_poll_messages", return_value=MagicMock()
        ):
...
class TestConnectCleanup:
    ...
    async def test_releases_lock_when_npm_install_fails(self):
        ...
        with patch(
            "gateway.platforms.whatsapp.check_whatsapp_requirements", return_value=True
        ), patch.object(Path, "exists", autospec=True, side_effect=_path_exists), patch(
...
class TestBridgeRuntimeFailure:
    ...
    async def test_closed_on_unexpected_exception(self):
        ...
        ), patch("subprocess.run", return_value=MagicMock(returncode=0)), patch(
...
class TestKillPortProcess:
    ...
    def test_uses_netstat_and_taskkill_on_windows(self):
        ...
        with patch("gateway.platforms.whatsapp._IS_WINDOWS", True), patch(
    ...
    def test_uses_fuser_on_linux(self):
        ...
        with patch("gateway.platforms.whatsapp._IS_WINDOWS", False), patch(
