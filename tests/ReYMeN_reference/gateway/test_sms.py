...
class TestStartupGuard:
    ...
    async def test_insecure_flag_does_not_set_fatal_error(self):
        ...
        with patch.dict(os.environ, {"SMS_INSECURE_NO_SIGNATURE": "true"}), patch(
            "aiohttp.web.AppRunner"
        ) as mock_runner_cls, patch("aiohttp.web.TCPSite") as mock_site_cls, patch(
    ...
    async def test_webhook_url_allows_start(self):
        mock_session = AsyncMock()
        with patch("aiohttp.web.AppRunner") as mock_runner_cls, patch(
            "aiohttp.web.TCPSite"
        ) as mock_site_cls, patch("aiohttp.ClientSession", return_value=mock_session):
            mock_runner_cls.return_value.setup = AsyncMock()
            mock_runner_cls.return_value.cleanup = AsyncMock()
            mock_site_cls.return_value.start = AsyncMock()
            adapter = self._make_adapter(
