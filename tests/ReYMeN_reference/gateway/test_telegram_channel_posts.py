...
posts are converted into ReYMeN gateway events instead of being silently
ignored.
"""
...
def _make_channel_message(text="channel id test @ReYMeN_bot"):
...
async def test_text_handler_uses_effective_message_for_channel_post(
    ...
    assert event.text == "channel id test @ReYMeN_bot"
