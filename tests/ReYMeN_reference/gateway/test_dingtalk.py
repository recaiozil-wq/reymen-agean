...
class TestDingTalkRequirements:
    def test_returns_false_when_sdk_missing(self, monkeypatch):
        with patch.dict("sys.modules", {"dingtalk_stream": None}), patch(
...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "ReYMeN"
...
class TestConnect:
    ...
    async def test_disconnect_finalizes_open_streaming_cards(self):
        ...
        async def fake_close_siblings(chat_id):
            ...
            assert (
                adapter._http_client is not None
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^ReYMeN"]}
        )
        assert adapter._message_matches_mention_patterns("ReYMeN please help") is True
        assert adapter._message_matches_mention_patterns("please ReYMeN help") is False

    def test_pattern_is_case_insensitive(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^ReYMeN"]}
        )
        assert adapter._message_matches_mention_patterns("ReYMeN help") is True
...
class TestShouldProcessMessage:
    ...
    def test_group_accepted_when_text_matches_wake_word(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch,
            extra={"require_mention": True, "mention_patterns": ["^ReYMeN"]},
        )
        msg = MagicMock(is_in_at_list=False)
        assert (
            adapter._should_process_message(
                msg, "ReYMeN help", is_group=True, chat_id="grp1"
            )
            is True
        )
...
class TestExtractTextMentions:
    def test_preserves_at_mentions_in_text(self):
        ...
        for text, expected in cases:
            ...
            msg.rich_text_content = None
            assert (
                DingTalkAdapter._extract_text(msg) == expected
