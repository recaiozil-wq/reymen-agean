...
def adapter(tmp_path):
    ...
    ~/.ReYMeN/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.ReYMeN/...)
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the ReYMeN host. The relay flattens Chat
...
class TestBuildMessageEvent:
    ...
    async def test_dm_different_top_level_threads_share_session(self, adapter):
        ...
        for tid in ("T_a", "T_b", "T_c"):
            ...
            event = await adapter._build_message_event(msg, env)
            assert (
                event.source.thread_id is None
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'ReYMeN is thinking…' stuck in chat. Race fix via
...
class TestPerUserAttachmentRouting:
    ...
    async def test_setup_files_writes_to_per_user_path(
        ...
        with patch.object(helper, "exchange_auth_code") as ex, patch.object(
            helper, "load_user_credentials", return_value=alice_creds
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/ReYMeN-chat"
            ),
        ...
        monkeypatch.setattr("ReYMeN_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("ReYMeN_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("ReYMeN_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "ReYMeN_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr("ReYMeN_cli.cli_output.print_info", lambda *_a, **_kw: None)
        monkeypatch.setattr(
            "ReYMeN_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "ReYMeN_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
