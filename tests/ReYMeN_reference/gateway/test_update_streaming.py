...
Tests the new --gateway mode for ReYMeN update, including:
...
class TestGatewayPrompt:
    ...
    def test_writes_prompt_file_and_reads_response(self, tmp_path):
        ...
        def write_response():
            time.sleep(0.3)
            (ReYMeN_home / ".update_response").write_text("y")
        ...
        assert not (ReYMeN_home / ".update_prompt.json").exists()
        assert not (ReYMeN_home / ".update_response").exists()
    ...
    def test_prompt_file_content(self, tmp_path):
        ...
        def capture_and_respond():
            ...
            prompt_path = ReYMeN_home / ".update_prompt.json"
            for _ in range(20):
                if prompt_path.exists():
                    prompt_data = json.loads(prompt_path.read_text())
                    (ReYMeN_home / ".update_response").write_text("n")
                    return
                time.sleep(0.1)
    ...
    def test_empty_response_returns_default(self, tmp_path):
        ...
        ReYMeN_home = tmp_path / ".ReYMeN"
        ReYMeN_home.mkdir()
        (ReYMeN_home / ".update_response").write_text("")
...
class TestRestoreStashWithInputFn:
    ...
    def test_uses_input_fn_when_provided(self, tmp_path):
        ...
        from ReYMeN_cli.main import _restore_stashed_changes
...
class TestUpdateCommandGatewayFlag:
    """Verify the gateway spawns ReYMeN update --gateway."""
...
class TestWatchUpdateProgress:
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        async def simulate_prompt_cycle():
            ...
            (ReYMeN_home / ".update_response").write_text("y")
            (ReYMeN_home / ".update_prompt.json").unlink(missing_ok=True)
            await asyncio.sleep(0.3)
            (ReYMeN_home / ".update_exit_code").write_text("0")
    ...
    async def test_prompt_is_recovered_after_watcher_restart(self, tmp_path):
        ...
        with patch("gateway.run._ReYMeN_home", ReYMeN_home):
            ...
            assert (ReYMeN_home / ".update_prompt.json").exists()
...
class TestUpdatePromptInterception:
    ...
    async def test_intercepts_response_when_prompt_pending(self, tmp_path):
        ...
        (ReYMeN_home / ".update_prompt.json").write_text(json.dumps({"prompt": "test"}))
        ...
        response_path = ReYMeN_home / ".update_response"
