...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_config_reasoning_effort(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "ReYMeN_cli.tools_config._get_platform_tools", lambda *_: set()
        )
...
class TestModelsEndpoint:
    ...
    def test_resolve_model_name_default_profile(self):
        ...
        with patch(
            "ReYMeN_cli.profiles.get_active_profile_name", return_value="default"
        ):
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert (
                data["features"]["session_continuity_header"] == "X-ReYMeN-Session-Id"
            )
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stable_session_id_across_turns(self, adapter):
        ...
        assert session_ids[0].startswith(
            "api-"
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-ReYMeN-Session-Id": "my-session-123",
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-ReYMeN-Session-Id": "existing-session",
                        "Authorization": "Bearer sk-secret",
                    },
    ...
    async def test_db_failure_falls_back_to_empty_history(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(
                ...
                "ReYMeN_state.SessionDB", side_effect=Exception("DB unavailable")
            ):
                mock_run.return_value = (
                    mock_result,
                    {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0},
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-ReYMeN-Session-Id": "some-session",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={
                        "model": "ReYMeN-agent",
