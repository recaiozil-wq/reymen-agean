...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("ReYMeN_MANAGED", "homebrew")

        # Guard: prevent any accidental fall-through from spawning a real
        # `ReYMeN update --gateway` against the CI checkout. The managed-install
        ...
        assert "brew upgrade ReYMeN-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _ReYMeN_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._ReYMeN_home", tmp_path), patch(
            "gateway.run.Path"
        ) as MockPath:
        ...
        with patch("gateway.run._ReYMeN_home", tmp_path):
            ...
            # project_root will be the real ReYMeN-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_ReYMeN_binary(self, tmp_path):
        """Returns error when ReYMeN is not on PATH and ReYMeN_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "ReYMeN update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m ReYMeN_cli.main when ReYMeN not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "Starting ReYMeN update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m ReYMeN_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "ReYMeN_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_ReYMeN_bin_prefers_which(self, tmp_path):
        """_resolve_ReYMeN_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_ReYMeN_bin

        with patch("shutil.which", return_value="/custom/path/ReYMeN"):
            result = _resolve_ReYMeN_bin()

        assert result == ["/custom/path/ReYMeN"]

    @pytest.mark.asyncio
    async def test_resolve_ReYMeN_bin_fallback(self):
        """_resolve_ReYMeN_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_ReYMeN_bin
        ...
        assert result == [sys.executable, "-m", "ReYMeN_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_ReYMeN_bin_returns_none_when_both_fail(self):
        """_resolve_ReYMeN_bin returns None when both strategies fail."""
        from gateway.run import _resolve_ReYMeN_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._ReYMeN_home", ReYMeN_home), patch(
            ...
            "shutil.which",
            side_effect=lambda x: "/usr/bin/ReYMeN"
            if x == "ReYMeN"
            else "/usr/bin/setsid",
        ...
        assert not (ReYMeN_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((ReYMeN_home / ".update_pending.json").read_text())
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        with patch("gateway.run._ReYMeN_home", ReYMeN_home), patch(
            "gateway.run.__file__", fake_file
        ), patch("shutil.which", side_effect=which_no_setsid), patch(
            "subprocess.Popen", mock_popen
        ):
            result = await runner._handle_update_command(event)
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        with patch("gateway.run._ReYMeN_home", ReYMeN_home), patch(
            ...
            "subprocess.Popen", side_effect=OSError("spawn failed")
        ):
            result = await runner._handle_update_command(event)

        assert "Failed to start update" in result
        # Pending file should be cleaned up
        assert not (ReYMeN_home / ".update_pending.json").exists()
        assert not (ReYMeN_home / ".update_exit_code").exists()
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_blocks_programmatic_interface(self, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_MANAGED", "")
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        # Later steps may legitimately return success ("Starting ReYMeN
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (ReYMeN_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = ReYMeN_home / ".update_pending.claimed.json"
        ...
        (ReYMeN_home / ".update_output.txt").write_text("done")
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (ReYMeN_home / ".update_pending.json").write_text(json.dumps(pending))
        (ReYMeN_home / ".update_output.txt").write_text("x" * 5000)
        (ReYMeN_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (ReYMeN_home / ".update_pending.json").write_text(json.dumps(pending))
        (ReYMeN_home / ".update_output.txt").write_text("Traceback: boom")
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (ReYMeN_home / ".update_pending.claimed.json").exists()
