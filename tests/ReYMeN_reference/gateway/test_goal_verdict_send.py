...
def _make_runner_with_adapter(session_id: str = None):
    ...
    # module-import time, defeating per-test ReYMeN_HOME monkeypatches).
...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(ReYMeN_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(ReYMeN_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(ReYMeN_home):
    """When the budget is exhausted, a '⏸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from ReYMeN_cli.goals import GoalManager, save_goal
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(ReYMeN_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(ReYMeN_home):
    ...
    with patch("ReYMeN_cli.goals.judge_goal", return_value=("done", "ok", False)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
