"""Tests for ReYMeN-api-server toolset and API server tool availability."""
...
class TestReYMeNApiServerToolset:
    """Tests for the ReYMeN-api-server toolset definition."""

    def test_toolset_exists(self):
        ts = get_toolset("ReYMeN-api-server")
        assert ts is not None

    def test_toolset_validates(self):
        assert validate_toolset("ReYMeN-api-server")

    def test_toolset_includes_web_tools(self):
        tools = resolve_toolset("ReYMeN-api-server")
...
class TestApiServerPlatformConfig:
    def test_platforms_dict_includes_api_server(self):
        ...
        assert PLATFORMS["api_server"]["default_toolset"] == "ReYMeN-api-server"
...
class TestApiServerAdapterToolset:
    ...
    def test_create_agent_reads_config_toolsets(self):
        ...
        with patch("gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, patch(
            "gateway.run._resolve_gateway_model"
        ) as mock_model, patch(
            "gateway.run._load_gateway_config"
        ) as mock_config, patch("run_agent.AIAgent") as mock_agent_cls:
            ...
            # No platform_toolsets override — should fall back to ReYMeN-api-server default
