...
class TestSendVoiceReply:
    ...
    async def test_calls_tts_and_send_voice(self, runner):
        ...
        with patch(
            ...
            "tools.tts_tool._strip_markdown_for_tts", side_effect=lambda t: t
        ), patch("os.path.isfile", return_value=True), patch("os.unlink"), patch(
            "os.makedirs"
        ):
    ...
    async def test_empty_text_after_strip_skips(self, runner):
        ...
        with patch("tools.tts_tool.text_to_speech_tool") as mock_tts, patch(
            "tools.tts_tool._strip_markdown_for_tts", return_value=""
        ):
    ...
    async def test_tts_failure_no_crash(self, runner):
        ...
        with patch(
            ...
            "tools.tts_tool._strip_markdown_for_tts", side_effect=lambda t: t
        ), patch("os.path.isfile", return_value=False), patch("os.makedirs"):
...
class TestVoiceInHelp:
    ...
    def test_voice_is_known_command(self):
        ...
        from ReYMeN_cli.commands import GATEWAY_KNOWN_COMMANDS
...
class TestVoiceChannelCommands:
    ...
    async def test_join_success(self, runner):
        ...
        event.source.chat_name = "ReYMeN Server / #general"
        runner.adapters[event.source.platform] = mock_adapter
        result = await runner._handle_voice_channel_join(event)
    ...
    async def test_input_reuses_bound_source_metadata(self, runner):
        ...
        bound_source = SessionSource(
            chat_id="123",
            chat_name="ReYMeN Server / #general",
            chat_type="group",
            user_id="user1",
            user_name="user1",
        ...
        assert event.source.chat_name == "ReYMeN Server / #general"
...
class TestAutoTtsEmptyTextGuard:
    ...
    def test_base_empty_check_in_source(self):
        ...
        assert (
            "if not speech_text" in source or "not speech_text" in source
...
class TestPacketDebugCounterIsInstanceLevel:
    ...
    def test_counter_initialized_in_init(self):
        ...
        assert (
            "_packet_debug_count" in r.__dict__
...
class TestVoiceTimeoutCleansRunnerState:
    ...
    async def test_runner_cleanup_method_removes_voice_mode(self, tmp_path):
        ...
        assert (
            runner._voice_mode["discord:999"] == "off"
...
class TestPlaybackTimeout:
    ...
    async def test_playback_timeout_fires(self):
        ...
        try:
            with patch("discord.FFmpegPCMAudio"), patch(
                "discord.PCMVolumeTransformer", side_effect=lambda s, **kw: s
            ):
...
class TestSendVoiceReplyCleanup:
    ...
    async def test_files_cleaned_on_send_exception(self, tmp_path):
        ...
        fake_audio = tmp_path / "ReYMeN_voice"
        fake_audio.mkdir()
        ...
        ), patch("tools.tts_tool._strip_markdown_for_tts", return_value="hello"), patch(
...
class TestVoiceChannelAwareness:
    ...
    def _make_adapter(self):
        ...
        adapter._voice_receivers = {}
        adapter._client = MagicMock()
        adapter._client.user = SimpleNamespace(id=99999, name="ReYMeNBot")
        return adapter
    ...
    def test_returns_info_with_members(self):
        ...
        bot_member = self._make_member(99999, "ReYMeNBot", is_bot=True)
