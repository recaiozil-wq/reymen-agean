...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure ReYMeN_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def _make_provider(self, tmp_path, monkeypatch, api_key="rdb-test-key"):
        ...
        (tmp_path / ".ReYMeN").mkdir(exist_ok=True)
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", ReYMeN_home=str(tmp_path / ".ReYMeN"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", ReYMeN_home=profile_home)
        assert p._client.project == "ReYMeN-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".ReYMeN" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ReYMeN_HOME", str(ReYMeN_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", ReYMeN_home=str(ReYMeN_home))
        return p
    ...
    def test_thread_accumulation_guard(self, tmp_path, monkeypatch):
        ...
        with patch.object(
            p, "_prefetch_context", side_effect=lambda q: time.sleep(0.5)
        ), patch.object(
