...
_ISSUER = "https://auth.example.com/application/o/ReYMeN"
_CLIENT_ID = "ReYMeN-dashboard"
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://ReYMeN.example/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://ReYMeN.example/auth/callback"
    ...
    def test_two_calls_differ(self, provider):
        a = provider.start_login(redirect_uri="https://ReYMeN.example/auth/callback")
        b = provider.start_login(redirect_uri="https://ReYMeN.example/auth/callback")
        assert (
...
class TestPluginRegister:
    ...
    def clear_env(self, monkeypatch):
        for var in (
            "ReYMeN_DASHBOARD_OIDC_ISSUER",
            "ReYMeN_DASHBOARD_OIDC_CLIENT_ID",
            "ReYMeN_DASHBOARD_OIDC_SCOPES",
        ):
    ...
    def test_skips_when_unconfigured(self, patch_config):
        ...
        assert "ReYMeN_DASHBOARD_OIDC_ISSUER" in oidc_plugin.LAST_SKIP_REASON
        assert "self_hosted" in oidc_plugin.LAST_SKIP_REASON

    def test_skips_when_only_issuer_set(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("ReYMeN_DASHBOARD_OIDC_ISSUER", _ISSUER)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("ReYMeN_cli.config.load_config", _broken)
        ctx = MagicMock()
