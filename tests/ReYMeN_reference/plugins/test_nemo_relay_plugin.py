...
class _FakeAtofExporter:
    ...
    def register(self, name):
        self.events.append(
            ("atof.register", name, self.config.output_directory, self.config.filename)
        )
...
class _FakeAtifExporter:
    ...
    def export_json(self):
        return json.dumps(
            {"session_id": self.session_id, "agent_name": self.agent_name}
        )
...
def test_nemo_relay_plugin_emits_llm_tool_and_exports_atif(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "ReYMeN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "atof")
    )
    monkeypatch.setenv("ReYMeN_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv(
        "ReYMeN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif")
    )
...
def test_nemo_relay_plugin_metadata_promotes_trajectory_and_subagent_ids(monkeypatch):
    ...
    turn_mark = next(
        ...
        if event[0] == "scope.event" and event[1] == "ReYMeN.turn.start"
    )
    ...
    start_mark = next(
        event
        for event in fake.events
        if event[0] == "scope.event" and event[1] == "ReYMeN.subagent.start"
    )
    ...
    stop_mark = next(
        event
        for event in fake.events
        if event[0] == "scope.event" and event[1] == "ReYMeN.subagent.stop"
    )
...
""",
    ...
    monkeypatch.setenv("ReYMeN_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv(
        "ReYMeN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "direct-atif")
    )
...
""",
    ...
    monkeypatch.setenv("ReYMeN_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv(
        "ReYMeN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "direct-atof")
    )
