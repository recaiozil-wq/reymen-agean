...
def test_start_spawns_subprocess_and_writes_active_pointer(tmp_path):
    ...
    assert captured_env["ReYMeN_MEET_URL"] == "https://meet.google.com/abc-defg-hij"
    assert captured_env["ReYMeN_MEET_GUEST_NAME"] == "Test Bot"
    assert captured_env["ReYMeN_MEET_DURATION"] == "15m"
...
def test_transcript_reads_last_n_lines(tmp_path):
    ...
    meeting_dir = (
        Path(os.environ["ReYMeN_HOME"]) / "workspace" / "meetings" / "abc-defg-hij"
    )
...
def test_stop_signals_process_and_clears_pointer(tmp_path):
    ...
    with patch.object(pm, "_pid_alive", side_effect=_alive), patch.object(
        pm.os, "kill", side_effect=_kill
...
def test_on_session_end_stops_live_bot():
    ...
    with patch.object(
        pm, "status", return_value={"ok": True, "alive": True}
...
def test_start_passes_mode_into_active_record():
    ...
    with patch.object(
        pm.subprocess, "Popen", return_value=_FakeProc(12345)
...
def test_start_realtime_env_vars_threaded_through():
    ...
    with patch.object(pm.subprocess, "Popen", side_effect=_fake_popen), patch.object(
        pm, "_pid_alive", return_value=False
    ):
        pm.start(
            "https://meet.google.com/abc-defg-hij",
            mode="realtime",
    ...
    assert captured_env["ReYMeN_MEET_MODE"] == "realtime"
    assert captured_env["ReYMeN_MEET_REALTIME_MODEL"] == "gpt-realtime"
    assert captured_env["ReYMeN_MEET_REALTIME_VOICE"] == "alloy"
    assert captured_env["ReYMeN_MEET_REALTIME_INSTRUCTIONS"] == "Be brief."
    assert captured_env["ReYMeN_MEET_REALTIME_KEY"] == "sk-test"
...
def test_meet_join_accepts_realtime_mode():
    ...
    with patch(
        "plugins.google_meet.tools.check_meet_requirements", return_value=True
    ), patch(
...
def test_cli_register_includes_node_subcommand():
    """`ReYMeN meet` argparse tree includes the node subtree."""
    import argparse
    from plugins.google_meet.cli import register_cli

    parser = argparse.ArgumentParser(prog="ReYMeN meet")
    register_cli(parser)
...
def test_looks_like_human_speaker():
    ...
    for s in (
        ...
        assert not _looks_like_human_speaker(
            s, "ReYMeN Agent"
        ), f"{s!r} should NOT be human"
    # Real names → human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "ReYMeN Agent"), f"{s!r} SHOULD be human"
