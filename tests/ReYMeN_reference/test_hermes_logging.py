"""Tests for ReYMeN_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    ReYMeN_logging._logging_initialized = False
    ...
    ReYMeN_logging._install_session_record_factory()
    yield
    ...
    ReYMeN_logging._logging_initialized = False
    ReYMeN_logging.clear_session_context()


@pytest.fixture
def ReYMeN_home(tmp_path, monkeypatch):
    """Provide an isolated ReYMeN_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_ReYMeN_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["ReYMeN_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, ReYMeN_home):
        log_dir = ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        assert log_dir == ReYMeN_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, force=True)
    ...
    def test_custom_log_level(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(
            ReYMeN_home=ReYMeN_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
    ...
    def test_writes_to_agent_log(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)

        test_logger = logging.getLogger("test_ReYMeN_logging.write_test")
        ...
        agent_log = ReYMeN_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)

        test_logger = logging.getLogger("test_ReYMeN_logging.warning_test")
        ...
        agent_log = ReYMeN_home / "logs" / "agent.log"
        errors_log = ReYMeN_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)

        test_logger = logging.getLogger("test_ReYMeN_logging.info_test")
    ...
    def test_reads_config_yaml(self, ReYMeN_home):
    ...
    def test_explicit_params_override_config(self, ReYMeN_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (ReYMeN_home / "config.yaml").write_text(yaml.dump(config))

        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, ReYMeN_home):
        """The custom record factory injects session_tag on all records."""
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_ReYMeN_session_injector", False), (
            "Record factory should have _ReYMeN_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, ReYMeN_home):
        ...
        gw_log = ReYMeN_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, ReYMeN_home):
    ...
    def test_gateway_log_receives_gateway_records(self, ReYMeN_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, ReYMeN_home):
    ...
    def test_agent_log_still_receives_all(self, ReYMeN_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, mode="cli")
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, mode="gui")

        logging.getLogger("ReYMeN_cli.web_server").info("dashboard online")
        ...
        gui_log = ReYMeN_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, ReYMeN_home):
        """When session context is set, log lines include [session_id]."""
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        ReYMeN_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, ReYMeN_home):
    ...
    def test_clear_session_context(self, ReYMeN_home):
        """After clearing, session tag disappears."""
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        ReYMeN_logging.set_session_context("xyz789")
        ReYMeN_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, ReYMeN_home):
        ...
        def thread_a():
            ReYMeN_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            ReYMeN_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        ReYMeN_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        ReYMeN_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = ReYMeN_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = ReYMeN_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in ReYMeN_logging.COMPONENT_PREFIXES
        # The gateway component captures both core gateway logs and the
        # ReYMeN_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "ReYMeN_plugins") == ReYMeN_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = ReYMeN_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == ReYMeN_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = ReYMeN_logging.COMPONENT_PREFIXES["cli"]
        assert "ReYMeN_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == ReYMeN_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = ReYMeN_logging.COMPONENT_PREFIXES["gui"]
        assert "ReYMeN_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        ReYMeN_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_ReYMeN_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, ReYMeN_home):
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home)
        ReYMeN_logging.setup_verbose_logging()
        ReYMeN_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        ReYMeN_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = ReYMeN_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        ReYMeN_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("ReYMeN_cli.config.is_managed", return_value=True):
                ReYMeN_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, ReYMeN_home):
        level, max_size, backup = ReYMeN_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, ReYMeN_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, ReYMeN_home):
        import yaml
        config = {"model": "test"}
        (ReYMeN_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> ReYMeN_logging._ManagedRotatingFileHandler:
        handler = ReYMeN_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that ReYMeN_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        ReYMeN_logging.setup_logging(ReYMeN_home=ReYMeN_home, mode="gateway")
        gw_path = ReYMeN_home / "logs" / "gateway.log"
        rotated = ReYMeN_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = ReYMeN_logging._safe_stderr()
