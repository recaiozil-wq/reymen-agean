...
class TestHonchoSession:
    def _make_session(self):
        return HonchoSession(
            ...
            assistant_peer_id="ReYMeN-assistant",
...
class TestToolsModeInitBehavior:
    ...
    def _make_provider_with_config(
        ...
        with patch(
            ...
            "ReYMeN_constants.get_reymen_home", return_value=MagicMock()
        ):
...
class TestPerSessionMigrateGuard:
    ...
    per-session creates a fresh Honcho session every ReYMeN run. Uploading
    ...
    def _make_provider_with_strategy(self, strategy, init_on_session_start=True):
        ...
        with patch(
            ...
            "plugins.memory.honcho.session.HonchoSessionManager",
            return_value=mock_manager,
        ), patch("ReYMeN_constants.get_reymen_home", return_value=MagicMock()):
...
class TestTrivialPromptHeuristic:
    ...
    def test_classifier_catches_common_trivial_forms(self):
        for t in (
            ...
            assert HonchoMemoryProvider._is_trivial_prompt(
                t
    ...
    def test_classifier_lets_substantive_prompts_through(self):
        for t in ("hello world", "what's my name", "explain this", "ok so what's next"):
            assert not HonchoMemoryProvider._is_trivial_prompt(
                t
...
class TestDialecticLifecycleSmoke:
    ...
    def test_full_multi_turn_session(self):
        ...
        responses = iter(
            [
                "prewarm: user is eri, works on ReYMeN",  # session-start prewarm
        ...
        with provider._prefetch_lock:
            assert provider._prefetch_result.startswith(
                "prewarm"
        ...
        self._await_thread(provider)
        assert (
            mgr.dialectic_query.call_count == 1
        ...
        assert (
            mgr.prefetch_context.call_count == 0
        ...
        assert (
            kwargs.get("reasoning_level") in {"medium", "high"}
        ...
        for t in (5, 6):
            ...
            self._await_thread(provider)
        assert (
            mgr.dialectic_query.call_count == 2
        ...
        assert (
            provider._last_dialectic_turn == 4
        ...
        self._await_thread(provider)
        assert (
            mgr.dialectic_query.call_count == 4
