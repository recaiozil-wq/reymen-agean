...
- Path resolution (absolute, relative to ReYMeN_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", ReYMeN_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.ReYMeN/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        for key in (
            ...
            "ReYMeN_SESSION_CHAT_ID",
            "ReYMeN_SESSION_CHAT_NAME",
        ):
        ...
        assert os.environ.get("ReYMeN_SESSION_PLATFORM") is None
        assert os.environ.get("ReYMeN_SESSION_CHAT_ID") is None
        assert os.environ.get("ReYMeN_SESSION_CHAT_NAME") is None
