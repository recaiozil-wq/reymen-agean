...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._ReYMeN_home", tmp_path), patch(
            "cron.scheduler._resolve_origin", return_value=None
        ), patch("dotenv.load_dotenv"), patch(
            "ReYMeN_state.SessionDB", return_value=fake_db
        ), patch(
    ...
    def test_run_job_reaps_stale_auxiliary_clients_per_tick(self, tmp_path):
        ...
        ), patch("run_agent.AIAgent") as mock_agent_cls, patch(
    ...
    def _make_run_job_patches(self, tmp_path):
        ...
        fake_db = MagicMock()
        return fake_db, [
            patch("cron.scheduler._ReYMeN_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            patch("dotenv.load_dotenv"),
            patch("ReYMeN_state.SessionDB", return_value=fake_db),
            patch(
                "ReYMeN_cli.runtime_provider.resolve_runtime_provider",
                return_value={
                    "api_key": "test-key",
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(
        ...
        resolves them from ``ReYMeN tools`` platform config for ``cron``
        ...
        # The cron default is _ReYMeN_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``ReYMeN tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        fake_db, patches = self._make_run_job_patches(tmp_path)
        # Even if the user has ``ReYMeN tools`` configured to enable web+file
    ...
    def test_tick_marks_empty_response_as_error(self, tmp_path):
        ...
        ), patch("cron.scheduler._resolve_origin", return_value=None), patch(
            "cron.scheduler.run_job", return_value=(True, "output", "", None)
        ):
            tick(verbose=False)
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("ReYMeN_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("ReYMeN_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("ReYMeN_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = (
                    get_session_env("ReYMeN_CRON_AUTO_DELIVER_PLATFORM") or None
                )
                seen["chat_id"] = (
                    get_session_env("ReYMeN_CRON_AUTO_DELIVER_CHAT_ID") or None
                )
                seen["thread_id"] = (
                    get_session_env("ReYMeN_CRON_AUTO_DELIVER_THREAD_ID") or None
                )
        ...
        assert os.getenv("ReYMeN_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("ReYMeN_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("ReYMeN_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("ReYMeN_CRON_AUTO_DELIVER_PLATFORM")
                        or None,
                        "chat_id": get_session_env("ReYMeN_CRON_AUTO_DELIVER_CHAT_ID")
                        or None,
...
class TestRunJobConfigLogging:
    ...
    def test_bad_config_yaml_is_logged(self, caplog, tmp_path):
        ...
        assert any(
            "failed to load config.yaml" in r.message for r in caplog.records
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_ReYMeN_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_ReYMeN_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(
        ...
        (tmp_path / "config.yaml").write_text(
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_ReYMeN_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_ReYMeN_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_ReYMeN_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_ReYMeN_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_ReYMeN_TEST_CRON_UNSET_VAR}"
...
class TestRunJobSkillBacked:
    def test_run_job_preserves_skill_env_passthrough_into_worker_thread(self, tmp_path):
        ...
        ), patch("tools.skills_tool.skill_view", side_effect=_skill_view), patch(
            "run_agent.AIAgent"
        ) as mock_agent_cls:
            mock_agent = MagicMock()
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(
        ...
        def _skill_view(name):
            ...
            return json.dumps(
                {"success": True, "content": "# google-workspace\nUse Google."}
            )
        ...
        with patch("cron.scheduler._ReYMeN_home", tmp_path), patch(
            ...
            "tools.credential_files._resolve_ReYMeN_home", return_value=tmp_path
        ), patch("dotenv.load_dotenv"), patch(
            "ReYMeN_state.SessionDB", return_value=fake_db
        ), patch(
            "ReYMeN_cli.runtime_provider.resolve_runtime_provider",
    ...
    def test_run_job_loads_multiple_skills_in_order(self, tmp_path):
        ...
        ) as skill_view_mock, patch("run_agent.AIAgent") as mock_agent_cls:
            mock_agent = MagicMock()
            mock_agent.run_conversation.return_value = {"final_response": "ok"}
            mock_agent_cls.return_value = mock_agent
...
class TestSilentDelivery:
    ...
    def test_silent_response_suppresses_delivery(self, caplog):
        with patch(
            "cron.scheduler.get_due_jobs", return_value=[self._make_job()]
        ), patch(
            "cron.scheduler.run_job", return_value=(True, "# output", "[SILENT]", None)
        ), patch("cron.scheduler.save_job_output", return_value="/tmp/out.md"), patch(
            "cron.scheduler._deliver_result"
        ) as deliver_mock, patch("cron.scheduler.mark_job_run"):
            from cron.scheduler import tick
    ...
    def test_silent_trailing_suppresses_delivery(self):
        ...
        with patch(
            ...
            "cron.scheduler.run_job", return_value=(True, "# output", response, None)
        ), patch("cron.scheduler.save_job_output", return_value="/tmp/out.md"), patch(
            "cron.scheduler._deliver_result"
        ) as deliver_mock, patch("cron.scheduler.mark_job_run"):
            from cron.scheduler import tick
    ...
    def test_failed_job_always_delivers(self):
        ...
        with patch(
            ...
            "cron.scheduler.run_job", return_value=(False, "# output", "", "some error")
        ), patch("cron.scheduler.save_job_output", return_value="/tmp/out.md"), patch(
            "cron.scheduler._deliver_result"
        ) as deliver_mock, patch("cron.scheduler.mark_job_run"):
            from cron.scheduler import tick
    ...
    def test_output_saved_even_when_delivery_suppressed(self):
        ...
        ), patch("cron.scheduler.save_job_output") as save_mock, patch(
            "cron.scheduler._deliver_result"
        ) as deliver_mock, patch("cron.scheduler.mark_job_run"):
    ...
    def test_whitespace_only_response_is_marked_failed_not_delivered(self):
        ...
        with patch(
            ...
            "cron.scheduler.run_job", return_value=(True, "# output", "   \n\t  ", None)
        ), patch("cron.scheduler.save_job_output", return_value="/tmp/out.md"), patch(
            "cron.scheduler._deliver_result"
...
class TestRunJobWakeGate:
    ...
    def test_script_runs_only_once_on_wake(self):
        ...
        with patch.object(
            scheduler, "_run_job_script", side_effect=_script_stub
    ...
    def test_script_failure_does_not_trigger_gate(self):
        ...
        with patch.object(
            scheduler, "_run_job_script", return_value=(False, '{"wakeAgent": false}')
        ), patch("run_agent.AIAgent", return_value=agent) as agent_cls:
            success, doc, final, err = scheduler.run_job(self._make_job())
    ...
    def test_no_script_path_runs_agent_normally(self):
        ...
        job.pop("script", None)
        with patch.object(scheduler, "_run_job_script") as script_fn, patch(
...
class TestBuildJobPromptBumpUse:
    ...
    def test_bump_use_called_for_loaded_skill(self):
        ...
        with patch("tools.skills_tool.skill_view", side_effect=_skill_view), patch(
    ...
    def test_bump_use_not_called_for_missing_skill(self):
        ...
        with patch("tools.skills_tool.skill_view", side_effect=_missing_view), patch(
            "tools.skill_usage.bump_use"
        ) as mock_bump:
    ...
    def test_bump_failure_does_not_break_prompt(self, caplog):
        ...
        with patch("tools.skills_tool.skill_view", side_effect=_skill_view), patch(
            "tools.skill_usage.bump_use", side_effect=RuntimeError("boom")
...
class TestParallelTick:
    ...
    def test_parallel_jobs_run_concurrently(self):
        ...
        with patch("cron.scheduler.get_due_jobs", return_value=jobs), patch(
            "cron.scheduler.advance_next_run"
        ), patch("cron.scheduler.run_job", side_effect=mock_run_job), patch(
            "cron.scheduler.save_job_output", return_value="/tmp/out.md"
    ...
    def test_parallel_jobs_isolated_contextvars(self):
        ...
        def mock_run_job(job):
            ...
            chat_id = get_session_env("ReYMeN_SESSION_CHAT_ID")
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """ReYMeN_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("ReYMeN_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestDeliverResultTimeoutCancelsFuture:
    ...
    def test_live_adapter_timeout_cancels_future_and_falls_back(self):
        ...
        with patch("gateway.config.load_gateway_config", return_value=mock_cfg), patch(
            "cron.scheduler.load_config",
            return_value={"cron": {"wrap_response": False}},
        ), patch("asyncio.run_coroutine_threadsafe", side_effect=fake_run_coro), patch(
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``ReYMeN cron create --deliver=<platform>`` silently
