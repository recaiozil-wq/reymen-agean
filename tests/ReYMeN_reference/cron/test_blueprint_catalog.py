...
the shared /blueprint command handler (ReYMeN_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated ReYMeN_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("ReYMeN://blueprint/morning-brief?")
    ...
    def test_catalog_entry_has_all_surfaces(self):
        ...
        assert entry["appUrl"].startswith("ReYMeN://")
...
class TestCommandHandler:
    def test_bare_lists_catalog(self, isolated_home):
        from ReYMeN_cli.blueprint_cmd import handle_blueprint_command
    ...
    def test_name_match_is_forgiving(self, isolated_home):
        from ReYMeN_cli.blueprint_cmd import handle_blueprint_command, match_blueprint
