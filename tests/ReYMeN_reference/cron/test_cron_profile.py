...
ReYMeN_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated ReYMeN root with a named profile and temp cron store."""
    root = tmp_path / "ReYMeN-root"
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "ReYMeN profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets ReYMeN_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        from ReYMeN_cli import runtime_provider as runtime_provider
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["ReYMeN_home_during_run"] == str(profile_home.resolve())
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("ReYMeN_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("ReYMeN_PROFILE_TEST_ONLY", raising=False)

        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["ReYMeN_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["ReYMeN_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["ReYMeN_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["ReYMeN_PROFILE_TEST_SHARED"] == "outer"
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('ReYMeN_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_ReYMeN_home", None)
    ...
    def test_run_job_without_profile_leaves_ReYMeN_home_untouched(
        ...
        assert observed["ReYMeN_home_during_init"] == str(root)
        assert os.environ["ReYMeN_HOME"] == str(root)
