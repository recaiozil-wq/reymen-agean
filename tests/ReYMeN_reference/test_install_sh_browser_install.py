...
Browser automation is optional. The installer should not leave ReYMeN
...
def test_install_script_skips_playwright_download_when_system_browser_exists() -> None:
    ...
    assert (
        "Skipping Playwright browser download; ReYMeN will use the system browser."
        in text
    )
...
def test_playwright_installs_are_timeout_guarded() -> None:
    ...
    assert (
        "run_browser_install_with_timeout 600 npx playwright install chromium" in text
    )
