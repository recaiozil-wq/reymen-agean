...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("ReYMeN/ReYMeN-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned ReYMeN/* and pr-* branches."""

    def test_prunes_orphaned_ReYMeN_branch(self, git_repo):
        """ReYMeN/ReYMeN-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "ReYMeN/ReYMeN-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "ReYMeN/ReYMeN-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "ReYMeN/ReYMeN-deadbeef" in result.stdout
        ...
        assert "ReYMeN/ReYMeN-deadbeef" in orphaned
        ...
        assert "ReYMeN/ReYMeN-deadbeef" not in result.stdout
