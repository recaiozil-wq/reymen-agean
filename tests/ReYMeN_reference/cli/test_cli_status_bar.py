...
class TestCLIStatusBar:
    ...
    def test_scrollback_box_width_returns_viewport_width(self):
        ...
        assert ReYMeNCLI._scrollback_box_width(20) == 32
        assert ReYMeNCLI._scrollback_box_width(32) == 32
        # Above the floor, return the actual viewport width — no cap.
        assert ReYMeNCLI._scrollback_box_width(48) == 48
        assert ReYMeNCLI._scrollback_box_width(80) == 80
        assert ReYMeNCLI._scrollback_box_width(120) == 120
        assert ReYMeNCLI._scrollback_box_width(200) == 200
...
class TestStatusBarWidthSource:
    ...
    def test_fragments_use_pt_width_over_shutil(self):
        ...
        ) as mock_get_app, patch("shutil.get_terminal_size") as mock_shutil:
    ...
    def test_build_status_bar_text_uses_pt_width(self):
        ...
        with patch("prompt_toolkit.application.get_app", return_value=mock_app), patch(
    ...
    def test_explicit_width_skips_pt_lookup(self):
        ...
        with patch("prompt_toolkit.application.get_app") as mock_get_app, patch(
            "shutil.get_terminal_size"
        ) as mock_shutil:
            text = cli_obj._build_status_bar_text(width=100)
...
class TestIdleSinceLastTurn:
    ...
    def test_hidden_before_first_turn(self):
        assert ReYMeNCLI._format_idle_since(None, turn_live=False) == ""

    def test_hidden_while_turn_is_live(self):
        assert ReYMeNCLI._format_idle_since(time.time() - 30, turn_live=True) == ""

    def test_shows_compact_idle_time_after_turn(self):
        label = ReYMeNCLI._format_idle_since(time.time() - 42, turn_live=False)
        assert label.startswith("✓ ")
        assert label == "✓ 42s"

    def test_scales_to_minutes(self):
        label = ReYMeNCLI._format_idle_since(time.time() - 3 * 60, turn_live=False)
        assert label == "✓ 3m"
