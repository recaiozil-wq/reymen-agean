...
Regression: the old implementation wrote ``ReYMeN_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``ReYMeN sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.ReYMeN/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_ReYMeN_home(
    ...
    cwd_leak = list(work.glob("ReYMeN_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.ReYMeN/sessions/saved/
    saved_dir = ReYMeN_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("ReYMeN_conversation_*.json"))
    ...
    assert "ReYMeN --resume 20260101_120000_abc123" in out, out
