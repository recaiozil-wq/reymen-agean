...
def test_gquota_uses_chat_console_when_tui_is_live():
    ...
    with patch("cli.ChatConsole", return_value=live_console), patch(
        "agent.google_oauth.get_valid_access_token",
        side_effect=GoogleOAuthError("No Google OAuth credentials found"),
    ), patch("agent.google_oauth.load_credentials", return_value=None), patch(
