...
Drives ``ReYMeNCLI.process_command("/reset now")`` against a minimal stand-in
and verifies:
...
def _make_cli_stub():
    """Build a minimal ReYMeNCLI-shaped object that can run ``process_command``
    ...
    self_.process_command = ReYMeNCLI.process_command.__get__(self_, type(self_))
