...
class TestCliApprovalUi:
    ...
    def test_background_task_registers_thread_local_approval_callbacks(self):
        ...
        with patch.object(cli_module, "AIAgent", FakeAgent), patch.object(
            cli_module, "_cprint"
        ...
        assert seen["approval"].__func__ is ReYMeNCLI._approval_callback
        assert seen["sudo"].__self__ is cli
        assert seen["sudo"].__func__ is ReYMeNCLI._sudo_password_callback
...
def _make_real_paint_cli_stub():
    ...
    cli._paint_now = ReYMeNCLI._paint_now.__get__(cli, ReYMeNCLI)
    cli._invalidate = ReYMeNCLI._invalidate.__get__(cli, ReYMeNCLI)
