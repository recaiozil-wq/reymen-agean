"""Tests for cli.ReYMeNCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full ReYMeNCLI (which requires extensive setup).
"""
...
def test_slash_confirm_modal_number_selection_submits_without_raw_input():
    ...
    _bound(ReYMeNCLI._submit_slash_confirm_response, self_)("always")
...
def test_slash_confirm_display_fragments_include_choice_mapping():
    ...
    fragments = _bound(ReYMeNCLI._get_slash_confirm_display_fragments, self_)()
...
def test_split_destructive_skip_recognized_tokens():
    ...
    assert ReYMeNCLI._split_destructive_skip("/reset now") == ("", True)
    assert ReYMeNCLI._split_destructive_skip("/clear --yes") == ("", True)
    assert ReYMeNCLI._split_destructive_skip("/undo -y") == ("", True)
...
def test_split_destructive_skip_case_insensitive():
    ...
    assert ReYMeNCLI._split_destructive_skip("/new NOW") == ("", True)
    # Substring match must NOT trigger — "Now-Title" is a literal title token.
    assert ReYMeNCLI._split_destructive_skip("/new Now-Title") == ("Now-Title", False)
...
def test_split_destructive_skip_handles_empty_and_none():
    ...
    assert ReYMeNCLI._split_destructive_skip(None) == ("", False)
    assert ReYMeNCLI._split_destructive_skip("") == ("", False)
    assert ReYMeNCLI._split_destructive_skip("   ") == ("", False)
