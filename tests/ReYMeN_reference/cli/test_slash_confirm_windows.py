...
def _make_cli():
    """Minimal ReYMeNCLI shell exposing the prompt/modal helpers."""
...
def _run_on_daemon(call, cli, *, platform, response, schedule=None):
    ...
    def _worker():
        try:
            ...
            ), patch.object(cli, "_prompt_text_input") as mock_stdin, patch.object(
                cli, "_invalidate"
            ), patch.object(
                cli,
...
class TestModal:
    ...
    def test_main_thread_with_app_uses_modal(self):
        ...
        ), patch.object(cli, "_restore_modal_input_snapshot"), patch.object(
            cli, "_invalidate"
    ...
    def test_daemon_thread_no_app_loop_uses_fallback(
        ...
        def _worker():
            try:
                with patch.object(sys, "platform", platform), patch.object(
                    cli, "_prompt_text_input", return_value="1"
                ) as mock_stdin, patch.object(cli, "_invalidate"):
...
class TestNativeWindowsNoRawInputDeadlock:
        ...
        def _worker():
            try:
                with patch.object(sys, "platform", "win32"), patch(
                    "builtins.input", side_effect=_blocking_input
                ), patch.object(cli, "_capture_modal_input_snapshot"), patch.object(
    ...
    def test_win32_scheduling_failure_cleanly_cancels_no_input(self):
        ...
        def _worker():
            with patch.object(sys, "platform", "win32"), patch(
                "builtins.input", side_effect=_tracking_input
            ), patch.object(cli, "_invalidate"):
                outcome["result"] = cli._prompt_text_input_modal(
                    title="/new",
                    detail="destroys conversation state",
