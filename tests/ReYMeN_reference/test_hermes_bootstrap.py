"""Tests for ReYMeN_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every ReYMeN entry point
(ReYMeN, ReYMeN-agent, ReYMeN-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every ReYMeN entry point imports ReYMeN_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported ReYMeN_bootstrap module.
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the ReYMeN CLI
...
class TestEntryPointsImportBootstrap:
    """Every ReYMeN entry point must import ReYMeN_bootstrap as its
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import ReYMeN_bootstrap' and that
        ...
        half-finished ``ReYMeN update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``ReYMeN_bootstrap`` as a top-level module) leaves ReYMeN
        ...
        assert first_import_name == "ReYMeN_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'ReYMeN_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import ReYMeN_bootstrap' line to be the first import."
        )
