...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``ReYMeN_cli``) for the package
    itself and ``ReYMeN_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``ReYMeN_cli`` without ``ReYMeN_cli.*``,
    so the wheel shipped ``ReYMeN_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'ReYMeN_cli.dashboard_auth'``.
...
def test_packaging_declared_as_core_dependency():
    ...
    ReYMeN_cli/main.py) yet was undeclared, so it only reached users
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (ReYMeN_cli/plugins.py) registers each bundled plugin by
    ...
    manifest = (REPO_ROOT / "MANIFEST.in").read_text(encoding="utf-8")
    assert (
        "recursive-include plugins" in manifest and "plugin.yaml" in manifest
...
def test_optional_mcps_manifests_ship_in_both_wheel_and_sdist():
    ...
    ReYMeN_cli/mcp_catalog.py resolves the catalog via get_optional_mcps_dir()
    ...
    `ReYMeN mcp catalog` and the dashboard catalog screen come up empty on
    ...
    manifest = (REPO_ROOT / "MANIFEST.in").read_text(encoding="utf-8")
    assert (
        "graft optional-mcps" in manifest
