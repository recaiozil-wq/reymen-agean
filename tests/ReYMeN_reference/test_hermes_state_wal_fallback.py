...
(SQLITE_PROTOCOL — typical on NFS/SMB), ReYMeN must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``ReYMeN_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    ReYMeN_state._set_last_init_error(None)
    yield
    ReYMeN_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    ReYMeN_state._wal_fallback_warned_paths.clear()
    yield
    ReYMeN_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see ReYMeN_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        ReYMeN_state._set_last_init_error("OperationalError: locking protocol")
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        ReYMeN_state._set_last_init_error("OperationalError: generic SQLite error")
