...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `ReYMeN_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("ReYMeN_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("ReYMeN_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    assert result["name"] == "ReYMeN-agent-dev"
