    ...
    with patch.dict(
        "sys.modules",
        {
            "ReYMeN_constants": MagicMock(
                get_reymen_home=MagicMock(
                    return_value="/tmp/ReYMeN_test_review_summary"
                )
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'ReYMeN-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'ReYMeN-release' patched"
    }
