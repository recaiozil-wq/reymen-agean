...
def test_main_version_prints_without_starting_server(monkeypatch, capsys):
    ...
    assert output
    assert "Starting ReYMeN-agent ACP adapter" not in output
...
def test_main_check_prints_ok_without_starting_server(monkeypatch, capsys):
    ...
    assert capsys.readouterr().out.strip() == "ReYMeN ACP check OK"


def test_main_setup_runs_model_configuration(monkeypatch):
    calls = {}

    def fake_ReYMeN_main():
        calls["argv"] = sys.argv[:]

    monkeypatch.setattr("ReYMeN_cli.main.main", fake_ReYMeN_main)
...
def test_main_setup_offers_browser_install_when_tty(monkeypatch):
    ...
    monkeypatch.setattr("ReYMeN_cli.main.main", lambda: None)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    """`ReYMeN-acp --setup-browser` routes through dep_ensure.ensure_dependency."""
    calls = []
    ...
    monkeypatch.setattr("ReYMeN_cli.dep_ensure.ensure_dependency", fake_ensure)
