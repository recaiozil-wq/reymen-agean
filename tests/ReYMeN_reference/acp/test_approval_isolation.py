...
- 96vc: ACP didn't set ReYMeN_EXEC_ASK, so `check_all_command_guards`
...
1. Setting ReYMeN_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds ReYMeN_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set ReYMeN_INTERACTIVE so
    ...
    (ReYMeN_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When ReYMeN_INTERACTIVE is set and an approval callback is
        ...
        # Without ReYMeN_INTERACTIVE: takes auto-approve path, callback NOT called
        ...
        assert called_with == [], (
            "without ReYMeN_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With ReYMeN_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with ReYMeN_INTERACTIVE the approval path should consult the "
