...
class TestStepCallback:
    ...
    def test_result_passed_to_build_tool_complete(self, mock_conn, event_loop_fixture):
        ...
        ) as mock_rcts, patch("acp_adapter.events.build_tool_complete") as mock_btc:
            future = MagicMock(spec=Future)
            future.result.return_value = None
            mock_rcts.return_value = future
    ...
    def test_tool_progress_captures_snapshot_metadata(
        ...
        with patch(
            "acp_adapter.events.make_tool_call_id", return_value="tc-meta"
        ), patch("acp_adapter.events._send_update") as mock_send, patch(
            "agent.display.capture_local_edit_snapshot", return_value="snapshot"
        ):
            cb = make_tool_progress_cb(
                mock_conn, "session-1", loop, tool_call_ids, tool_call_meta
            )
