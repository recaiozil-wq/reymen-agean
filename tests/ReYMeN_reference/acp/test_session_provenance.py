...
def test_root_session_no_compression(db):
    ...
    assert prov["currentReYMeNSessionId"] == "root1"
    assert prov["rootReYMeNSessionId"] == "root1"
    assert prov["parentReYMeNSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_ReYMeN_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentReYMeNSessionId"] == "old"
    assert prov["rootReYMeNSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousReYMeNSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootReYMeNSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootReYMeNSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"ReYMeN"}
    assert "sessionProvenance" in meta["ReYMeN"]
    assert meta["ReYMeN"]["sessionProvenance"]["currentReYMeNSessionId"] == "root1"
