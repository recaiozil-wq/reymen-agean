...
def test_main_wrapper_preserves_docker_workdir() -> None:
    ...
    # first exec / if-block.
    activate_idx = text.index("/opt/ReYMeN/.venv/bin/activate")
    restore_idx = text.index('cd "$_ReYMeN_orig_cwd"')
    exec_idx = text.index("if [ $# -eq 0 ]")
    assert activate_idx < restore_idx < exec_idx, (
        "cd $_ReYMeN_orig_cwd must appear after venv activation and "
...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid ReYMeN ReYMeN dashboard" in text
...
def test_dashboard_run_does_not_derive_insecure_from_bind_host() -> None:
    ...
    ``ReYMeN_DASHBOARD_HOST``. Doing so disables the OAuth auth gate on
    ...
    The opt-in is now explicit: ``ReYMeN_DASHBOARD_INSECURE=1`` (truthy).
...
def test_stage2_hook_repairs_profiles_and_cron_ownership_on_every_boot() -> None:
    ...
    assert 'if [ -d "$ReYMeN_HOME/profiles" ]; then' in text
    assert 'chown -R ReYMeN:ReYMeN "$ReYMeN_HOME/profiles" 2>/dev/null || true' in text

    assert 'if [ -d "$ReYMeN_HOME/cron" ]; then' in text
    assert 'chown -R ReYMeN:ReYMeN "$ReYMeN_HOME/cron" 2>/dev/null || true' in text
