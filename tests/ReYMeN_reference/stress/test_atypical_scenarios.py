...
normal tests assume away:
  ...
  - Filesystem: ReYMeN_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own ReYMeN_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            os.environ["ReYMeN_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("ReYMeN_cli", "plugins", "gateway")):
                    del sys.modules[m]
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "ReYMeN_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    """ReYMeN_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="ReYMeN with spaces ")
    os.environ["ReYMeN_HOME"] = weird
    ...
    try:
        ...
        print(f"  ReYMeN_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("ReYMeN_home_with_unicode")
def _(home, kb):
    """ReYMeN_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/ReYMeN_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  ReYMeN_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("ReYMeN_home_via_symlink")
def _(home, kb):
    """ReYMeN_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="ReYMeN_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["ReYMeN_HOME"] = link1
...
def _(home, kb):
    ...
    try:
        from ReYMeN_cli import web_server as ws  # noqa
    except Exception:
        pass
