...
def worker_loop(worker_id: int, ReYMeN_home: str, result_file: str) -> None:
    ...
    while time.monotonic() - start < 40:
        ...
        try:
            ...
            try:
                ...
                events.append(
                    {"kind": "sqlite_err", "op": "claim", "err": str(e)[:100]}
                )
                continue
            if claimed is None:
            ...
            events.append(
                {"kind": "claimed", "task": tid, "worker": worker_id, "run_id": run.id}
            )
            ...
            try:
                ...
                events.append(
                    {"kind": "sqlite_err", "op": "complete", "err": str(e)[:100]}
                )
        finally:
            conn.close()
...
def reclaimer_loop(ReYMeN_home: str, result_file: str) -> None:
    os.environ["ReYMeN_HOME"] = ReYMeN_home
    os.environ["HOME"] = ReYMeN_home
    sys.path.insert(0, WT)
    from ReYMeN_cli import kanban_db as kb
    ...
    while time.monotonic() - start < 42:
        ...
        try:
            try:
                ...
                if n:
                    events.append(
                        {"kind": "reclaimed", "count": n, "t": time.monotonic() - start}
                    )
            except sqlite3.OperationalError as e:
...
def main():
    home = tempfile.mkdtemp(prefix="ReYMeN_reclaim_race_")
