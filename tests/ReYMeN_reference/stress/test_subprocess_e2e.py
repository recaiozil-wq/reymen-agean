...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves ReYMeN_cli.kanban_db on its own
  ...
  - worker logs are captured to ReYMeN_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "ReYMeN_KANBAN_TASK": task.id,
            "ReYMeN_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="ReYMeN_e2e_")
    os.environ["ReYMeN_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from ReYMeN_cli import kanban_db as kb

    # Point the `ReYMeN` CLI child processes will run at the worktree
    # ReYMeN_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "ReYMeN")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m ReYMeN_cli.main "$@"
""")
