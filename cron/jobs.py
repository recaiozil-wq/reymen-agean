...
# POSIX file locking (cross-process advisory lock).
# None on Windows and other non-POSIX platforms.
try:
...
ReYMeN_DIR = get_ReYMeN_home().resolve()
CRON_DIR = ReYMeN_DIR / "cron"
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _ReYMeN_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _ReYMeN_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _normalize_profile(profile: Optional[str]) -> Optional[str]:
    ...
    is canonicalized with the same rules as ``ReYMeN -p`` and must refer to an
    ...
    # so profile directories can move with the ReYMeN root.
...
def create_job(
    ...
    Args:
        ...
        workdir: Optional absolute path.  When set, the job runs as if launched
                ...
                predictably.
        profile: Optional ReYMeN profile name. When set, the job runs with
                that profile's ReYMeN_HOME so profile-specific config,
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _ReYMeN_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _ReYMeN_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _ReYMeN_now().strftime("%Y-%m-%d_%H-%M-%S")
