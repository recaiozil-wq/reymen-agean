...
def motor_kaydet(motor) -> None:
    """Motor'a CLARIFY aracını kaydet."""
    if not hasattr(motor, "_plugin_arac_kaydet"):
        return
    motor._plugin_arac_kaydet(
        "CLARIFY",
        lambda soru="", secenekler="", varsayilan="": run(
            soru=soru,
            secenekler=[s.strip() for s in secenekler.split("|") if s.strip()] if secenekler else None,
            varsayilan=varsayilan,
        ),
        "Net olmayan taleplerde kullanıcıya seçenek sunar veya açıklama ister.\n"
        'Kullanım: CLARIFY(soru="Hangi ortam?", secenekler="local|docker", varsayilan="local")',
    )
