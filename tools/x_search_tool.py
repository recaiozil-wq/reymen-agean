...
TOOL_META = {
    "ad": "x_search",
    "versiyon": "1.0.0",
    "aciklama": "X/Twitter'da tweet ara, kullanici profili getir, son tweetleri listele.",
    "kategori": "web",
    "parametreler": {
        "islem": {
            "tip": "str",
            "aciklama": "Islem: 'ara', 'profil', 'son_tweetler'",
            "zorunlu": True,
        },
        "sorgu": {
            "tip": "str",
            "aciklama": "Arama sorgusu (ara icin) veya kullanici adi (profil/son_tweetler icin)",
            "zorunlu": True,
        },
        "max_sonuc": {
            "tip": "int",
            "aciklama": "Maks sonuc sayisi (varsayilan: 10)",
            "zorunlu": False,
        },
        "lang": {
            "tip": "str",
            "aciklama": "Dil filtresi (ornek: tr, en)",
            "zorunlu": False,
        },
    },
    "ornek": (
        'X_SEARCH(islem="ara", sorgu="yapay zeka", max_sonuc=3, lang="tr")\n'
        'X_SEARCH(islem="profil", sorgu="elonmusk")\n'
        'X_SEARCH(islem="son_tweetler", sorgu="elonmusk", max_sonuc=5)'
    ),
}


def run(islem: str, sorgu: str, max_sonuc: int = 10, lang: str = "") -> str:
    """X/Twitter islemlerini yurut.

    Args:
        islem: 'ara', 'profil', 'son_tweetler'
        sorgu: Arama sorgusu veya kullanici adi
    ...
    if islem == "ara":
        return tweet_ara(sorgu, int(max_sonuc), lang)
    elif islem == "profil":
        return kullanici_profili(sorgu)
    elif islem == "son_tweetler":
        return son_tweetler(sorgu, int(max_sonuc))
    else:
        return f"[X] Bilinmeyen islem: '{islem}'. Kullan: ara, profil, son_tweetler"


def check_fn(parametreler: dict) -> tuple:
    """Dogrumala: islem ve sorgu zorunlu."""
    if not parametreler.get("islem"):
        return False, "X_SEARCH: 'islem' parametresi zorunludur (ara/profil/son_tweetler)"
    if not parametreler.get("sorgu"):
        return False, "X_SEARCH: 'sorgu' parametresi zorunludur"
    return True, ""
