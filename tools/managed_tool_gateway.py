...
def auth_json_path():
    """Return the ReYMeN auth store path, respecting ReYMeN_HOME overrides."""
    return get_ReYMeN_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`ReYMeN tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from ReYMeN_cli.auth import resolve_nous_access_token
