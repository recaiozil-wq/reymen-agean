...
# -*- coding: utf-8 -*-
"""
tools/mcp_manager.py — MCP (Model Context Protocol) Yöneticisi.
...
try:
    ...
    import logging
    logger = logging.getLogger("hermes.mcp_manager")
    logging.basicConfig(level=logging.INFO)

# ── Varsayılan config.yaml yolu ───────────────────────────────────
...
class MCPServerBaglantisi:
    ...
    async def tools_kesfet(self) -> int:
        ...
        if "error" in sonuc:
            logger.warning(f"MCP [{self.ad}] keşif hatası: {sonuc['error']}")
            return 0
...
class MCPManager:
    ...
    def _config_yukle(self) -> dict:
        ...
        try:
            ...
            logger.warning("PyYAML yok, config yüklenemedi")
        except Exception as e:
    ...
    async def baslat(self) -> int:
        ...
        for ad, cfg in servers_config.items():
            ...
            try:
                ...
                logger.info(f"MCP [{ad}]: {sayi} tool keşfedildi ({cfg.get('transport', 'tcp')})")
                toplam += sayi
            except Exception as e:
                logger.warning(f"MCP [{ad}] başlatılamadı: {e}")

        self._basladi = True
        logger.info(f"MCP: {len(self._sunucular)} sunucu, {toplam} tool")
        return toplam
    ...
    async def cagir(self, sunucu: str, arac: str, args: Optional[dict] = None) -> dict:
        ...
        if not baglanti:
            ...
            return {"durum": "hata", "hata": f"'{sunucu}' bulunamadı. Mevcut: {mevcut}"}
