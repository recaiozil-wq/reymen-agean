...
Engine'ler:
  ...
  - EdgeTTS      â€” edge-tts (yerel, ücretsiz, internet gerekli)
  - WhisperSTT   â€” OpenAI Whisper API (ses â†’ yazÄ±, OPENAI_API_KEY gerekli)
  - StubVoice    â€” local dummy (baÄŸÄ±mlÄ±lÄ±k gerektirmez, simüle eder)
...
def _cikti_dizini() -> str:
    """Ses dosyalarÄ± için çÄ±ktÄ± dizinini oluÅŸtur ve döndür."""
...
def _get_openai_api_key() -> str:
    """OPENAI_API_KEY ortam deÄŸiÅŸkenini döndür."""
...
class VoiceEngine(ABC):
    """Tüm ses engine'leri için soyut taban sÄ±nÄ±fÄ±."""
    ...
    def seslendir(self, text: str, voice: str = "alloy") -> str:
        """Metni sese çevir, ses dosyasÄ± yolunu döndür."""
        ...

    @abstractmethod
    def yaziya_cevir(self, ses_dosyasi: str) -> str:
        """Ses dosyasÄ±nÄ± metne çevir, metni döndür."""
        ...
...
class OpenAITTS(VoiceEngine):
    """OpenAI TTS API ile metni sese çevirir. OPENAI_API_KEY ortam deÄŸiÅŸkeni gerekli.
    ...
    def yaziya_cevir(self, ses_dosyasi: str) -> str:
        """Ses dosyasÄ±nÄ± OpenAI Whisper ile metne çevir.

        Not: OpenAITTS engine'i STT için deÄŸil TTS içindir.
...
class EdgeTTS(VoiceEngine):
    ...
    VarsayÄ±lan ses: tr-TR-AhmetNeural (Türkçe erkek)
    """
    ...
    def seslendir(self, text: str, voice: str = "tr-TR-AhmetNeural") -> str:
        ...
        Args:
            text: Seslendirilecek metin.
            voice: Edge TTS ses adÄ± (örn. tr-TR-AhmetNeural, en-US-AriaNeural).
...
class WhisperSTT(VoiceEngine):
    ...
    def yaziya_cevir(self, ses_dosyasi: str) -> str:
        ...
        try:
            ...
            )  # binary'yi latin-1 ile güvenli taÅŸÄ±
            body_parts.append(f"\r\n--{boundary}--\r\n")
...
class VoiceRegistry:
    """Ses engine'lerini kaydet, seç ve çalÄ±ÅŸtÄ±r.
    ...
    def sec(self, ad: str) -> Optional[VoiceEngine]:
        """Ada göre engine seç. Bulunamazsa varsayÄ±lanÄ± döndür."""
    ...
    def yaziya_cevir(self, engine_adi: str, ses_dosyasi: str) -> str:
        """Bir engine ile ses dosyasÄ±nÄ± metne çevir."""
        if not ses_dosyasi:
...
def _get_registry() -> VoiceRegistry:
    """Global VoiceRegistry singleton'Ä±nÄ± oluÅŸtur/döndür."""
    global _registry
...
def seslendir(text: str, voice: str = "alloy", backend: str = "") -> str:
    """SESLENDIR tool'u â€” metni sese çevir.
...
def yaziya_cevir(ses_dosyasi: str, backend: str = "whisper") -> str:
    """YAZIYA_CEVIR tool'u â€” ses dosyasÄ±nÄ± metne çevir.
