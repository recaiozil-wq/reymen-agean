...
Ä°ki bileÅŸen:
  1. HallucinationFiltresi
       ...
       yanlÄ±ÅŸ sürüm referansÄ±, öz-referans halüsinasyon) tespit eder.
       YanÄ±tÄ± deÄŸiÅŸtirmez; yalnÄ±zca uyarÄ± listesi üretir.

  2. HITLSikistirici
       motor.ekstra_riskli kümesini geniÅŸleterek ek araçlarÄ± HITL onayÄ±na alÄ±r.
...
_KESIN_IDDIA_KALIPLARI: list[str] = [
    r"\bkesinlikle\b",
    r"\bmutlaka\b",
    r"\bher\s+zaman\s+(?:çalÄ±ÅŸÄ±r|doÄŸru|iÅŸe\s+yarar)\b",
    r"\basla\s+(?:hata\s+vermez|çökmez|baÅŸarÄ±sÄ±z\s+olmaz)\b",
    r"\b%100\b",
    r"\bgarantili\b",
]

# Eski / yanlÄ±ÅŸ sürüm referanslarÄ±
...
# Derleme â€” tek seferlik, modül yüklenirken
...
class Uyari:
    ...
    kod: str  # Makine tarafÄ±ndan okunabilir tanÄ±mlayÄ±cÄ± (ör. "EMIN_OLMAYAN")
    mesaj: str  # Ä°nsan için açÄ±klama
...
class HallucinationFiltresi:
    ...
    YanÄ±tÄ± deÄŸiÅŸtirmez â€” uyarÄ± listesi üretir ve yanÄ±tÄ± olduÄŸu gibi döndürür.
    ReAct döngüsü veya kullanÄ±cÄ± uyarÄ±larÄ± göz önünde bulundurarak karar verebilir.

    Args:
        esik_skor: Toplam risk puanÄ± bu eÅŸiÄŸi geçerse yanÄ±t "riskli" sayÄ±lÄ±r.
                   VarsayÄ±lan: 2.0
    """
    ...
    def filtrele(
        ...
        """YanÄ±tÄ± hallucination için tara.

        Args:
            yanit:  LLM'den gelen ham metin.
            hedef:  KullanÄ±cÄ± görevinin kÄ±sa açÄ±klamasÄ± (iliÅŸki kontrolü için).
    ...
    # â”€â”€ Kontrol yöntemleri â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _emin_olmayan_kontrol(self, yanit: str) -> list[Uyari]:
        if _EMIN_OLMAYAN_RE.search(yanit):
            return [
                Uyari(
                    kod="EMIN_OLMAYAN",
                    mesaj="YanÄ±t belirsiz/emin olmayan dil içeriyor â€” doÄŸrulama önerilir.",
                    skor=0.5,
                )
            ]
        return []
    ...
    def _kesin_iddia_kontrol(self, yanit: str) -> list[Uyari]:
        if _KESIN_IDDIA_RE.search(yanit):
            return [
                Uyari(
                    kod="KESIN_IDDIA",
                    mesaj="YanÄ±t doÄŸrulanamaz kesin iddia içeriyor.",
                    skor=1.0,
                )
            ]
        return []
    ...
    def _yanlis_surum_kontrol(self, yanit: str) -> list[Uyari]:
        ...
        if m:
            return [
                Uyari(
                    kod="YANLIS_SURUM",
                    mesaj=f"Eski/yanlÄ±ÅŸ sürüm referansÄ±: '{m.group()}'.",
                    skor=1.5,
                )
            ]
        return []
    ...
    def _oz_referans_kontrol(self, yanit: str) -> list[Uyari]:
        if _OZ_REFERANS_RE.search(yanit):
            return [
                Uyari(
                    kod="OZ_REFERANS",
                    mesaj="LLM internet/disk eriÅŸimi iddiasÄ± â€” muhtemel halüsinasyon.",
                    skor=2.0,
                )
            ]
        return []
    ...
    def _uzun_yanit_kontrol(self, yanit: str) -> list[Uyari]:
        if len(yanit) > _UZUN_YANIT_ESIGI:
            return [
                Uyari(
                    kod="UZUN_YANIT",
                    mesaj=f"YanÄ±t uzun ({len(yanit)} karakter) â€” baÅŸ/son kontrolü önerilir.",
                    skor=0.3,
                )
            ]
        return []
    ...
    def _iliski_kontrol(self, yanit: str, hedef: str) -> list[Uyari]:
        """Hedef kelimeleriyle yanÄ±t arasÄ±ndaki örtüÅŸmeyi basitçe kontrol eder."""
        ...
        if hedef_kelimeleri and not hedef_kelimeleri & yanit_kelimeleri:
            return [
                Uyari(
                    kod="ILISKISIZ",
                    mesaj="YanÄ±t hedefe çok az atÄ±f içeriyor.",
                    skor=0.5,
                )
            ]
        return []
...
# VarsayÄ±lan ek riskli araçlar
_EK_RISKLI_ARACLAR: frozenset[str] = frozenset(
    {
        "DOSYA_YAZ",  # Disk yazma
        "TARAYICI_AC",  # TarayÄ±cÄ± açma
        "WEB_ARA",  # Ä°nternet eriÅŸimi
        "TELEGRAM_GONDER",  # DÄ±ÅŸ mesaj gönderme
        "GORUNTU_ANALIZ",  # Görüntü â†’ LLaVA
    }
)


class HITLSikistirici:
    """HITL onay eÅŸiÄŸini yükseltip daha fazla aracÄ± onaya tabi tutan yönetici.

    motor.ekstra_riskli kümesini geniÅŸleterek ek araçlarÄ± HITL akÄ±ÅŸÄ±na alÄ±r.
    geri_al() çaÄŸrÄ±sÄ±yla orijinal küme geri yüklenir.

    Args:
        motor:       ReAct motoru (calistir() metoduna sahip nesne).
        ek_araclar:  Onaya eklenecek araç adlarÄ± kümesi.
    ...
    def sikilas(self) -> None:
        """RISKLI kümesini geniÅŸlet ve HITL eÅŸiÄŸini yükselt."""
        if self._aktif:
            logger.debug(
                "[HITL] Zaten sÄ±kÄ±laÅŸtÄ±rÄ±lmÄ±ÅŸ, tekrar çaÄŸrÄ± görmezden gelindi."
            )
            return
        if not self._motor:
        ...
        # ekstra_riskli attr'u yoksa boÅŸ kümeyle baÅŸlat
        ...
        self._aktif = True
        logger.info(
            "[HITL] SÄ±kÄ±laÅŸtÄ±rÄ±ldÄ±: %d araç onaya eklendi.", len(self._ek_araclar)
        )
...
def motor_hitl_yamasi_uygula(motor: object) -> None:
    ...
    Motor varsayÄ±lan olarak yalnÄ±zca sabit RISKLI kümesine bakar.
    Bu yama, motor.ekstra_riskli kümesini de kontrol etmesini saÄŸlar.
    Yama yalnÄ±zca bir kez uygulanÄ±r; tekrar çaÄŸrÄ±lmasÄ± güvenlidir.
...
if __name__ == "__main__":
    ...
    testler: list[tuple[str, str]] = [
        ...
        ("Bu iÅŸlevi çaÄŸÄ±rabilirsin.", "fonksiyon çaÄŸÄ±r"),
        ("Bu kesinlikle %100 çalÄ±ÅŸacak.", "güvenilir kod"),
