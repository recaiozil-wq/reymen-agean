...
class OgrenmeDeposu:
    ...
    def sayi(self) -> int:
        if not self._dosya.exists():
            return 0
        try:
            return sum(1 for s in open(self._dosya, "r", encoding="utf-8") if s.strip())
        except Exception as e:
    ...
    def _temizle(self):
        ...
        try:
            ...
            if len(satirlar) <= MAKS_KAYIT:
                return
            with open(self._dosya, "w", encoding="utf-8") as f:
                for s in satirlar[-MAKS_KAYIT:]:
                    f.write(s + "\n")
        except Exception as e:
...
class SurekliOgrenmeYoneticisi:
    ...
    def ozet(self) -> str:
        ...
        if alanlar:
            ...
            for a in alanlar:
                satirlar.append(
                    f"    {a}: {len(self.depo.hepsini_getir(alan=a, limit=10000))} kayit"
                )
        if self.adaptif:
            try:
                satirlar.append(f"  Tercih: {self.adaptif.tercih_sayisi()} adet")
            except Exception as e:
...
def get_yonetici():
    ...
    if _yonetici is None:
        _yonetici = SurekliOgrenmeYoneticisi()
    return _yonetici
...
def motor_kaydet(motor) -> None:
    ...
    try:
        ...
        motor._plugin_arac_kaydet(
            "OGREN",
            lambda ham="": (ogren(*_ayristir(ham)) if ham else ogrenme_ozeti()),
            "Ogrenme kaydet: OGREN(alan=..., icerik=...)",
        )
        motor._plugin_arac_kaydet(
            "OGRENMELERI_GETIR",
            lambda a="": ogrenmeleri_getir(a),
            "Gecmis ogrenmeler: OGRENMELERI_GETIR(alan=...)",
        )
        motor._plugin_arac_kaydet(
            "OGRENME_OZETI", lambda: ogrenme_ozeti(), "Istatistikler."
        )
...
def _ayristir(ham: str) -> tuple:
    ...
    return (
        alan.group(1) if alan else "genel",
        icerik.group(1) if icerik else (ham.strip().strip('"') or "bos"),
        "tool",
    )
