...
def _ana():
    if not os.path.exists(BROKEN):
        print(f"skill_index.db not found at: {BROKEN}")
        return
    ...
    try:
        ...
        if os.path.exists(GOOD):
            ...
            try:
                ...
                cn.execute(
                    "CREATE VIRTUAL TABLE skill_fts USING fts5(ad, aciklama, icerik, kaynak, content='', tokenize='unicode61')"
                )
                ...
                for row in rows:
                    cn.execute(
                        "INSERT INTO skill_fts (ad, aciklama, icerik, kaynak) VALUES (?, ?, ?, ?)",
                        row,
                    )
                conn_new.commit()
