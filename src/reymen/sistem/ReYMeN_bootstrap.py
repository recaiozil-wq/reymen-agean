...
should be imported at the very top of every ReYMeN entry point
(``ReYMeN``, ``ReYMeN-agent``, ``ReYMeN-acp``, ``python -m gateway.run``,
...
def apply_windows_utf8_bootstrap() -> bool:
    ...
    for stream_name in ("stdout", "stderr"):
        ...
        try:
            ...
            logger.warning("[ReymenBootstrap] Dosya/klasor hatasi (L105): %s", OSError)
            pass
    ...
    if stdin is not None:
        ...
        if reconfigure is not None:
            try:
                reconfigure(encoding="utf-8", errors="replace")
            except (OSError, ValueError) as _ReYMeN_b_e117:
                print(f"[UYARI] ReYMeN_bootstrap.py:118 - {_ReYMeN_b_e117}")

    _bootstrap_applied = True
    return True


# Apply on import â€” entry points just need ``import ReYMeN_bootstrap``
# (or ``from ReYMeN_bootstrap import apply_windows_utf8_bootstrap``) at
