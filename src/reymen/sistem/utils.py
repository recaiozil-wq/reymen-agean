...
def env_float(
    ...
    Args:
        ...
        min_value: Optional lower clamp (inclusive).
        max_value: Optional upper clamp (inclusive).

    Returns:
        A finite float within [min_value, max_value] if both are set.
    """
...
def env_int(
    ...
    Returns:
        An int within [min_value, max_value] if both are set.
    """
    raw = os.environ.get(name)
    if raw is None:
        return default
...
def _restore_file_mode(path: Path, mode: "int | None") -> None:
    ...
    try:
        os.chmod(path, mode)
    except OSError as _utils_e56:
        print(f"[UYARI] utils.py:57 - {_utils_e56}")
    ...
    **dump_kwargs: Any,
    ...
    try:
        ...
        if mode is not None:
            ...
            except OSError as _utils_e139:
                print(f"[UYARI] utils.py:140 - {_utils_e139}")
        else:
        ...
        try:
            os.unlink(tmp_path)
        except OSError as _utils_e148:
            print(f"[UYARI] utils.py:149 - {_utils_e148}")
        raise
...
def atomic_yaml_write(
    ...
    *,
    ...
    try:
        ...
        try:
            os.unlink(tmp_path)
        except OSError as _utils_e200:
            print(f"[UYARI] utils.py:201 - {_utils_e200}")
        raise
...
def atomic_roundtrip_yaml_update(
    ...
    try:
        ...
        try:
            os.unlink(tmp_path)
        except OSError as _utils_e264:
            print(f"[UYARI] utils.py:265 - {_utils_e264}")
        raise
