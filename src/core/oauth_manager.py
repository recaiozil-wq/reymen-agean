...
class GoogleOAuthProvider(OAuthProvider):
    ...
    def _http_post(self, url: str, data: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise OAuthError(
                f"HTTP {e.code}: {body[:200]}", provider="google", status_code=e.code
            )
...
class GitHubOAuthProvider(OAuthProvider):
    ...
    def get_user_info(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace") if e.fp else ""
            raise OAuthError(
                f"GitHub userinfo HTTP {e.code}: {body[:200]}", provider="github"
            )
...
class DiscordOAuthProvider(OAuthProvider):
    ...
    def callback_handler(self, code: str, redirect_uri: str = "") -> OAuthToken:
        ...
        try:
            ...
            token.user_id = str(user_raw.get("id", ""))
            token.email = user_raw.get("email", "")
            token.display_name = user_raw.get("global_name", "") or user_raw.get(
                "username", ""
            )
    ...
    def token_refresh(self, refresh_token: str) -> OAuthToken:
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace") if e.fp else ""
            raise OAuthError(
                f"Discord refresh HTTP {e.code}: {body[:200]}", provider="discord"
            )
    ...
    def get_user_info(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            body = e.read().decode("utf-8", errors="replace") if e.fp else ""
            raise OAuthError(
                f"Discord userinfo HTTP {e.code}: {body[:200]}", provider="discord"
            )
...
class OAuthManager:
    ...
    def callback(self, provider: str, code: str, redirect_uri: str = "") -> OAuthToken:
        ...
        if not p:
            raise OAuthError(
                f"Bilinmeyen OAuth provider: '{provider}'", provider=provider
            )
    ...
    def durum(self, provider: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "access_token_prefix": token.access_token[:10] + "..."
...
def _oauth_giris_tool(**kw) -> str:
    ...
    if not provider:
        return (
            "[HATA] OAUTH_GIRIS: provider parametresi zorunlu (google/github/discord)"
        )
