...
from core.model_adapter import get_active_adapter
...
def run_script(path: str) -> tuple[bool, str]:
    """Python script Ã§alÄ±ÅŸtÄ±r. DÃ¶ner: (baÅŸarÄ±lÄ±_mÄ±, Ã§Ä±ktÄ±/hata)"""
    try:
        r = subprocess.run(
        ...
        return False, "TIMEOUT: 120s aÅŸÄ±ldÄ±"
    except Exception as e:
        return False, str(e)


def ask_model_to_fix(code: str, error: str) -> str:
    """LLM'e hata mesajÄ±nÄ± gÃ¶nder, dÃ¼zeltilmiÅŸ kod al"""
    prompt = f"""Åu Python kodu Ã§alÄ±ÅŸtÄ±rÄ±ldÄ± ve hata verdi.
...
2. DÃ¼zelt
3. Sadece Ã§alÄ±ÅŸan Python kodunu dÃ¶ndÃ¼r â€” aÃ§Ä±klama yok, markdown yok
"""
    return _get_adapter().complete(prompt)


def solve_step(step_name: str, script_path: str) -> bool:
    """Tek adÄ±mÄ± Ã§Ã¶z. Hata varsa max 3 kere dene."""
    path = Path(script_path)
    if not path.exists():
        print(f"[{step_name}] âŒ Script bulunamadÄ±: {path}")
        return False

    for attempt in range(1, MAX_RETRIES + 1):
        print(f"[{step_name}] â”€â”€ Deneme {attempt}/{MAX_RETRIES}")
        success, output = run_script(str(path))
        if success:
            print(f"[{step_name}] âœ… GeÃ§ti")
            _log("logs/run_log.jsonl", step_name, attempt, True, output[:400])
            return True

        print(f"[{step_name}] âŒ Hata â†’ modele gÃ¶nderiliyor")
        ...
        print(f"[{step_name}]   â†’ Fix yazÄ±ldÄ±: {fix_path.name}")
        path = fix_path

    print(f"[{step_name}] ğŸ’€ GeÃ§ilemedi â€” loglandÄ±")
    _log("logs/fail_log.jsonl", step_name, MAX_RETRIES, False, output[:400])
    return False


def solve_all(steps: list[tuple[str, str]]) -> dict:
    """TÃ¼m adÄ±mlarÄ± sÄ±rayla Ã§Ã¶z. DÃ¶ner: {step_name: success_bool}"""
    results = {}
...
# â”€â”€ DoÄŸrudan hata Ã§Ã¶zÃ¼mÃ¼ (motor.py'den Ã§aÄŸrÄ±lacak) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def coz_hata(hata: str, kod: str = "", ad: str = "acil_fix") -> str:
    """Herhangi bir hatayÄ± LLM'e sor, dÃ¼zeltilmiÅŸ kod al.
    Motor.py'de try/except iÃ§inde Ã§aÄŸrÄ±lÄ±r."""
    if not kod:
        return f"[COZ] Hata loglandÄ±: {hata[:200]}"
