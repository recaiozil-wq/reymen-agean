...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      ReYMeN_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      ReYMeN_DASHBOARD_PORTAL_URL       — defaults to
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``ReYMeN-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``ReYMeN_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set ReYMeN_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
  providers registered" the gate would otherwise emit.
"""


__all__ = ['Any', 'DashboardAuthProvider', 'Dict', 'InvalidCodeError', 'LoginStart', 'NousDashboardAuthProvider', 'Optional', 'ProviderError', 'PyJWKClient', 'RefreshExpiredError', 'Session', 'annotations', 'cfg_get', 'complete_login', 'load_config', 'refresh_session', 'register', 'revoke_session', 'start_login', 'verify_session']
...
from ReYMeN_cli.dashboard_auth import (
...
# Production Portal URL. Override via ReYMeN_DASHBOARD_PORTAL_URL for
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"ReYMeN_session_pkce\"]``
        ...
        cookie_payload = {
            "ReYMeN_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between ReYMeN_DASHBOARD_PORTAL_URL / ReYMeN_DASHBOARD_OAUTH_CLIENT_ID
...
def _load_config_oauth_section() -> dict:
    ...
    """
    try:
        from ReYMeN_cli.config import cfg_get, load_config
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``ReYMeN_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("ReYMeN_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``ReYMeN_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("ReYMeN_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``ReYMeN_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set ReYMeN_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``ReYMeN_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "ReYMeN_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Nous Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a ReYMeN Agent "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"ReYMeN_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
