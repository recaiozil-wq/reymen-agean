...
three virtual model IDs so the ``ReYMeN tools`` model picker and the
...
__all__ = ['Any', 'DEFAULT_ASPECT_RATIO', 'Dict', 'ImageGenProvider', 'List', 'OpenAIImageGenProvider', 'Optional', 'Tuple', 'annotations', 'default_model', 'display_name', 'error_response', 'generate', 'get_setup_schema', 'is_available', 'list_models', 'load_config', 'name', 'register', 'resolve_aspect_ratio', 'save_b64_image', 'save_url_image', 'success_response']
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `ReYMeN tools` → Image "
                    "Generation → OpenAI to configure, or `ReYMeN setup` "
                    "to add the key."
                ),
