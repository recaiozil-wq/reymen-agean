...
Output is saved as PNG under ``$ReYMeN_HOME/cache/images/``.
"""


__all__ = ['Any', 'DEFAULT_ASPECT_RATIO', 'Dict', 'ImageGenProvider', 'List', 'OpenAICodexImageGenProvider', 'Optional', 'Tuple', 'annotations', 'default_model', 'display_name', 'error_response', 'flush', 'generate', 'get_setup_schema', 'is_available', 'list_models', 'load_config', 'name', 'register', 'resolve_aspect_ratio', 'save_b64_image', 'success_response']
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `ReYMeN auth codex` (or `ReYMeN setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`ReYMeN auth codex` (or `ReYMeN setup` → Codex) to sign in."
                ),
