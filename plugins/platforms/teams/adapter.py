"""
Microsoft Teams platform adapter for ReYMeN Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "ReYMeN"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        ReYMeN_action = data.get("ReYMeN_action", "")
        session_key = data.get("session_key", "")

        if not ReYMeN_action or not session_key:
        ...
        choice = choice_map.get(ReYMeN_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="ReYMeN_approve",
                    data={**btn_data_base, "ReYMeN_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="ReYMeN_approve",
                    data={**btn_data_base, "ReYMeN_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="ReYMeN_approve",
                    data={**btn_data_base, "ReYMeN_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="ReYMeN_approve",
                    data={**btn_data_base, "ReYMeN_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"ReYMeN\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.ReYMeN/.env")
