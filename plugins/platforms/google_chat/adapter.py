...
# ``model_tools`` import time, so ``ReYMeN status``, ``ReYMeN chat``, etc.
...
# ``ReYMeN_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "ReYMeN is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a ReYMeN MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ReYMeN_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from ReYMeN_constants import get_ReYMeN_home as _get_ReYMeN_home
            _ReYMeN_home = _get_ReYMeN_home()
        except (ModuleNotFoundError, ImportError):
            _ReYMeN_home = _Path.home() / ".ReYMeN"
        self._thread_count_store = _ThreadCountStore(
            _ReYMeN_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "ReYMeN is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ReYMeN_HOME", str(_Path.home() / ".ReYMeN"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ReYMeN_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a ReYMeN MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'ReYMeN is thinking…' marker message.
        ...
        "ReYMeN is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "ReYMeN is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "ReYMeN is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "ReYMeN is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "ReYMeN is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``ReYMeN setup``.

    The setup wizard at ``ReYMeN_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.ReYMeN/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. ReYMeN-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.ReYMeN/.env")
    print_info("Restart the gateway: ReYMeN gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``ReYMeN cron`` running as a
    separate process from ``ReYMeN gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the ReYMeN plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'ReYMeN-agent[google_chat]'",
        ...
        # the "ReYMeN is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'ReYMeN is thinking…' marker message "
