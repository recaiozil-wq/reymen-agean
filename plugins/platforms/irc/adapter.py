"""
IRC Platform Adapter for ReYMeN Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the ReYMeN agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "ReYMeN-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :ReYMeN Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :ReYMeN Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: ReYMeN_, ReYMeN_1, ReYMeN_2...
...
def check_requirements() -> bool:
    ...
    # ReYMeN setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `ReYMeN gateway setup` flow for the IRC platform.

    Lazy-imports ``ReYMeN_cli.setup`` helpers so the plugin stays importable
    in non-CLI contexts (gateway runtime, tests).
    """
    from ReYMeN_cli.setup import (
        prompt,
        prompt_yes_no,
        save_env_value,
        get_env_value,
    ...
    print_info("Connect ReYMeN to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #ReYMeN — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.ReYMeN/.env")
    print_info("Restart the gateway for changes to take effect: ReYMeN gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "ReYMeN-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "ReYMeN-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :ReYMeN Agent (cron)")
