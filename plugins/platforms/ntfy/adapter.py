"""ntfy platform adapter (ReYMeN plugin).
...
``plugins/platforms/ntfy/``. The ReYMeN plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "ReYMeN-in"              # subscribe topic (incoming)
          publish_topic: "ReYMeN-out"     # optional — defaults to topic
...
_ECHO_TAG = "ReYMeN-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``ReYMeN cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a ReYMeN dependency",
