"""
LINE Messaging API platform adapter for ReYMeN Agent.
...
to ReYMeN Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ReYMeN_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            ReYMeN_home = Path(get_ReYMeN_home()).resolve()
        except Exception:
            ReYMeN_home = Path.home().joinpath(".ReYMeN").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``ReYMeN status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``ReYMeN status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ReYMeN setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.ReYMeN/.env`` via ``ReYMeN_cli.config``.
    """
    print()
    ...
    try:
        from ReYMeN_cli.config import get_env_var, set_env_var
    except ImportError:
        print("ReYMeN_cli.config not available; set LINE_* vars manually in ~/.ReYMeN/.env")
        return
