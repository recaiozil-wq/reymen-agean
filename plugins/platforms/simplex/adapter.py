"""SimpleX Chat platform adapter (ReYMeN plugin).
...
This adapter ships as a ReYMeN platform plugin under
``plugins/platforms/simplex/``. The ReYMeN plugin loader scans the
...
discoverable and `ReYMeN setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside ReYMeN — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"ReYMeN-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ReYMeN setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.ReYMeN/.env`` via ``ReYMeN_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("ReYMeN_cli.config not available; set SIMPLEX_* vars manually in ~/.ReYMeN/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `ReYMeN gateway status` without
