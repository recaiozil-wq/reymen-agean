...
session-scoped state file under ``$ReYMeN_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ReYMeN_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_ReYMeN_home()) / "workspace" / "meetings"
...
def _clear_active() -> None:
    try:
        ...
        logger.warning("[fix_01_sessiz_except] FileNotFoundError")
...
def start(
    ...
    *,
    ...
    If a bot is already running for this ReYMeN install, leave it first —
    ...
    env = os.environ.copy()
    env["ReYMeN_MEET_URL"] = url
    env["ReYMeN_MEET_OUT_DIR"] = str(out)
    env["ReYMeN_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ReYMeN_MEET_HEADED"] = "1"
    if auth_state:
        env["ReYMeN_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ReYMeN_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ReYMeN_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ReYMeN_MEET_MODE"] = mode
    if realtime_model:
        env["ReYMeN_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ReYMeN_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ReYMeN_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ReYMeN_MEET_REALTIME_KEY"] = realtime_api_key
...
def stop(*, reason: str = "requested") -> Dict[str, Any]:
    ...
    if pid and _pid_alive(pid):
        try:
            ...
            logger.warning("[fix_01_sessiz_except] ProcessLookupError")
        for _ in range(20):
