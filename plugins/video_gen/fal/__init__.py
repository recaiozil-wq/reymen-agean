...
__all__ = ['Any', 'Dict', 'FALVideoGenProvider', 'List', 'Optional', 'Tuple', 'VideoGenProvider', 'annotations', 'capabilities', 'default_model', 'display_name', 'error_response', 'fal_key_is_configured', 'generate', 'get_setup_schema', 'import_fal_client', 'is_available', 'list_models', 'load_config', 'name', 'prefers_gateway', 'register', 'resolve_managed_tool_gateway', 'success_response']
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `ReYMeN tools` → Video Generation."
            ) from exc
        raise
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `ReYMeN tools` → Video Generation → FAL to configure) "
                    "or sign in to Nous (`ReYMeN setup`) for managed gateway access."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `ReYMeN tools` → Video Generation."
                    ),
