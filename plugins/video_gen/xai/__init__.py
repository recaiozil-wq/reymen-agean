...
__all__ = ['Any', 'Dict', 'List', 'Optional', 'ReYMeN_xai_user_agent', 'Tuple', 'VideoGenProvider', 'XAIVideoGenProvider', 'annotations', 'capabilities', 'default_model', 'display_name', 'error_response', 'generate', 'get_setup_schema', 'is_available', 'list_models', 'name', 'register', 'resolve_xai_http_credentials', 'success_response']
...
def _xai_user_agent() -> str:
    try:
        ...
        return ReYMeN_xai_user_agent()
    except Exception:
        return "ReYMeN-agent/video_gen"
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``ReYMeN_cli/tools_config.py``) so the picker doesn't blindly
    ...
    async def _generate_async(
        ...
        *,
        ...
        if not api_key:
            return error_response(
                error=(
                    "No xAI credentials found. Sign in via `ReYMeN auth add xai-oauth` "
