"""langfuse — ReYMeN plugin for Langfuse observability.

Traces ReYMeN conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the ReYMeN plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``ReYMeN plugins enable
observability/langfuse`` or ``ReYMeN tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``ReYMeN tools`` or ~/.ReYMeN/.env):
  ReYMeN_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  ReYMeN_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  ReYMeN_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  ReYMeN_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  ReYMeN_LANGFUSE_RELEASE     - release/version tag
  ReYMeN_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  ReYMeN_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  ReYMeN_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""

__all__ = ['Any', 'CanonicalUsage', 'Decimal', 'Dict', 'Langfuse', 'Optional', 'TraceState', 'annotations', 'dataclass', 'estimate_usage_cost', 'field', 'get_pricing_entry', 'normalize_usage', 'on_post_llm_call', 'on_post_tool_call', 'on_pre_llm_call', 'on_pre_llm_request', 'on_pre_tool_call', 'propagate_attributes', 'register']
...
_LANGFUSE_KEY_PREFIXES: Dict[str, str] = {
    "ReYMeN_LANGFUSE_PUBLIC_KEY": "pk-lf-",
    "ReYMeN_LANGFUSE_SECRET_KEY": "sk-lf-",
}
...
def _debug_enabled() -> bool:
    return _env_bool("ReYMeN_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the ReYMeN plugin system —
    ...
    public_key = _env("ReYMeN_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("ReYMeN_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("ReYMeN_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("ReYMeN_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset ReYMeN_LANGFUSE_PUBLIC_KEY / ReYMeN_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("ReYMeN_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("ReYMeN_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("ReYMeN_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("ReYMeN_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid ReYMeN_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("ReYMeN_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="ReYMeN turn",
                tags=["ReYMeN", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older ReYMeN branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current ReYMeN also has a turn-scoped
    ...
    # messages is a list only for legacy ReYMeN branches that fired
    # pre_llm_call with API messages directly. Current ReYMeN fires
...
def register(ctx) -> None:
    ...
    # ReYMeN versions.  pre_api_request / post_api_request fire per API
