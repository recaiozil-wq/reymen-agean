"""disk_cleanup — ephemeral file cleanup for ReYMeN Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ReYMeN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ReYMeN_HOME and /tmp/ReYMeN-*
Never touches: ~/.ReYMeN/logs/ or any system directory.
"""
...
try:
    ...
    def get_ReYMeN_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ReYMeN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".ReYMeN").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ReYMeN_HOME/logs/``."""
    return get_ReYMeN_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ReYMeN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ReYMeN_HOME or ``/tmp/ReYMeN-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    ReYMeN_home = get_ReYMeN_home()
    try:
        path.resolve().relative_to(ReYMeN_home)
        return True
    except (ValueError, OSError):
        logger.warning("[fix_01_sessiz_except] Exception")
    # Allow /tmp/ReYMeN-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("ReYMeN-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ReYMeN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ReYMeN_HOME (but leave ReYMeN_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "ReYMeN-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(ReYMeN_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == ReYMeN_home:
                continue
            try:
                rel_parts = dirpath.relative_to(ReYMeN_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    ReYMeN_home = get_ReYMeN_home()
    try:
        rel = path.resolve().relative_to(ReYMeN_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "ReYMeN-agent",
        }:
            return None
        ...
        # Path isn't under ReYMeN_HOME (e.g. /tmp/ReYMeN-*) — fall through.
