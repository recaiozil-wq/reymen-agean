...
Config: Uses the existing Honcho config chain:
  1. $ReYMeN_HOME/honcho.json (profile-scoped)
  2. ~/.honcho/config.json (legacy global)
  3. Environment variables
"""


__all__ = ['Any', 'Dict', 'HonchoClientConfig', 'HonchoMemoryProvider', 'HonchoSessionManager', 'List', 'MemoryProvider', 'Optional', 'Path', 'annotations', 'atomic_json_write', 'cmd_setup', 'get_ReYMeN_home', 'get_config_schema', 'get_honcho_client', 'get_tool_schemas', 'handle_tool_call', 'initialize', 'is_available', 'liveness_snapshot', 'name', 'on_memory_write', 'on_session_end', 'on_turn_start', 'post_setup', 'prefetch', 'queue_prefetch', 'register', 'sanitize_context', 'save_config', 'shutdown', 'sync_turn', 'system_prompt_block', 'tool_error']
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, ReYMeN_home):
        """Write config to $ReYMeN_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(ReYMeN_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                ...
                # behave as they did before. New setups via `ReYMeN honcho setup`
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Skip under per-session strategy: every ReYMeN run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_ReYMeN_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "ReYMeN-default",
