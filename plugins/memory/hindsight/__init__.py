...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: ReYMeN)
...
Or via $ReYMeN_HOME/hindsight/config.json (profile-scoped), falling back to
~/.hindsight/config.json (legacy, shared) for backward compatibility.
"""


__all__ = ['Any', 'Console', 'Dict', 'Hindsight', 'HindsightEmbedded', 'HindsightMemoryProvider', 'List', 'MemoryProvider', 'Path', 'Version', 'annotations', 'atomic_json_write', 'cfg_get', 'datetime', 'get_ReYMeN_home', 'get_config_schema', 'get_tool_schemas', 'handle_tool_call', 'initialize', 'is_available', 'masked_secret_prompt', 'name', 'on_session_switch', 'pkg_version', 'post_setup', 'prefetch', 'queue_prefetch', 'register', 'safe_schedule_threadsafe', 'save_config', 'shutdown', 'sync_turn', 'system_prompt_block', 'timezone', 'tool_error']
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so ReYMeN can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ReYMeN_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_ReYMeN_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "ReYMeN": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "ReYMeN"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this ReYMeN config."""
    profile = config.get("profile", "ReYMeN")
    return str(profile or "ReYMeN")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active ReYMeN profile name (from agent_identity)
      {workspace} — ReYMeN workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``ReYMeN-{user}`` with no user becomes ``ReYMeN``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between ReYMeN Agent and the User"
    ...
    def save_config(self, values, ReYMeN_home):
        """Write config to $ReYMeN_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(ReYMeN_home) / "hindsight"
    ...
    def post_setup(self, ReYMeN_home: str, config: dict) -> None:
        ...
        from ReYMeN_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "ReYMeN")
        ...
        self.save_config(provider_config, ReYMeN_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(ReYMeN_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(ReYMeN_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "ReYMeN"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: ReYMeN-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between ReYMeN Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "ReYMeN"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "ReYMeN"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "ReYMeN", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "ReYMeN")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between ReYMeN Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See ReYMeN-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When ReYMeN created/used that client on the shared async loop,
                    ...
                    try:
                        ...
                        logger.warning("[fix_01_sessiz_except] RuntimeError")
                else:
