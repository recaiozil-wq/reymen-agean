...
Config (env vars or ReYMeN config.yaml under retaindb:):
...
__all__ = ['Any', 'Dict', 'List', 'MemoryProvider', 'Path', 'RetainDBMemoryProvider', 'add_memory', 'annotations', 'ask_user', 'datetime', 'delete_file', 'delete_memory', 'enqueue', 'get_ReYMeN_home', 'get_agent_model', 'get_config_schema', 'get_file', 'get_profile', 'get_tool_schemas', 'handle_tool_call', 'ingest_file', 'ingest_session', 'initialize', 'is_available', 'list_files', 'name', 'on_memory_write', 'prefetch', 'query_context', 'queue_prefetch', 'quote', 'read_file_content', 'register', 'request', 'search', 'seed_agent_identity', 'shutdown', 'sync_turn', 'system_prompt_block', 'timezone', 'tool_error', 'upload_file']
...
class _Client:
    ...
    def upload_file(self, data: bytes, filename: str, remote_path: str, mime_type: str, scope: str, project_id: str | None) -> dict:
        ...
        token = self.api_key.replace("Bearer ", "").strip()
        headers = {"Authorization": f"Bearer {token}", "x-sdk-runtime": "ReYMeN-plugin"}
    ...
    def read_file_content(self, file_id: str) -> bytes:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {token}", "x-sdk-runtime": "ReYMeN-plugin"}, timeout=30, allow_redirects=True)
        resp.raise_for_status()
        return resp.content
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > ReYMeN-<profile> > "default"
        # If unset, the API auto-creates and uses the "default" project — no config required.
        explicit = os.environ.get("RETAINDB_PROJECT")
        if explicit:
            project = explicit
        else:
            ReYMeN_home = str(kwargs.get("ReYMeN_home", ""))
            profile_name = os.path.basename(ReYMeN_home) if ReYMeN_home else ""
            project = f"ReYMeN-{profile_name}" if (profile_name and profile_name not in {"", ".ReYMeN"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "ReYMeN") or "ReYMeN"

        from ReYMeN_constants import get_ReYMeN_home
        ReYMeN_home_path = get_ReYMeN_home()
        db_path = ReYMeN_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = ReYMeN_home_path / "SOUL.md"
        if soul_path.exists():
