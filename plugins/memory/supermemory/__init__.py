...
__all__ = ['Any', 'Dict', 'List', 'MemoryProvider', 'Optional', 'Path', 'Supermemory', 'SupermemoryMemoryProvider', 'add_memory', 'annotations', 'atomic_json_write', 'datetime', 'forget_by_query', 'forget_memory', 'get_ReYMeN_home', 'get_config_schema', 'get_profile', 'get_tool_schemas', 'handle_tool_call', 'ingest_conversation', 'initialize', 'is_available', 'name', 'on_memory_write', 'on_session_end', 'on_turn_start', 'prefetch', 'register', 'save_config', 'search_memories', 'shutdown', 'sync_turn', 'system_prompt_block', 'timezone', 'tool_error']
...
def _load_supermemory_config(ReYMeN_home: str) -> dict:
    config = _default_config()
    config_path = Path(ReYMeN_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, ReYMeN_home: str) -> None:
    config_path = Path(ReYMeN_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `ReYMeN memory setup`.
        # All other options are documented for $ReYMeN_HOME/supermemory.json
    ...
    def save_config(self, values, ReYMeN_home):
        ...
        _save_supermemory_config(sanitized, ReYMeN_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from ReYMeN_constants import get_ReYMeN_home
        self._ReYMeN_home = kwargs.get("ReYMeN_home") or str(get_ReYMeN_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._ReYMeN_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "ReYMeN", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "ReYMeN_memory", "target": target, "type": "explicit_memory"},
