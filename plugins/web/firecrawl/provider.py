...
def _load_firecrawl_cls() -> type:
    ...
    if _FIRECRAWL_CLS_CACHE is None:
        try:
            from reymen.tools.lazy_deps import ensure as _lazy_ensure
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``reymen tools`` setup flow.
    """
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            ...
            "run `reymen tools` and select Nous Subscription as the web provider."
        )
    else:
...
class FirecrawlWebSearchProvider(WebSearchProvider):
    ...
    async def extract(self, urls: List[str], **kwargs: Any) -> List[Dict[str, Any]]:
        ...
        """
        from reymen.tools.interrupt import is_interrupted as _is_interrupted
