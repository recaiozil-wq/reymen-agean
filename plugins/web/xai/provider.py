...
``{title, url, description, position}`` rows every other ReYMeN web
provider produces.
...
prefers ReYMeN-managed xAI Grok OAuth (via ``ReYMeN auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.ReYMeN/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``ReYMeN tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `ReYMeN auth` to sign in with "
