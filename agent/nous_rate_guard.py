...
# ── ReYMeN uyumlu provider rate guard API ────────────────────────────────────
# beyin.py bu 3 fonksiyonu bekliyor: izin_ver / basla / bitir

_NOUS_PROVIDERS = {"nous", "nousresearch", "nous-hermes"}

def rate_guard_izin_ver(provider: str) -> bool:
    """Provider'in istek yapmasina izin var mi?

    Nous/NousResearch icin mevcut rate limit durumunu kontrol eder.
    Diger providerlar icin her zaman True döner (engel yok).
    """
    if provider and provider.lower() in _NOUS_PROVIDERS:
        return nous_rate_limit_remaining() is None
    return True


def rate_guard_basla(provider: str) -> None:
    """Provider istegi basliyor — su an izleme gerekmez."""
    pass


def rate_guard_bitir(provider: str) -> None:
    """Provider istegi bitti — su an izleme gerekmez."""
    pass
