...
DEFAULT_AGENT_IDENTITY = (
    "You are ReYMeN Agent (Hermes Agent), an intelligent AI assistant created by Nous Research. "
...
get_ReYMeN_home = get_hermes_home  # noqa: N816

_REYMEN_MD_NAMES = (".ReYMeN.md", "ReYMeN.md", ".reymen.md")


def _find_ReYMeN_md(cwd: Path) -> Optional[Path]:
    """Find the nearest .ReYMeN.md from cwd up to git root."""
    stop_at = _find_git_root(cwd)
    current = cwd.resolve()
    for directory in [current, *current.parents]:
        for name in _REYMEN_MD_NAMES:
            candidate = directory / name
            if candidate.is_file():
                return candidate
        if stop_at and directory == stop_at:
...
def _load_ReYMeN_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """Load .ReYMeN.md from cwd_path hierarchy."""
    md_path = _find_ReYMeN_md(cwd_path)
    if md_path is None:
        return ""
    try:
    ...
    raw = _scan_context_content(raw, str(md_path))
    raw = _truncate_content(raw, str(md_path))
    header = f"## {md_path.name}\n\n"
    return header + raw
