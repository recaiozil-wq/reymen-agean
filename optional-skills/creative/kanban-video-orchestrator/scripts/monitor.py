#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `ReYMeN kanban list` and
...
def ReYMeN_available() -> bool:
    return shutil.which("ReYMeN") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed ReYMeN CLI."""
    try:
        out = subprocess.run(
            ["ReYMeN", "kanban", "list", "--tenant", tenant, "--json"],
    ...
    # Fallback: textual parse of `ReYMeN kanban list`
    out = subprocess.run(
        ["ReYMeN", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["ReYMeN", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not ReYMeN_available():
        print("ERROR: 'ReYMeN' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
