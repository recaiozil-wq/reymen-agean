...
Usage (via cron with --no-agent):
    ...
    ReYMeN cron create ReYMeN-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ReYMeN_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name ReYMeN-issues --repo NousResearch/ReYMeN-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.ReYMeN/.env to avoid the 60 req/hr
