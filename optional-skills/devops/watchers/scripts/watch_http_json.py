...
Usage (via cron with --no-agent):
    ...
    ReYMeN cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ReYMeN_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "ReYMeN-Watcher/1.0"})
