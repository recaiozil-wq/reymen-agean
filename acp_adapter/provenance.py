...
This is an additive ReYMeN extension surfaced under ACP ``_meta.reymen`` so
...
compression rotates the internal ReYMeN head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.reymen.sessionProvenance`` for an ACP session.

    Args:
        db: A session database (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_session_id: The live internal DB session id.
        previous_session_id: The internal id from before the most recent
            turn, when known.

    Returns:
        A dict suitable for ``{"reymen": {"sessionProvenance": <dict>}}`` under
    ...
    if previous_session_id:
        provenance["previousSessionId"] = previous_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"reymen": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"reymen": {"sessionProvenance": prov}}
