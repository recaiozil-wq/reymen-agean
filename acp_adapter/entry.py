"""CLI entry point for the ReYMeN-agent ACP adapter.

Loads environment variables from ``~/.ReYMeN/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ReYMeN_HOME (default ``~/.ReYMeN``)."""
    from ReYMeN_cli.env_loader import load_ReYMeN_dotenv

    ReYMeN_home = get_ReYMeN_home()
    loaded = load_ReYMeN_dotenv(ReYMeN_home=ReYMeN_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", ReYMeN_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="ReYMeN-acp",
        description="Run ReYMeN Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print ReYMeN version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive ReYMeN provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.ReYMeN/node/ "
...
def _print_version() -> None:
    from ReYMeN_cli import __version__ as ReYMeN_version

    print(ReYMeN_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import ReYMeNACPAgent  # noqa: F401

    print("ReYMeN ACP check OK")


def _run_setup() -> None:
    from ReYMeN_cli.main import main as ReYMeN_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "ReYMeN", "model"]
        ReYMeN_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``ReYMeN postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting ReYMeN-agent ACP adapter")
    ...
    import acp
    from .server import ReYMeNACPAgent
