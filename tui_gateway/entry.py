# -*- coding: utf-8 -*-
"""TUI gateway giriş noktası — stdio üzerinden JSON-RPC dispatcher."""
import os
import sys

# Guard against a local utils/ (or other package) in CWD shadowing installed
# ReYMeN modules.  ReYMeN_cli sets ReYMeN_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("ReYMeN_PYTHON_SRC_ROOT", "")
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ReYMeN_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ReYMeN_TUI_SIDECAR_URL")
...
# tested; override via ``ReYMeN_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ReYMeN_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ReYMeN_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``ReYMeN --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def main():
    ...
    # server stalled the whole shell on "summoning ReYMeN…" for the full
    ...
    for raw in sys.stdin:
        ...
        try:
            ...
            logger.exception("dispatch exception method=%r id=%r", method, req_id)
            resp = {
                "jsonrpc": "2.0",
