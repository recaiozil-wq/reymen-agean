"""Persistent slash-command worker — one ReYMeNCLI per TUI session.
...
def _run(cli: ReYMeNCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ReYMeN_SESSION_KEY"] = args.session_key
    os.environ["ReYMeN_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ReYMeNCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
